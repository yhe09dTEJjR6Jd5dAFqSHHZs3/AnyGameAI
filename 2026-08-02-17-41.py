...
RELEASE_SOURCE_SHA256 = "4e1a418ef9218ac23f4928f19cb040220b48ac639a5f5b1063a372ee43ae41cb"
...
SPATIAL_CONTEXT_CHANNELS = 3
LEGACY_SPATIAL_CONTEXT_DIM = (
    SPATIAL_FULL_PIXELS + SPATIAL_HUD_PIXELS + SPATIAL_MOUSE_PIXELS
)
SPATIAL_CONTEXT_DIM = SPATIAL_CONTEXT_CHANNELS * LEGACY_SPATIAL_CONTEXT_DIM
...
SPATIAL_BRANCH_FEATURE_DIM = 2 * SPATIAL_CONTEXT_CHANNELS * (
...
V83_FEATURE_DIM = BASE_FEATURE_DIM + LEGACY_SPATIAL_CONTEXT_DIM
...
OUTCOME_SCORE = 3
OUTCOME_LEVEL = 4
OUTCOME_TYPES = frozenset({OUTCOME_NONE, OUTCOME_DEATH, OUTCOME_VICTORY, OUTCOME_SCORE, OUTCOME_LEVEL})
SKILL_HEAD_SIZE = 128
SKILL_MIN_STEPS = 4
SKILL_MAX_STEPS = 32
...
WORLD_MODEL_OVERSHOOT_MIN_STEPS = 5
WORLD_MODEL_OVERSHOOT_MAX_STEPS = 20
...
VALIDATION_EPISODE_FRACTION = 0.20
...
WORLD_STOCHASTIC_SIZE = 16
RSSM_MIN_LOG_STD = -5.0
RSSM_MAX_LOG_STD = 2.0
RSSM_KL_FREE_NATS = 0.25
RSSM_KL_WEIGHT = 0.04
WORLD_MODEL_MAX_PLANNING_HORIZON = 20
WORLD_MODEL_SKILL_MIN_HORIZON = 8
WORLD_MODEL_SKILL_MAX_HORIZON = 32
...
MODEL_ARCHIVE_V20_REQUIRED_MEMBERS = MODEL_ARCHIVE_V19_REQUIRED_MEMBERS | frozenset({
...
MODEL_ARCHIVE_REQUIRED_MEMBERS = MODEL_ARCHIVE_V20_REQUIRED_MEMBERS | frozenset({
    ...
    "world_posterior_state_w.npy", "world_posterior_observation_w.npy",
    "world_posterior_b.npy",
    "world_continuation_w.npy", "world_continuation_b.npy",
})
...
GLOBAL_BACKBONE_REQUIRED_MEMBERS = frozenset({
    ...
    "training_rounds.npy", *(f"{key}.npy".casefold() for key in GLOBAL_BACKBONE_PARAMETER_KEYS),
})
GLOBAL_BACKBONE_ALLOWED_MEMBERS = GLOBAL_BACKBONE_REQUIRED_MEMBERS | frozenset({
    "updated_at.npy",
})
...
REQUIRED_SCRIPT_FUNCTIONS = frozenset({
    ...
    "train_world_model_overshooting", "world_model_planning_confidence",
    "distill_teacher_model", "online_adapter_hidden", "smooth_reward",
    "refresh_global_backbone", "decode_temporal_state_with_length",
    "_latest_temporal_hidden", "adaptive_action_catalog_limit",
    "rssm_prior", "rssm_posterior", "rssm_latent_from_state",
    "accelerated_temporal_policy", "live_challenger_passes",
    "promote_live_challenger",
})
...
REQUIRED_SCRIPT_CALLS = {
    ...
    "refresh_global_backbone": frozenset({"load_training_data", "pretrain_visual_encoder", "save_global_backbone"}),
...
def adaptive_runtime_settings(
    ...
    planning_horizon = max(4, min(WORLD_MODEL_MAX_PLANNING_HORIZON, int(config.get("planning_horizon", 8))))
    ...
    if tier == "low_numpy":
        ...
        latent_horizon = max(4, min(WORLD_MODEL_MAX_PLANNING_HORIZON, planning_horizon))
    else:
    ...
    return {
        ...
        "planning_horizon": max(4, min(WORLD_MODEL_MAX_PLANNING_HORIZON, planning_horizon)),
...
def configured_teacher_hidden_size(config: dict) -> int:
    try:
        requested = int(config.get("hidden_size", TEACHER_HIDDEN_SIZE))
    except (TypeError, ValueError, OverflowError):
        requested = TEACHER_HIDDEN_SIZE
    return max(256, min(TEACHER_HIDDEN_SIZE, requested))


def adaptive_action_catalog_limit(config: dict, runtime_tier: str) -> int:
    """Reserve catalog space for human controls on every supported tier."""
    tier_floor = {
        "low_numpy": 512,
        "mid_onnx": 768,
        "high_directml": 1024,
    }.get(str(runtime_tier), 512)
    try:
        configured = int(config.get("max_action_count", 512))
    except (TypeError, ValueError, OverflowError):
        configured = 512
    return max(512, min(1024, max(configured, tier_floor)))
...
DEFAULT_CONFIG = {
    ...
    "online_model_learning_rate": 0.00008,
    ...
    "behavior_cloning_weight": 2.2,
    "expert_occupancy_reward_weight": 0.40,
    "iql_expectile": 0.80,
    "iql_temperature": 1.5,
    ...
    "candidate_min_improvement": 0.005,
    ...
    "world_uncertainty_penalty": 0.25,
...
CONFIG_RANGES = {
    ...
    "planning_horizon": (1, WORLD_MODEL_MAX_PLANNING_HORIZON),
    ...
    "expert_occupancy_reward_weight": (0.0, 1.5),
            ...
            if source_schema < CONFIG_SCHEMA:
                if source_schema < 38:
                    if int(merged.get("max_action_count", 0)) == 256:
                        merged["max_action_count"] = 512
                    if abs(float(merged.get("behavior_cloning_weight", 0.0)) - 1.4) < 1e-12:
                        merged["behavior_cloning_weight"] = DEFAULT_CONFIG["behavior_cloning_weight"]
                    merged["expert_occupancy_reward_weight"] = DEFAULT_CONFIG[
                        "expert_occupancy_reward_weight"
                    ]
                if source_schema < 26:
                    if int(merged.get("max_action_count", 0)) == 256:
                ...
                if source_schema < 36 and int(merged.get("hidden_size", 0)) == DEFAULT_HIDDEN_SIZE:
                    merged["hidden_size"] = TEACHER_HIDDEN_SIZE
                if source_schema < 37:
                    merged["minimum_human_transitions"] = 512
                    merged["minimum_validation_episodes"] = 20
                    merged["validation_episode_fraction"] = 0.20
                    merged["candidate_min_improvement"] = 0.005
                    merged["planning_weight"] = 0.38
                    merged["planning_disagreement_penalty"] = 0.60
                    merged["world_uncertainty_penalty"] = 0.25
...
def _onnx_attribute_int(name: str, value: int) -> bytes:
    return (
        _protobuf_field_text(1, name)
        + _protobuf_field_varint(3, int(value))
        + _protobuf_field_varint(20, 2)
    )


def _onnx_attribute_ints(name: str, values: tuple[int, ...]) -> bytes:
    payload = bytearray(_protobuf_field_text(1, name))
    for value in values:
        payload.extend(_protobuf_field_varint(8, int(value)))
    payload.extend(_protobuf_field_varint(20, 7))
    return bytes(payload)


def _onnx_node_with_attributes(
    op_type: str,
    inputs: tuple[str, ...],
    outputs: tuple[str, ...],
    name: str,
    ...
    payload = bytearray()
    for value in inputs:
        payload.extend(_protobuf_field_text(1, value))
    for value in outputs:
        payload.extend(_protobuf_field_text(2, value))
    payload.extend(_protobuf_field_text(3, name))
    payload.extend(_protobuf_field_text(4, op_type))
    for attribute in attributes:
        payload.extend(_protobuf_field_bytes(5, attribute))
    return bytes(payload)
...
def quantized_vision_model_bytes(model_parameters: dict | None = None) -> bytes:
    ...
    initializers = (
        ...
        _onnx_tensor(
            "stem_w", 3,
            (CNN_CHANNELS, MODEL_PIXEL_CHANNELS, 3, 3), conv_values,
        ),
        _onnx_tensor("stem_w_scale", 1, (), struct.pack("<f", conv_scale)),
        _onnx_tensor("stem_w_zero", 3, (), bytes([0])),
        _onnx_tensor("stem_y_scale", 1, (), struct.pack("<f", 0.02)),
        _onnx_tensor("stem_y_zero", 2, (), bytes([0])),
        _onnx_tensor(
            "stem_bias", 6, (CNN_CHANNELS,),
            struct.pack("<" + "i" * CNN_CHANNELS, *conv_bias_values),
        ),
        _onnx_tensor(
            "conv2_depthwise", 1, (CNN_CHANNELS, 1, 3, 3), conv2_depthwise
        ),
        _onnx_tensor(
            "conv2_pointwise", 1,
            (CNN_MID_CHANNELS, CNN_CHANNELS, 1, 1), conv2_pointwise,
        ),
        _onnx_tensor("conv2_bias", 1, (CNN_MID_CHANNELS,), conv2_bias),
        _onnx_tensor(
            "conv3_depthwise", 1, (CNN_MID_CHANNELS, 1, 3, 3), conv3_depthwise
        ),
        _onnx_tensor(
            "conv3_pointwise", 1,
            (CNN_OUTPUT_CHANNELS, CNN_MID_CHANNELS, 1, 1), conv3_pointwise,
        ),
        _onnx_tensor("conv3_bias", 1, (CNN_OUTPUT_CHANNELS,), conv3_bias),
    )
    nodes = (
        _onnx_node_with_attributes(
            "QLinearConv",
            (
                "input", "x_scale", "x_zero", "stem_w", "stem_w_scale",
                "stem_w_zero", "stem_y_scale", "stem_y_zero", "stem_bias",
            ),
            ("stem_q",),
            "quantized_stem",
        ...
        _onnx_node(
            "DequantizeLinear",
            ("stem_q", "stem_y_scale", "stem_y_zero"),
            ("stem",),
            "dequantize_stem",
        ),
        _onnx_node_with_attributes(
            "Conv", ("stem", "conv2_depthwise"), ("middle_depthwise",),
            "middle_depthwise_conv",
            (
                _onnx_attribute_int("group", CNN_CHANNELS),
                _onnx_attribute_ints("strides", (2, 2)),
            ),
        ),
        _onnx_node(
            "Conv", ("middle_depthwise", "conv2_pointwise", "conv2_bias"),
            ("middle_pre",), "middle_pointwise_conv",
        ),
        _onnx_node("Relu", ("middle_pre",), ("middle",), "middle_relu"),
        _onnx_node_with_attributes(
            "Conv", ("middle", "conv3_depthwise"), ("final_depthwise",),
            "final_depthwise_conv",
            (
                _onnx_attribute_int("group", CNN_MID_CHANNELS),
                _onnx_attribute_ints("strides", (2, 2)),
            ),
        ),
        _onnx_node(
            "Conv", ("final_depthwise", "conv3_pointwise", "conv3_bias"),
            ("final_pre",), "final_pointwise_conv",
        ),
        _onnx_node("Relu", ("final_pre",), ("final",), "final_relu"),
    )
    graph = bytearray()
    for node in nodes:
        graph.extend(_protobuf_field_bytes(1, node))
    graph.extend(_protobuf_field_text(2, "AnyGameAIFullVisualEncoder"))
    for tensor in initializers:
        graph.extend(_protobuf_field_bytes(5, tensor))
    graph.extend(
        _protobuf_field_bytes(
    ...
    for name, dims in (
        ("stem", (1, CNN_CHANNELS, stem_height, stem_width)),
        ("middle", (1, CNN_MID_CHANNELS, middle_height, middle_width)),
        ("final", (1, CNN_OUTPUT_CHANNELS, final_height, final_width)),
    ):
...
def temporal_policy_model_bytes(model: dict) -> bytes:
    """Export frame projection, recurrent memory, adapter, and policy head."""
    np = import_numpy(False)
    hidden_size = int(model["hidden_size"])
    combined_size = int(model["frame_proj"].shape[0])
    action_count = int(len(model["action_factors"]))
    if action_count <= 0:
        raise ValueError("策略动作表为空")

    def tensor(name: str, value) -> bytes:
        array = np.asarray(value, dtype=np.float32)
        return _onnx_tensor(name, 1, tuple(int(v) for v in array.shape), array.tobytes(order="C"))

    memory_width = max(8, min(hidden_size // 4, hidden_size // 2))
    ...
    memory_gru = np.eye(hidden_size, dtype=np.float32)
    memory_previous = np.zeros((hidden_size, hidden_size), dtype=np.float32)
    memory_start = hidden_size - memory_width
    for offset in range(memory_width):
        output = memory_start + offset
        memory_gru[output, output] = 0.65
        memory_gru[offset, output] += 0.35 * (1.0 - memory_decays[offset])
        memory_previous[output, output] = 0.35 * memory_decays[offset]

    factors = model["action_factors"].astype(np.int64, copy=False)
    control_selector = np.zeros((len(CONTROL_KINDS), action_count), dtype=np.float32)
    mouse_selector = np.zeros((MOUSE_HEAD_SIZE, action_count), dtype=np.float32)
    for action in range(action_count):
        control_selector[int(factors[action, 0]), action] = 1.0
        mouse_selector[int(factors[action, 1]), action] = 1.0
    initializers = [
        tensor("frame_proj", model["frame_proj"]),
        tensor("frame_bias", model["frame_bias"]),
        tensor("wz", model["Wz"]), tensor("uz", model["Uz"]), tensor("bz", model["bz"]),
        tensor("wr", model["Wr"]), tensor("ur", model["Ur"]), tensor("br", model["br"]),
        tensor("wh", model["Wh"]), tensor("uh", model["Uh"]), tensor("bh", model["bh"]),
        tensor("memory_gru", memory_gru),
        tensor("memory_previous", memory_previous),
        tensor("online_adapter_w", model["online_adapter_w"]),
        tensor("online_adapter_b", model["online_adapter_b"]),
        tensor("policy_control_w", model["policy_control_w"]),
        tensor("policy_control_b", model["policy_control_b"]),
        tensor("policy_key_w", model["policy_key_w"]),
        tensor("policy_key_b", model["policy_key_b"]),
        tensor("policy_mouse_w", model["policy_mouse_w"]),
        tensor("policy_mouse_b", model["policy_mouse_b"]),
        tensor("policy_button_w", model["policy_button_w"]),
        tensor("policy_button_b", model["policy_button_b"]),
        tensor("policy_action_w", model["policy_action_w"]),
        tensor("policy_action_b", model["policy_action_b"]),
        tensor("control_selector", control_selector),
        tensor("mouse_selector", mouse_selector),
        tensor("key_selector", model["action_key_multihot"].T),
        tensor("button_selector", model["action_button_multihot"].T),
        tensor("one", np.asarray([1.0], dtype=np.float32)),
        tensor("online_scale", np.asarray([ONLINE_ADAPTER_SCALE], dtype=np.float32)),
        tensor("quarter", np.asarray([0.25], dtype=np.float32)),
        tensor("factor_weight", np.asarray([0.70], dtype=np.float32)),
        tensor("exact_weight", np.asarray([0.30], dtype=np.float32)),
    ]
    nodes: list[bytes] = []
    previous = "initial_hidden"
    for step in range(TEMPORAL_FRAMES):
        prefix = f"step{step}"
        nodes.extend(
            (
                _onnx_node("MatMul", (prefix, "frame_proj"), (prefix + "_projected_linear",), prefix + "_project"),
                _onnx_node("Add", (prefix + "_projected_linear", "frame_bias"), (prefix + "_projected_pre",), prefix + "_project_bias"),
                _onnx_node("Tanh", (prefix + "_projected_pre",), (prefix + "_projected",), prefix + "_project_tanh"),
                _onnx_node("MatMul", (prefix + "_projected", "wz"), (prefix + "_z_input",), prefix + "_z_input_mm"),
                _onnx_node("MatMul", (previous, "uz"), (prefix + "_z_memory",), prefix + "_z_memory_mm"),
                _onnx_node("Add", (prefix + "_z_input", prefix + "_z_memory"), (prefix + "_z_sum",), prefix + "_z_add"),
                _onnx_node("Add", (prefix + "_z_sum", "bz"), (prefix + "_z_pre",), prefix + "_z_bias"),
                _onnx_node("Sigmoid", (prefix + "_z_pre",), (prefix + "_z",), prefix + "_z_sigmoid"),
                _onnx_node("MatMul", (prefix + "_projected", "wr"), (prefix + "_r_input",), prefix + "_r_input_mm"),
                _onnx_node("MatMul", (previous, "ur"), (prefix + "_r_memory",), prefix + "_r_memory_mm"),
                _onnx_node("Add", (prefix + "_r_input", prefix + "_r_memory"), (prefix + "_r_sum",), prefix + "_r_add"),
                _onnx_node("Add", (prefix + "_r_sum", "br"), (prefix + "_r_pre",), prefix + "_r_bias"),
                _onnx_node("Sigmoid", (prefix + "_r_pre",), (prefix + "_r",), prefix + "_r_sigmoid"),
                _onnx_node("Mul", (prefix + "_r", previous), (prefix + "_reset_memory",), prefix + "_reset_mul"),
                _onnx_node("MatMul", (prefix + "_projected", "wh"), (prefix + "_candidate_input",), prefix + "_candidate_input_mm"),
                _onnx_node("MatMul", (prefix + "_reset_memory", "uh"), (prefix + "_candidate_memory",), prefix + "_candidate_memory_mm"),
                _onnx_node("Add", (prefix + "_candidate_input", prefix + "_candidate_memory"), (prefix + "_candidate_sum",), prefix + "_candidate_add"),
                _onnx_node("Add", (prefix + "_candidate_sum", "bh"), (prefix + "_candidate_pre",), prefix + "_candidate_bias"),
                _onnx_node("Tanh", (prefix + "_candidate_pre",), (prefix + "_candidate",), prefix + "_candidate_tanh"),
                _onnx_node("Sub", ("one", prefix + "_z"), (prefix + "_one_minus_z",), prefix + "_inverse_z"),
                _onnx_node("Mul", (prefix + "_one_minus_z", previous), (prefix + "_kept",), prefix + "_keep_memory"),
                _onnx_node("Mul", (prefix + "_z", prefix + "_candidate"), (prefix + "_updated",), prefix + "_update_memory"),
                _onnx_node("Add", (prefix + "_kept", prefix + "_updated"), (prefix + "_gru",), prefix + "_gru_add"),
                _onnx_node("MatMul", (prefix + "_gru", "memory_gru"), (prefix + "_slow_gru",), prefix + "_slow_gru_mm"),
                _onnx_node("MatMul", (previous, "memory_previous"), (prefix + "_slow_previous",), prefix + "_slow_previous_mm"),
                _onnx_node("Add", (prefix + "_slow_gru", prefix + "_slow_previous"), (prefix + "_candidate_hidden",), prefix + "_slow_add"),
                _onnx_node("Mul", (f"mask{step}", prefix + "_candidate_hidden"), (prefix + "_masked_candidate",), prefix + "_mask_candidate"),
                _onnx_node("Sub", ("one", f"mask{step}"), (prefix + "_inverse_mask",), prefix + "_inverse_mask_node"),
                _onnx_node("Mul", (prefix + "_inverse_mask", previous), (prefix + "_masked_previous",), prefix + "_mask_previous"),
                _onnx_node("Add", (prefix + "_masked_candidate", prefix + "_masked_previous"), (prefix + "_hidden",), prefix + "_hidden_add"),
            )
        )
        previous = prefix + "_hidden"
    nodes.extend(
        (
            _onnx_node("Identity", (previous,), ("hidden",), "hidden_output"),
            _onnx_node("Mul", (previous, "online_adapter_w"), ("adapter_product",), "adapter_multiply"),
            _onnx_node("Add", ("adapter_product", "online_adapter_b"), ("adapter_pre",), "adapter_bias"),
            _onnx_node("Tanh", ("adapter_pre",), ("adapter_residual",), "adapter_tanh"),
            _onnx_node("Mul", ("adapter_residual", "online_scale"), ("adapter_scaled",), "adapter_scale"),
            _onnx_node("Add", (previous, "adapter_scaled"), ("adapted_hidden",), "adapter_add"),
            _onnx_node("MatMul", ("adapted_hidden", "policy_control_w"), ("control_linear",), "policy_control_mm"),
            _onnx_node("Add", ("control_linear", "policy_control_b"), ("control_logits",), "policy_control_add"),
            _onnx_node("MatMul", ("adapted_hidden", "policy_key_w"), ("key_linear",), "policy_key_mm"),
            _onnx_node("Add", ("key_linear", "policy_key_b"), ("key_logits",), "policy_key_add"),
            _onnx_node("MatMul", ("adapted_hidden", "policy_mouse_w"), ("mouse_linear",), "policy_mouse_mm"),
            _onnx_node("Add", ("mouse_linear", "policy_mouse_b"), ("mouse_logits",), "policy_mouse_add"),
            _onnx_node("MatMul", ("adapted_hidden", "policy_button_w"), ("button_linear",), "policy_button_mm"),
            _onnx_node("Add", ("button_linear", "policy_button_b"), ("button_logits",), "policy_button_add"),
            _onnx_node("MatMul", ("control_logits", "control_selector"), ("control_action",), "select_control"),
            _onnx_node("MatMul", ("mouse_logits", "mouse_selector"), ("mouse_action",), "select_mouse"),
            _onnx_node("MatMul", ("key_logits", "key_selector"), ("key_action",), "select_keys"),
            _onnx_node("MatMul", ("button_logits", "button_selector"), ("button_action",), "select_buttons"),
            _onnx_node("Add", ("control_action", "mouse_action"), ("factor_pair_a",), "factor_add_a"),
            _onnx_node("Add", ("key_action", "button_action"), ("factor_pair_b",), "factor_add_b"),
            _onnx_node("Add", ("factor_pair_a", "factor_pair_b"), ("factor_sum",), "factor_add"),
            _onnx_node("Mul", ("factor_sum", "quarter"), ("factor_logits",), "factor_average"),
            _onnx_node("MatMul", ("adapted_hidden", "policy_action_w"), ("exact_linear",), "policy_action_mm"),
            _onnx_node("Add", ("exact_linear", "policy_action_b"), ("exact_logits",), "policy_action_add"),
            _onnx_node("Mul", ("factor_logits", "factor_weight"), ("factor_weighted",), "factor_blend"),
            _onnx_node("Mul", ("exact_logits", "exact_weight"), ("exact_weighted",), "exact_blend"),
            _onnx_node("Add", ("factor_weighted", "exact_weighted"), ("policy_logits",), "policy_blend"),
            _onnx_node_with_attributes("Softmax", ("policy_logits",), ("probabilities",), "policy_softmax", (_onnx_attribute_int("axis", 1),)),
        )
    )
    graph = bytearray()
    for node in nodes:
        graph.extend(_protobuf_field_bytes(1, node))
    graph.extend(_protobuf_field_text(2, "AnyGameAITemporalPolicy"))
    for initializer in initializers:
        graph.extend(_protobuf_field_bytes(5, initializer))
    for step in range(TEMPORAL_FRAMES):
        graph.extend(
            _protobuf_field_bytes(
                11, _onnx_value_info(f"step{step}", 1, (1, combined_size))
            )
        )
        graph.extend(
            _protobuf_field_bytes(11, _onnx_value_info(f"mask{step}", 1, (1, 1)))
        )
    graph.extend(
        _protobuf_field_bytes(
            11, _onnx_value_info("initial_hidden", 1, (1, hidden_size))
        )
    )
    graph.extend(_protobuf_field_bytes(12, _onnx_value_info("hidden", 1, (1, hidden_size))))
    graph.extend(_protobuf_field_bytes(12, _onnx_value_info("probabilities", 1, (1, action_count))))
    graph.extend(_protobuf_field_bytes(12, _onnx_value_info("adapted_hidden", 1, (1, hidden_size))))
    output = bytearray()
    output.extend(_protobuf_field_varint(1, 8))
    output.extend(_protobuf_field_text(2, APP_NAME))
    output.extend(_protobuf_field_text(3, APP_VERSION))
    output.extend(_protobuf_field_bytes(7, bytes(graph)))
    output.extend(_protobuf_field_bytes(8, _protobuf_field_varint(2, 13)))
    return bytes(output)


def accelerated_temporal_policy_session(model: dict):
    if str(model.get("runtime_tier", "low_numpy")) == "low_numpy":
        return None
    try:
        revision = (
            int(model.get("optimizer_step", 0)),
            int(model.get("online_updates", 0)),
            str(model.get("action_hash", "")),
        )
        cached_export = model.get("_temporal_onnx_export")
        if (
            isinstance(cached_export, tuple)
            and len(cached_export) == 2
            and cached_export[0] == revision
            ...
            payload = temporal_policy_model_bytes(model)
            model["_temporal_onnx_export"] = (revision, payload)
        key = "temporal:" + hashlib.sha256(payload).hexdigest()
    except Exception:
        return None
    with ONNX_SESSION_LOCK:
        cached = ONNX_SESSION_CACHE.get(key)
...
def accelerated_temporal_policy(
    np,
    model: dict,
    combined_steps: list,
    initial_hidden,
):
    session = accelerated_temporal_policy_session(model)
    if session is None or not 1 <= len(combined_steps) <= TEMPORAL_FRAMES:
        return None
    combined_size = int(model["frame_proj"].shape[0])
    feed = {
        "initial_hidden": np.asarray(initial_hidden, dtype=np.float32).reshape(1, -1)
    }
    for step in range(TEMPORAL_FRAMES):
        valid = step < len(combined_steps)
        feed[f"step{step}"] = (
            np.asarray(combined_steps[step], dtype=np.float32).reshape(1, combined_size)
            if valid
            else np.zeros((1, combined_size), dtype=np.float32)
        )
        feed[f"mask{step}"] = np.asarray([[1.0 if valid else 0.0]], dtype=np.float32)
    try:
        hidden, probabilities, adapted = session.run(
            ["hidden", "probabilities", "adapted_hidden"], feed
        )
        hidden = hidden[0].astype(np.float32, copy=False)
        probabilities = probabilities[0].astype(np.float32, copy=False)
        adapted = adapted[0].astype(np.float32, copy=False)
        if (
            hidden.shape != (int(model["hidden_size"]),)
            or probabilities.shape != (len(model["action_factors"]),)
            or not np.isfinite(hidden).all()
            or not np.isfinite(probabilities).all()
        ):
            return None
        cache_key = hashlib.blake2s(hidden.tobytes(), digest_size=8).digest()
        model["_accelerated_policy_cache"] = (
            cache_key,
            probabilities.copy(),
            adapted.copy(),
        )
...
class ScreenSampler:
    ...
    def _build_spatial_context(self, raw: bytes) -> bytes:
        ...
        if self.width != SPATIAL_FULL_WIDTH or self.height != SPATIAL_FULL_HEIGHT:
            return bytes(SPATIAL_CONTEXT_DIM)
        red_plane = bytearray(SPATIAL_FULL_PIXELS)
        green_plane = bytearray(SPATIAL_FULL_PIXELS)
        blue_plane = bytearray(SPATIAL_FULL_PIXELS)
        luminance = bytearray(SPATIAL_FULL_PIXELS)
        ...
        # Locate stable, high-contrast HUD rows instead of assuming that every
        # game places its score and health at the exact screen edges.
        row_scores: list[float] = []
        for row in range(SPATIAL_FULL_HEIGHT):
            values = luminance[
                row * SPATIAL_FULL_WIDTH:(row + 1) * SPATIAL_FULL_WIDTH
            ]
            contrast = max(values) - min(values)
            horizontal_edges = sum(
                abs(int(values[index]) - int(values[index - 1]))
                for index in range(1, SPATIAL_FULL_WIDTH)
            ) / max(1, SPATIAL_FULL_WIDTH - 1)
            edge_prior = 1.0 - min(
                1.0,
                abs((row + 0.5) / SPATIAL_FULL_HEIGHT - 0.5) * 1.65,
            )
            row_scores.append(
                0.55 * contrast + 0.35 * horizontal_edges + 8.0 * (1.0 - edge_prior)
            )

        half = SPATIAL_FULL_HEIGHT // 2
        top_center = max(range(half), key=lambda row: row_scores[row])
        bottom_center = max(
            range(half, SPATIAL_FULL_HEIGHT), key=lambda row: row_scores[row]
        )

        def hud_strip(plane: bytearray, center: int) -> bytes:
            top = max(
                0,
                min(
                    SPATIAL_FULL_HEIGHT - SPATIAL_HUD_STRIP_HEIGHT,
                    int(center) - SPATIAL_HUD_STRIP_HEIGHT // 2,
                ),
            )
            start = top * SPATIAL_FULL_WIDTH
            end = start + SPATIAL_HUD_STRIP_HEIGHT * SPATIAL_FULL_WIDTH
            return bytes(plane[start:end])

        hud_planes = b"".join(
            hud_strip(plane, top_center) + hud_strip(plane, bottom_center)
            ...
            for target_y in range(SPATIAL_MOUSE_SIZE):
                ...
                for target_x in range(SPATIAL_MOUSE_SIZE):
                    ...
                    if 0 <= source_x < SPATIAL_FULL_WIDTH:
                        mouse[target_y * SPATIAL_MOUSE_SIZE + target_x] = plane[
                            source_y * SPATIAL_FULL_WIDTH + source_x
                        ]
            mouse_planes.append(bytes(mouse))
        context = (
            bytes(red_plane)
            + bytes(green_plane)
            + bytes(blue_plane)
        ...
        if len(context) != SPATIAL_CONTEXT_DIM:
            raise RuntimeError("高精度 RGB 空间分支尺寸无效")
        return context
...
def learned_control_preferences(profile: dict, database_path: Path) -> dict[str, float]:
    ...
    try:
        ...
        try: rows=connection.execute("SELECT action,COUNT(*) FROM transitions WHERE source='human' AND capture_failed=0 GROUP BY action").fetchall()
...
def profile_paths(profile_id: str, repair_unsafe: bool = False) -> dict[str, Path]:
    ...
    paths = {
        ...
        "student_low_candidate": root / "student_low_online_candidate.npz",
        "student_mid_candidate": root / "student_mid_online_candidate.npz",
        "student_high_candidate": root / "student_high_online_candidate.npz",
        "student_low_candidate_target": root / "student_low_online_candidate_target.npz",
        "student_mid_candidate_target": root / "student_mid_online_candidate_target.npz",
        "student_high_candidate_target": root / "student_high_online_candidate_target.npz",
    }
    managed_files = (
        ...
        paths["student_low_candidate"], paths["student_mid_candidate"], paths["student_high_candidate"],
        paths["student_low_candidate_target"], paths["student_mid_candidate_target"],
        paths["student_high_candidate_target"],
...
def runtime_student_candidate_paths(paths: dict[str, Path], runtime_tier: str) -> tuple[Path, Path]:
    tier = runtime_student_tier(runtime_tier)
    return paths[f"student_{tier}_candidate"], paths[f"student_{tier}_candidate_target"]
...
def _empty_live_model_statistics() -> dict:
    return {
        "sessions": 0,
        "episodes": 0,
        "victories": 0,
...
def ensure_live_model_evaluation(profile: dict) -> dict:
    raw_root = profile.get("live_model_evaluation")
    raw_tiers = raw_root.get("tiers", {}) if isinstance(raw_root, dict) else {}
    tiers = {}
    for tier in ("low", "mid", "high"):
        raw_tier = raw_tiers.get(tier, {}) if isinstance(raw_tiers, dict) else {}
        cleaned_tier = {"generation": max(0, int(raw_tier.get("generation", 0) or 0))}
        for variant in ("champion", "challenger"):
            raw = raw_tier.get(variant, {}) if isinstance(raw_tier, dict) else {}
            stats = _empty_live_model_statistics()
            for key in ("sessions", "episodes", "victories", "deaths", "levels", "steps"):
                try:
                    stats[key] = max(0, min(1_000_000_000, int(raw.get(key, 0))))
                except (AttributeError, TypeError, ValueError):
                    stats[key] = 0
            for key in ("score_total", "safety_total"):
                try:
                    value = float(raw.get(key, 0.0))
                except (AttributeError, TypeError, ValueError):
                    value = 0.0
                stats[key] = value if math.isfinite(value) else 0.0
            stats["safety_total"] = max(0.0, stats["safety_total"])
            stats["updated_at"] = str(raw.get("updated_at", ""))[:64] if isinstance(raw, dict) else ""
            cleaned_tier[variant] = stats
        tiers[tier] = cleaned_tier
    root = {"schema": 1, "tiers": tiers}
    profile["live_model_evaluation"] = root
    return root
...
def record_live_model_session(
    ...
    stats = root["tiers"][tier][
        "challenger" if variant == "challenger" else "champion"
    ]
    stats["sessions"] += 1
    for key in ("episodes", "victories", "deaths", "levels", "steps"):
        stats[key] += max(0, int(metrics.get(key, 0)))
    stats["score_total"] += float(metrics.get("score_total", 0.0))
    stats["safety_total"] += max(0.0, float(metrics.get("safety_total", 0.0)))
...
def _live_model_rates(stats: dict) -> dict[str, float]:
    ...
    return {
        "win_rate": float(stats.get("victories", 0)) / episodes,
        "death_rate": float(stats.get("deaths", 0)) / episodes,
        "score": float(stats.get("score_total", 0.0)) / episodes,
        "safety": float(stats.get("safety_total", 0.0)) / steps,
    }
...
def live_challenger_passes(profile: dict, runtime_tier: str, minimum_improvement: float) -> bool:
    ...
    if min(int(champion["sessions"]), int(challenger["sessions"])) < 2:
        return False
    if min(int(champion["episodes"]), int(challenger["episodes"])) < 2:
        return False
    left = _live_model_rates(champion)
    right = _live_model_rates(challenger)
    safe = right["safety"] <= left["safety"] + 0.02
    survival = right["death_rate"] <= left["death_rate"] + 0.05
    preserves_wins = right["win_rate"] >= left["win_rate"] - 0.02
    preserves_score = right["score"] >= left["score"] - max(0.02, abs(left["score"]) * 0.05)
    improves = (
        right["win_rate"] >= left["win_rate"] + 0.05
        or right["score"] >= left["score"] + max(0.0, float(minimum_improvement))
    )
    return bool(safe and survival and preserves_wins and preserves_score and improves)


def _atomic_model_copy(source: Path, destination: Path) -> None:
    if not source.is_file() or source.is_symlink():
        raise RuntimeError("候选模型文件无效")
    destination.parent.mkdir(parents=True, exist_ok=True)
    temp = temporary_sibling_path(destination, ".promote")
    temp.unlink(missing_ok=True)
    try:
...
def promote_live_challenger(
    ...
    all_paths: dict[str, Path] | None = None,
) -> bool:
    if not candidate_path.is_file() or not candidate_target_path.is_file():
        return False
    _atomic_model_copy(candidate_path, active_path)
    _atomic_model_copy(candidate_target_path, active_target_path)
    if isinstance(all_paths, dict):
        teacher_candidate = all_paths.get("candidate_model")
        teacher_target_candidate = all_paths.get("candidate_target_model")
        if (
            isinstance(teacher_candidate, Path)
            and isinstance(teacher_target_candidate, Path)
            and teacher_candidate.is_file()
            and teacher_target_candidate.is_file()
        ):
            for destination_name in ("model", "best_model"):
                _atomic_model_copy(
                    teacher_candidate, all_paths[destination_name]
                )
            for destination_name in ("target_model", "best_target_model"):
                _atomic_model_copy(
                    teacher_target_candidate, all_paths[destination_name]
                )
            teacher_candidate.unlink(missing_ok=True)
            teacher_target_candidate.unlink(missing_ok=True)
    tier = runtime_student_tier(runtime_tier)
    root = ensure_live_model_evaluation(profile)["tiers"][tier]
    root["generation"] = int(root.get("generation", 0)) + 1
    root["champion"] = dict(root["challenger"])
    root["challenger"] = _empty_live_model_statistics()
    candidate_path.unlink(missing_ok=True)
    candidate_target_path.unlink(missing_ok=True)
    return True
    ...
    return {
        ...
        "live_model_evaluation": {
            ...
            "tiers": {
                tier: {
                    "generation": 0,
                    "champion": _empty_live_model_statistics(),
                    "challenger": _empty_live_model_statistics(),
                }
                for tier in ("low", "mid", "high")
            },
        },
...
def action_metadata_ready(profile: dict) -> bool:
    ...
    if not isinstance(profile.get("skills"), list):
        return False
    if not isinstance(profile.get("live_model_evaluation"), dict):
        return False
...
def autoregressive_action_beam(
    np,
    model: dict,
    hidden,
    profile_actions: list[dict],
    ...
    beam_width: int = ACTION_GENERATION_BEAM_WIDTH,
) -> list[dict]:
    """Generate control→keys→buttons→pointer→duration candidates."""
    width = max(16, min(32, int(beam_width)))
    hidden_value = online_adapter_hidden(
        np, model, np.asarray(hidden, dtype=np.float32)
    )
    policy_heads, q_heads = _model_head_outputs(np, model, hidden_value)
    control_logits, key_logits, mouse_logits, button_logits, _ = policy_heads
    control_q, key_q, mouse_q, button_q, _ = q_heads
    critic = critic_weight_vector(np, critic_weights)

    def log_probability(logits, index: int) -> float:
        values = np.asarray(logits, dtype=np.float64)
        maximum = float(values.max(initial=0.0))
        normalizer = maximum + math.log(
            max(1e-12, float(np.exp(np.clip(values - maximum, -60.0, 30.0)).sum()))
        )
        return float(values[int(index)] - normalizer)

    def critic_score(head, index: int) -> float:
        return float(np.asarray(head, dtype=np.float32)[:, int(index)] @ critic)

    def prune(items: list[dict]) -> list[dict]:
        items.sort(key=lambda item: float(item["factorized_generated_score"]), reverse=True)
        return items[:width]

    control_order = [
        int(value)
        for value in np.argsort(np.asarray(control_logits, dtype=np.float64))[::-1]
    ]
    beams = []
        ...
        beams.append(
            {
                ...
                "action": normalized_action({}),
                "duration_index": DURATION_HEAD_SIZE // 2,
                "factorized_generated_score": (
                    log_probability(control_logits, kind)
                    + 0.35 * critic_score(control_q, kind)
                ),
            }
        )
    beams = prune(beams)
    ...
    safe_keys = sorted(
        (int(key) for key in SAFE_KEY_VKS if 0 <= int(key) < KEY_HEAD_SIZE),
        key=lambda key: float(key_logits[key]),
        reverse=True,
    )[:8]
    expanded = []
    for beam in beams:
        kind = int(beam["control_kind"])
        if CONTROL_KINDS[kind] in ("keyboard", "mixed"):
            key_sets = [(key,) for key in safe_keys[:4]]
            if len(safe_keys) >= 2:
                key_sets.extend((tuple(safe_keys[:2]), tuple((safe_keys[0], safe_keys[2]))))
            if CONTROL_KINDS[kind] == "mixed":
                key_sets.append(())
        else:
            key_sets = [()]
        for keys in key_sets:
            ...
            candidate["action"] = normalized_action(action)
            if keys:
                key_policy = sum(
                    math.log(max(1e-9, float(_sigmoid(np, key_logits[key]))))
                    for key in keys
                ) / math.sqrt(len(keys))
                key_value = sum(critic_score(key_q, key) for key in keys) / math.sqrt(len(keys))
                candidate["factorized_generated_score"] += key_policy + 0.35 * key_value
            expanded.append(candidate)
    beams = prune(expanded)

    button_names = ("left", "right", "middle", "x1", "x2")
    button_order = [
        int(value)
        for value in np.argsort(np.asarray(button_logits, dtype=np.float64))[::-1]
    ]
    expanded = []
    for beam in beams:
        kind_name = CONTROL_KINDS[int(beam["control_kind"])]
        if kind_name in ("click", "mixed"):
            button_sets = [(button_order[0],)]
            button_sets.extend((index,) for index in button_order[1:3])
            button_sets.append(tuple(button_order[:2]))
            if kind_name == "mixed":
                button_sets.append(())
        else:
            button_sets = [()]
        for indices in button_sets:
            candidate = dict(beam)
            action = dict(candidate["action"])
            action["buttons"] = [button_names[index] for index in indices]
            candidate["action"] = normalized_action(action)
            if indices:
                button_policy = sum(
                    math.log(max(1e-9, float(_sigmoid(np, button_logits[index]))))
                    for index in indices
                ) / math.sqrt(len(indices))
                button_value = sum(critic_score(button_q, index) for index in indices) / math.sqrt(len(indices))
                candidate["factorized_generated_score"] += button_policy + 0.35 * button_value
            expanded.append(candidate)
    beams = prune(expanded)
    ...
    absolute = [
        ...
        for value in np.argsort(
            np.asarray(mouse_logits[1:1 + MOUSE_GRID_WIDTH * MOUSE_GRID_HEIGHT], dtype=np.float64)
        )[::-1][:4]
    ]
    relative = [
        577 + int(value)
        for value in np.argsort(np.asarray(mouse_logits[577:602], dtype=np.float64))[::-1][:3]
    ]
    wheel = [
        602 + int(value)
        for value in np.argsort(np.asarray(mouse_logits[602:607], dtype=np.float64))[::-1][:2]
    ]
    expanded = []
    for beam in beams:
        kind_name = CONTROL_KINDS[int(beam["control_kind"])]
        if kind_name in ("pointer", "click", "mixed"):
            mouse_buckets = absolute + relative
            if kind_name in ("click", "mixed"):
                mouse_buckets.append(0)
        elif kind_name == "wheel":
            mouse_buckets = wheel
        else:
            mouse_buckets = [0]
        for bucket in mouse_buckets:
            candidate = dict(beam)
            action = dict(candidate["action"])
            action.update({"mouse_x": -1, "mouse_y": -1, "mouse_dx": 0, "mouse_dy": 0, "mouse_wheel": 0})
            if 1 <= bucket <= MOUSE_GRID_WIDTH * MOUSE_GRID_HEIGHT:
                index = bucket - 1
                action["mouse_x"] = index % MOUSE_GRID_WIDTH
                action["mouse_y"] = index // MOUSE_GRID_WIDTH
            elif 577 <= bucket < 602:
                index = bucket - 577
                action["mouse_dx"] = index % 5 - 2
                action["mouse_dy"] = index // 5 - 2
            elif 602 <= bucket < 607:
                action["mouse_wheel"] = bucket - 604
            candidate["action"] = normalized_action(action)
            candidate["factorized_generated_score"] += (
                log_probability(mouse_logits, bucket)
                + 0.35 * critic_score(mouse_q, bucket)
            )
            expanded.append(candidate)
    beams = prune(expanded)
    ...
    for beam in beams:
        kind = int(beam["control_kind"])
        duration_probability, duration_q = conditional_duration_outputs(
            np, model, hidden_value, kind
        )
        duration_value = np.asarray(duration_q, dtype=np.float32).T @ critic
        duration_score = (
            np.log(np.maximum(duration_probability, 1e-9))
            + 0.35 * duration_value
        )
        for duration in np.argsort(duration_score)[::-1][:2]:
            candidate = dict(beam)
            candidate["duration_index"] = int(duration)
            candidate["factorized_generated_score"] += float(duration_score[int(duration)])
            expanded.append(candidate)
    beams = prune(expanded)

    allowed = [
        index
        for index in range(len(profile_actions))
        if not blocked_actions or index not in blocked_actions
    ]
    if not allowed:
        return []
    memory_probabilities = np.asarray(memorized_probabilities, dtype=np.float64)
    memory_values = np.asarray(memorized_values, dtype=np.float64)
    plan = (
        np.zeros_like(memory_values)
        if planning_values is None
        else np.asarray(planning_values, dtype=np.float64)
    )
    risk = (
        np.zeros_like(memory_values)
        if uncertainty is None
        else np.asarray(uncertainty, dtype=np.float64)
    )
    planning_weight = (
        max(0.0, min(1.0, float(critic_weights.get("planning_weight", 0.38))))
        if isinstance(critic_weights, dict)
        else 0.38
    )
    unique: dict[tuple[str, int], dict] = {}
    allowed_actions = [profile_actions[index] for index in allowed]
    for beam in beams:
        action = normalized_action(beam["action"])
        local_index = nearest_action_index(allowed_actions, action)
        memory_index = allowed[local_index]
        memorized_score = (
            math.log(max(1e-9, float(memory_probabilities[memory_index])))
            + float(memory_values[memory_index])
            + planning_weight * float(plan[memory_index])
            - 0.25 * float(risk[memory_index])
        )
        candidate = dict(beam)
        candidate["action"] = action
        candidate["memorized_action_id"] = int(memory_index)
        candidate["memorized_action_score"] = float(memorized_score)
        key = (action_signature(action), int(candidate["duration_index"]))
        previous = unique.get(key)
        if previous is None or float(candidate["factorized_generated_score"]) > float(previous["factorized_generated_score"]):
    ...
    def normalized_scores(values):
        ...
        scale = max(0.25, float(np.std(array)))
        return np.tanh((array - center) / scale)

    generated = normalized_scores(
        [float(item["factorized_generated_score"]) for item in candidates]
    )
    memorized = normalized_scores(
        [float(item["memorized_action_score"]) for item in candidates]
    )
    for index, candidate in enumerate(candidates):
        candidate["final_action_score"] = (
            0.70 * float(generated[index])
            + 0.30 * float(memorized[index])
        )
    candidates.sort(key=lambda item: float(item["final_action_score"]), reverse=True)
...
def _pack_temporal_payload(frames: list[bytes], action_history: list[int], duration_history: list[float]) -> bytes:
    normalized_frames = [
        normalize_feature_bytes(frame) for frame in frames[-TEMPORAL_FRAMES:]
    ]
    if not normalized_frames:
        normalized_frames = [bytes(FEATURE_DIM)]
    valid_history_length = len(normalized_frames)
...
def _legacy_temporal_valid_length(frames: list[bytes], actions: list[int]) -> int:
    start = 0
    while (
        start + 1 < len(frames)
        and int(actions[start]) == -1
        and int(actions[start + 1]) == -1
        and frames[start] == frames[start + 1]
    ):
        start += 1
    return max(1, len(frames) - start)


def decode_temporal_state_with_length(
    state: bytes,
) -> tuple[list[bytes], list[int], list[float], int]:
    ...
    try:
        ...
        candidate = raw
    magic = b""
    for supported_magic in (
        TEMPORAL_STATE_MAGIC,
        LEGACY_TEMPORAL_STATE_MAGIC,
        LEGACY_TEMPORAL_STATE_MAGIC_V2,
    ):
        if candidate.startswith(supported_magic):
            magic = supported_magic
            break
    if not magic:
        try:
        ...
        return (
            ...
            [0.0] * ACTION_HISTORY_LENGTH,
            1,
        )
    offset = len(magic)
    header_size = 3 if magic == TEMPORAL_STATE_MAGIC else 2
    if offset + header_size > len(candidate):
        raise ValueError("时序状态内容无效")
    if magic == TEMPORAL_STATE_MAGIC:
        frame_count, history_count, valid_history_length = struct.unpack_from(
            "<BBB", candidate, offset
        )
    else:
        frame_count, history_count = struct.unpack_from("<BB", candidate, offset)
        valid_history_length = 0
    offset += header_size
    if magic == TEMPORAL_STATE_MAGIC:
        valid_shape = (
            frame_count == TEMPORAL_FRAMES
            and history_count == ACTION_HISTORY_LENGTH
            and 1 <= valid_history_length <= TEMPORAL_FRAMES
        )
    elif magic == LEGACY_TEMPORAL_STATE_MAGIC:
        # AGT3 existed both as a one-step state and as a padded four-step state.
    ...
    for _ in range(frame_count):
        ...
        if length not in (
            ...
            V83_FEATURE_DIM,
            FEATURE_DIM,
        ) or offset + length > len(candidate):
    ...
    while len(durations) < ACTION_HISTORY_LENGTH:
        durations.insert(0, 0.0)
    frames = frames[-TEMPORAL_FRAMES:]
    actions = actions[-ACTION_HISTORY_LENGTH:]
    durations = durations[-ACTION_HISTORY_LENGTH:]
    if magic != TEMPORAL_STATE_MAGIC:
        valid_history_length = _legacy_temporal_valid_length(frames, actions)
    return frames, actions, durations, int(valid_history_length)


def decode_temporal_state(state: bytes) -> tuple[list[bytes], list[int], list[float]]:
    frames, actions, durations, _ = decode_temporal_state_with_length(state)
...
def _adaptive_hud_bands(
    frame: bytes,
    minimum_height: int = 7,
    maximum_bands: int = 6,
) -> tuple[tuple[int, int], ...]:
    """Locate text/HUD rows from local contrast instead of fixed coordinates."""
    ...
    for y in range(FEATURE_HEIGHT):
        ...
        contrast = (max(row) - min(row)) / 255.0
        horizontal_edges = sum(
            1 for left, right in zip(row, row[1:]) if abs(int(right) - int(left)) >= 32
        ) / max(1, FEATURE_WIDTH - 1)
        bright_dark = sum(abs(int(value) - mean) >= 48 for value in row) / FEATURE_WIDTH
        row_scores.append(0.48 * contrast + 0.34 * horizontal_edges + 0.18 * bright_dark)
    candidates = []
    minimum = max(5, min(FEATURE_HEIGHT, int(minimum_height)))
    for height in range(minimum, min(FEATURE_HEIGHT, minimum + 12) + 1, 2):
        for top in range(0, FEATURE_HEIGHT - height + 1):
            bottom = top + height
            score = sum(row_scores[top:bottom]) / height
            # HUDs are often near an edge, while outcome overlays are central;
            # keep both by applying only a very small positional prior.
            center = (top + bottom) * 0.5 / FEATURE_HEIGHT
            positional = 1.04 if center <= 0.30 or center >= 0.70 else 1.0
            candidates.append((score * positional, top, bottom))
    candidates.sort(reverse=True)
    selected: list[tuple[int, int]] = []
    for score, top, bottom in candidates:
        if score < 0.16:
            break
        if any(
            max(0, min(bottom, other_bottom) - max(top, other_top))
            / max(1, min(bottom - top, other_bottom - other_top))
            > 0.62
            for other_top, other_bottom in selected
        ):
            continue
        selected.append((top, bottom))
        if len(selected) >= max(1, int(maximum_bands)):
            break
...
def recognize_hud_score(frame: bytes) -> tuple[int | None, float]:
    ...
    bands = tuple(dict.fromkeys((
        *_adaptive_hud_bands(frame, 7, 6),
        (0, max(9, FEATURE_HEIGHT // 3)),
        (FEATURE_HEIGHT * 3 // 4, FEATURE_HEIGHT),
    )))
...
_STATUS_VICTORY_WORDS = (
    "WIN", "VICTORY", "CLEAR", "SUCCESS", "COMPLETE", "PASSED",
    "VICTOIRE", "GAGNE", "SIEG", "GEWONNEN", "VITTORIA", "VITORIA",
    "GANASTE", "GANADOR", "VENCEU", "SUCESSO", "SHOURI", "SEIKOU",
)
_STATUS_DEATH_WORDS = (
    "GAMEOVER", "DEFEAT", "DEAD", "FAILED", "TRYAGAIN",
    "DEFAITE", "PERDU", "NIEDERLAGE", "VERLOREN", "SCONFITTA",
    "DERROTA", "FALHOU", "MORREU", "SHIPPAI",
)
...
def smooth_reward(value: float, scale: float = 2.0) -> float:
    ...
    if not math.isfinite(numeric):
        return 0.0
    return math.tanh(numeric / max(1e-6, float(scale)))
...
def classify_transition_reward(
    ...
    confirmed_level_transition = 0.0
...
def load_human_progress_reference(path: Path, limit: int = 512) -> list[str]:
    ...
    try:
        rows = connection.execute(
            "SELECT episode_id,step,state,reward,terminated FROM transitions "
            "WHERE source='human' AND capture_failed=0 ORDER BY rowid DESC LIMIT ?",
            (max(64, int(limit) * 8),),
        ).fetchall()
...
def transition_outcome_labels(metrics: dict) -> tuple[int, float, float]:
    score_delta = float(metrics.get("score_delta", 0.0))
    if not math.isfinite(score_delta):
        score_delta = 0.0
    victory = max(0.0, min(1.0, float(metrics.get("confirmed_victory", 0.0))))
    death = max(0.0, min(1.0, float(metrics.get("confirmed_death", 0.0))))
    level = max(0.0, min(1.0, float(metrics.get("level_transition", 0.0))))
    score_confidence = max(0.0, min(1.0, float(metrics.get("score_delta_confidence", 0.0))))
    if victory > 0.0:
        return OUTCOME_VICTORY, victory, score_delta
    if death > 0.0:
        return OUTCOME_DEATH, death, score_delta
    if level > 0.0:
        return OUTCOME_LEVEL, level, score_delta
    if abs(score_delta) > 0.0 and score_confidence > 0.0:
        return OUTCOME_SCORE, score_confidence, score_delta
...
def insert_transitions(path: Path, rows: list[tuple]) -> None:
    ...
    for row in rows:
        ...
        if len(row) == 16:
            values = tuple(row)
            legacy_boundary = bool(values[7])
            explicit_terminal = bool(
                legacy_boundary
                and int(values[12]) in (OUTCOME_DEATH, OUTCOME_VICTORY, OUTCOME_LEVEL)
                and float(values[13]) >= 0.50
            )
            normalized_rows.append(
                values
                + (
                    int(explicit_terminal),
                    int(legacy_boundary and not explicit_terminal),
                    0,
                )
            )
            continue
        if len(row) == 15:
            values = tuple(row) + (str(row[0]),)
            legacy_boundary = bool(values[7])
            normalized_rows.append(
                values + (0, int(legacy_boundary), 0)
            )
            continue
        if len(row) == 12:
            values = tuple(row) + (OUTCOME_NONE, 0.0, 0.0, str(row[0]))
            normalized_rows.append(values + (0, int(bool(values[7])), 0))
            continue
        if len(row) != 9:
        ...
        normalized_rows.append(values + (0, int(bool(done)), 0))
    canonical_rows = []
    for row in normalized_rows:
        values = list(row)
        flags = tuple(int(bool(values[index])) for index in (16, 17, 18))
        if sum(flags) > 1:
            raise ValueError("轨迹边界类型互斥")
        values[16:19] = flags
        values[7] = int(any(flags))
        if flags[2]:
            values[5] = 0.0
            values[6] = None
            values[9:12] = [0.0, 0.0, 0.0]
            values[12:15] = [OUTCOME_NONE, 0.0, 0.0]
    ...
    try:
        connection.executemany(
            ...
            "outcome_type,outcome_confidence,score_delta,run_id,"
            "terminated,truncated,capture_failed"
            ") VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
            canonical_rows,
        )
        connection.commit()
...
def mark_episode_boundary(
    ...
    *,
    ...
    try:
        connection.execute(
            "UPDATE transitions SET done=1,terminated=?,truncated=?,capture_failed=? "
            "WHERE episode_id=? AND step=(SELECT MAX(step) FROM transitions WHERE episode_id=?)",
            (
                int(bool(terminated)),
                int(bool(truncated)),
                int(bool(capture_failed)),
                episode_id,
                episode_id,
            ),
        )
...
def load_transition_graph(
    ...
    try:
        cursor = connection.execute(
            "SELECT state,action,reward,next_state,terminated FROM transitions "
            "WHERE next_state IS NOT NULL AND capture_failed=0 "
...
def ensure_database(path: Path) -> bool:
    ...
    try:
        ...
        try:
            ...
            connection.execute(
                ...
                CREATE TABLE IF NOT EXISTS transitions(
                    ...
                    outcome_type INTEGER NOT NULL DEFAULT 0,
            ...
            reward_expected = legacy_expected + [
                "task_reward", "exploration_reward", "safety_penalty",
            ]
            if columns == reward_expected and database_version <= 14:
                connection.execute(
                    "ALTER TABLE transitions ADD COLUMN outcome_type INTEGER NOT NULL DEFAULT 0"
                )
                connection.execute(
                    "ALTER TABLE transitions ADD COLUMN outcome_confidence REAL NOT NULL DEFAULT 0"
                )
                connection.execute(
                    "ALTER TABLE transitions ADD COLUMN score_delta REAL NOT NULL DEFAULT 0"
                )
                columns = [
                    row[1]
                    for row in connection.execute("PRAGMA table_info(transitions)")
                ]
            expected = reward_expected + [
                "outcome_type", "outcome_confidence", "score_delta",
            ]
            if columns == expected and database_version <= 15:
                connection.execute(
                    "ALTER TABLE transitions ADD COLUMN run_id TEXT NOT NULL DEFAULT ''"
                )
                connection.execute(
                    "UPDATE transitions SET run_id=episode_id WHERE run_id=''"
                )
                columns = [
                    row[1]
                    for row in connection.execute("PRAGMA table_info(transitions)")
            ...
            if columns == expected and database_version <= 16:
                connection.execute(
                    "ALTER TABLE transitions ADD COLUMN terminated INTEGER NOT NULL DEFAULT 0"
                )
                connection.execute(
                    "ALTER TABLE transitions ADD COLUMN truncated INTEGER NOT NULL DEFAULT 0"
                )
                connection.execute(
                    "ALTER TABLE transitions ADD COLUMN capture_failed INTEGER NOT NULL DEFAULT 0"
                )
                connection.execute(
                    "UPDATE transitions SET terminated=CASE WHEN done=1 AND "
                    "outcome_type IN (1,2,4) AND outcome_confidence>=0.5 THEN 1 ELSE 0 END"
                )
                connection.execute(
                    "UPDATE transitions SET truncated=CASE WHEN done=1 AND terminated=0 "
                    "THEN 1 ELSE 0 END,capture_failed=0"
                )
                columns = [
                    row[1]
                    for row in connection.execute("PRAGMA table_info(transitions)")
                ]
            expected = expected + ["terminated", "truncated", "capture_failed"]
            if columns != expected:
            ...
            connection.execute("CREATE INDEX IF NOT EXISTS idx_transitions_run ON transitions(run_id, episode_id, step)")
            connection.execute(
            ...
            connection.execute(
                "UPDATE transitions SET terminated=CASE WHEN done=1 AND "
                "outcome_type IN (1,2,4) AND outcome_confidence>=0.5 THEN 1 ELSE terminated END "
                "WHERE done=1"
            )
            connection.execute(
                "UPDATE transitions SET truncated=CASE WHEN done=1 AND terminated=0 "
                "AND capture_failed=0 THEN 1 ELSE truncated END WHERE done=1"
            )
        ...
        try:
            connection.execute("PRAGMA journal_mode=WAL")
            connection.execute("PRAGMA synchronous=NORMAL")
            connection.execute("CREATE TABLE transitions(episode_id TEXT NOT NULL,step INTEGER NOT NULL,source TEXT NOT NULL,state BLOB NOT NULL,action INTEGER NOT NULL,reward REAL NOT NULL,next_state BLOB,done INTEGER NOT NULL,priority REAL NOT NULL,task_reward REAL NOT NULL DEFAULT 0,exploration_reward REAL NOT NULL DEFAULT 0,safety_penalty REAL NOT NULL DEFAULT 0,outcome_type INTEGER NOT NULL DEFAULT 0,outcome_confidence REAL NOT NULL DEFAULT 0,score_delta REAL NOT NULL DEFAULT 0,run_id TEXT NOT NULL DEFAULT '',terminated INTEGER NOT NULL DEFAULT 0,truncated INTEGER NOT NULL DEFAULT 0,capture_failed INTEGER NOT NULL DEFAULT 0,PRIMARY KEY(episode_id,step))")
            ...
            connection.execute("CREATE INDEX idx_transitions_run ON transitions(run_id, episode_id, step)")
...
def verify_experience_database(path: Path, action_count: int, scan_limit: int, stop_event: threading.Event | None) -> dict:
    ...
    try:
        ...
        for (episode_id, step, source, state, action, reward, next_state, done, priority,
             task_reward, exploration_reward, safety_penalty,
             outcome_type, outcome_confidence, score_delta, run_id,
             terminated, truncated, capture_failed) in cursor:
            if stop_event is not None and stop_event.is_set():
                raise RuntimeError("操作已取消")
            checked += 1
            try:
                ...
                if int(done) not in (0, 1) or not math.isfinite(float(priority)) or float(priority) <= 0:
                    raise ValueError
                boundary_flags = (
                    int(terminated), int(truncated), int(capture_failed)
                )
                if (
                    any(value not in (0, 1) for value in boundary_flags)
                    or sum(boundary_flags) > 1
                    or bool(int(done)) != bool(sum(boundary_flags))
                ):
                    raise ValueError
                if (
                    not math.isfinite(float(task_reward)) or not -4.0001 <= float(task_reward) <= 4.0001
                    ...
                    or int(outcome_type) not in OUTCOME_TYPES
                    or not math.isfinite(float(outcome_confidence))
                    or not 0.0 <= float(outcome_confidence) <= 1.0001
                    or not math.isfinite(float(score_delta))
                    or abs(float(score_delta)) > 1.0e12
                ):
                    raise ValueError
...
def upgrade_legacy_spatial_context(context: bytes) -> bytes:
    if len(context) != LEGACY_SPATIAL_CONTEXT_DIM:
        raise ValueError("旧空间分支尺寸错误")
    full_end = SPATIAL_FULL_PIXELS
    hud_end = full_end + SPATIAL_HUD_PIXELS
    full = bytes(context[:full_end])
    hud = bytes(context[full_end:hud_end])
    mouse = bytes(context[hud_end:])
    upgraded = full * 3 + hud * 3 + mouse * 3
    if len(upgraded) != SPATIAL_CONTEXT_DIM:
        raise RuntimeError("旧空间分支 RGB 迁移失败")
    return upgraded
...
def normalize_feature_bytes(feature: bytes) -> bytes:
    if len(feature) == FEATURE_DIM:
        return feature
    if len(feature) == V83_FEATURE_DIM:
        return bytes(feature[:BASE_FEATURE_DIM]) + upgrade_legacy_spatial_context(
            bytes(feature[BASE_FEATURE_DIM:])
        )
...
def count_samples(path: Path) -> tuple[int, int]:
    ...
    try:
        total=int(connection.execute("SELECT COUNT(*) FROM transitions WHERE capture_failed=0").fetchone()[0]);human=int(connection.execute("SELECT COUNT(*) FROM transitions WHERE source='human' AND capture_failed=0").fetchone()[0]);return total,human
    finally: connection.close()
...
def _spatial_branch_features(np, feature: bytes):
    ...
    full_start = 0
    hud_start = SPATIAL_CONTEXT_CHANNELS * SPATIAL_FULL_PIXELS
    mouse_start = hud_start + SPATIAL_CONTEXT_CHANNELS * SPATIAL_HUD_PIXELS
    tokens = []
    for channel in range(SPATIAL_CONTEXT_CHANNELS):
        start = full_start + channel * SPATIAL_FULL_PIXELS
        tokens.append(
            _adaptive_spatial_pool(
                np,
                context[start:start + SPATIAL_FULL_PIXELS],
                SPATIAL_FULL_HEIGHT,
                SPATIAL_FULL_WIDTH,
    ...
    for channel in range(SPATIAL_CONTEXT_CHANNELS):
        start = hud_start + channel * SPATIAL_HUD_PIXELS
        tokens.append(
            _adaptive_spatial_pool(
                np,
                context[start:start + SPATIAL_HUD_PIXELS],
                SPATIAL_HUD_HEIGHT,
                SPATIAL_FULL_WIDTH,
    ...
    for channel in range(SPATIAL_CONTEXT_CHANNELS):
        start = mouse_start + channel * SPATIAL_MOUSE_PIXELS
        tokens.append(
            _adaptive_spatial_pool(
                np,
                context[start:start + SPATIAL_MOUSE_PIXELS],
                SPATIAL_MOUSE_SIZE,
                SPATIAL_MOUSE_SIZE,
    ...
    result = np.concatenate(tokens).astype(np.float32, copy=False)
...
def _visual_pyramid_features_from_stages(np, stem, middle, final):
    """Pool spatial tokens after all convolutional stages ran in ONNX."""
    final_pool, _ = _pool_channel_features(
        np, final, CNN_POOL_ROWS, CNN_POOL_COLUMNS
    )
    middle_pool, _ = _pool_channel_features(
        np, middle, CNN_MID_POOL_ROWS, CNN_MID_POOL_COLUMNS
    )
    height, width = stem.shape[1:]
    top_rows = max(1, height // 5)
    bottom_rows = max(1, height // 6)
    ...
    return np.concatenate((final_pool, middle_pool, hud_pool)).astype(
...
def feature_vector(np, feature: bytes):
    frames, actions, durations, valid_length = decode_temporal_state_with_length(
        feature
    )
...
def initialize_model(np, input_dim: int, hidden_size: int, output_size: int) -> dict:
    ...
    model = {
        ...
        # A zero-initialized residual adapter is the only representation layer
        # that online play may change.  Zero keeps old/offline behavior exact.
        "online_adapter_w": np.zeros(hidden, dtype=np.float32),
        "online_adapter_b": np.zeros(hidden, dtype=np.float32),
        ...
        "world_stochastic_w": weight(
            (WORLD_MODEL_MEMBERS, WORLD_STOCHASTIC_SIZE, WORLD_LATENT_SIZE),
            0.012,
        ),
        # RSSM prior predicts a stochastic distribution from the deterministic
        # transition; the posterior additionally observes the next encoded
        # state.  Each head stores concatenated mean and log-standard-deviation.
        "world_prior_state_w": weight(
            (WORLD_MODEL_MEMBERS, WORLD_LATENT_SIZE, 2 * WORLD_STOCHASTIC_SIZE),
            0.025,
        ),
        "world_prior_action_w": weight(
            (WORLD_MODEL_MEMBERS, ACTION_EMBEDDING_SIZE, 2 * WORLD_STOCHASTIC_SIZE),
            0.018,
        ),
        "world_prior_duration_w": weight(
            (WORLD_MODEL_MEMBERS, DURATION_EMBEDDING_SIZE, 2 * WORLD_STOCHASTIC_SIZE),
            0.014,
        ),
        "world_prior_b": np.zeros(
            (WORLD_MODEL_MEMBERS, 2 * WORLD_STOCHASTIC_SIZE), dtype=np.float32
        ),
        "world_posterior_state_w": weight(
            (WORLD_MODEL_MEMBERS, WORLD_LATENT_SIZE, 2 * WORLD_STOCHASTIC_SIZE),
            0.020,
        ),
        "world_posterior_observation_w": weight(
            (WORLD_MODEL_MEMBERS, WORLD_LATENT_SIZE, 2 * WORLD_STOCHASTIC_SIZE),
            0.020,
        ),
        "world_posterior_b": np.zeros(
            (WORLD_MODEL_MEMBERS, 2 * WORLD_STOCHASTIC_SIZE), dtype=np.float32
        ),
        ...
        "world_continuation_w": weight(
            (WORLD_MODEL_MEMBERS, WORLD_LATENT_SIZE, 1), 0.02
        ),
        "world_continuation_b": np.full(
            (WORLD_MODEL_MEMBERS, 1), 1.5, dtype=np.float32
        ),
        ...
        "world_validation_reward_mae": -1.0,
...
def load_model(np, path: Path, input_dim: int, hidden_size: int, output_size: int, action_list: list[dict]) -> tuple[dict, bool]:
    ...
    try:
        ...
        with np.load(path, allow_pickle=False) as data:
            ...
            if archive_schema not in (15, 16, 17, 18, 19, 20, 21, 22, 23, MODEL_SCHEMA):
                raise ValueError("模型版本不兼容")
            required_members = (
                MODEL_ARCHIVE_REQUIRED_MEMBERS
                if archive_schema == MODEL_SCHEMA
                else MODEL_ARCHIVE_V20_REQUIRED_MEMBERS
                if archive_schema >= 20
            ...
            model["world_validation_reward_mae"] = float(data["world_validation_reward_mae"][0]) if "world_validation_reward_mae" in data else -1.0
            ...
            world_keys = {
                ...
                "world_dynamics_action_w", "world_dynamics_duration_w", "world_stochastic_w",
                "world_prior_state_w", "world_prior_action_w",
                "world_prior_duration_w", "world_prior_b",
                "world_posterior_state_w", "world_posterior_observation_w",
                "world_posterior_b",
                "world_dynamics_b",
            ...
            # Preserve every visual branch that existed in schema 22, leave the
            # newly inserted semantic rows at their safe random initialization,
            # and move the old context rows behind the expanded visual block.
            ...
            old_spatial_end = min(
                base_visual_encoder_output_dim() + SPATIAL_BRANCH_FEATURE_DIM,
                old_projection.shape[0],
            )
            rows_after_spatial = max(0, old_projection.shape[0] - old_spatial_end)
            if rows_after_spatial >= MODEL_GLOBAL_FEATURES:
                new_projection[
                    old_base_visual:old_spatial_end,
                    :column_overlap,
                ] = old_projection[
                    old_base_visual:old_spatial_end,
                    :column_overlap,
                ]
                context_count = min(MODEL_GLOBAL_FEATURES, rows_after_spatial)
                new_projection[
                    visual_encoder_output_dim():visual_encoder_output_dim() + context_count,
                    :column_overlap,
                ] = old_projection[
                    old_spatial_end:old_spatial_end + context_count,
                    :column_overlap,
                ]
            else:
            ...
            if archive_schema < MODEL_SCHEMA:
                # Newly introduced parameters retain safe initialization when
                # migrating older archives (the online adapter stays identity).
                changed = True
...
def load_global_backbone(np) -> dict | None:
    try:
        members = validate_npz_archive(GLOBAL_BACKBONE_PATH)
        if (
            not GLOBAL_BACKBONE_REQUIRED_MEMBERS.issubset(members)
            or not members.issubset(GLOBAL_BACKBONE_ALLOWED_MEMBERS)
        ):
            return None
        with np.load(GLOBAL_BACKBONE_PATH, allow_pickle=False) as data:
            if int(data["schema"][0]) != GLOBAL_BACKBONE_SCHEMA:
                return None
            result = {
                key: data[key].copy()
                for key in GLOBAL_BACKBONE_PARAMETER_KEYS
            }
            result.update(
                {
...
def save_global_backbone(np, model: dict) -> None:
    GLOBAL_BACKBONE_PATH.parent.mkdir(parents=True, exist_ok=True)
    temp = temporary_sibling_path(GLOBAL_BACKBONE_PATH)
    temp.unlink(missing_ok=True)
    values = {
        "schema": np.asarray([GLOBAL_BACKBONE_SCHEMA], dtype=np.int32),
    ...
    try:
        ...
        members = validate_npz_archive(temp)
        if (
            not GLOBAL_BACKBONE_REQUIRED_MEMBERS.issubset(members)
            or not members.issubset(GLOBAL_BACKBONE_ALLOWED_MEMBERS)
        ):
            raise RuntimeError("共享骨干归档无效")
        os.replace(temp, GLOBAL_BACKBONE_PATH)
    finally:
        temp.unlink(missing_ok=True)


def apply_global_backbone(np, model: dict, full_transfer: bool = False) -> bool:
    backbone = load_global_backbone(np)
    if backbone is None:
        return False
    del full_transfer
    # Until a game/goal condition is part of the model, only visual encoder
    # parameters may cross profile boundaries.
    keys = GLOBAL_BACKBONE_PARAMETER_KEYS
    changed = False
    for key in keys:
        source = backbone.get(key)
        destination = model.get(key)
        if (
            isinstance(source, np.ndarray)
            and isinstance(destination, np.ndarray)
            and source.shape == destination.shape
        ):
            model[key] = source.astype(destination.dtype, copy=True)
            changed = True
    if changed:
        _synchronize_quantized_conv_weights(np, model)
        _reset_optimizer_state(np, model)
...
def refresh_global_backbone(
    np,
    index: dict,
    config: dict,
    stop_event: threading.Event | None,
) -> bool:
    profile_ids = sorted(index.get("profiles", {}))
    if not profile_ids:
        return False
    settings = adaptive_global_training_settings(config, len(profile_ids))
    global_limit = min(8192, int(settings["sample_limit"]))
    shared_records: list[dict] = []
    shared_validation: list[dict] = []
    universal = universal_actions()
    for profile_id in profile_ids:
        raise_if_cancelled(stop_event)
        try:
            paths = profile_paths(profile_id)
            profile = migrate_profile(
                read_json_file(paths["profile"], MAX_PROFILE_JSON_BYTES), profile_id
            )
            if profile is None:
                continue
            dataset = load_training_data(
                ...
                paths["db"],
                len(profile["actions"]),
                int(settings["per_profile_limit"]),
                stop_event,
                int(config["n_step_horizon"]),
                float(config["validation_episode_fraction"]),
                config,
            )
            for record in list((dataset or {}).get("train", [])):
                if 0 <= int(record.get("action", -1)) < len(universal):
                    shared_records.append(record)
            for record in list((dataset or {}).get("validation", [])):
                if 0 <= int(record.get("action", -1)) < len(universal):
                    shared_validation.append(record)
    ...
    if not shared_records:
        return False
    rng = random.Random(0xBACC80)
    rng.shuffle(shared_records)
    rng.shuffle(shared_validation)
    shared_records = shared_records[:global_limit]
    shared_validation = shared_validation[:min(1024, max(64, global_limit // 4))]
    backbone = initialize_model(
        np, MODEL_INPUT_DIM, configured_teacher_hidden_size(config), len(universal)
    )
    backbone["action_factors"] = action_factor_matrix(np, universal)
    backbone["action_key_multihot"] = action_key_multihot_matrix(np, universal)
    backbone["action_button_multihot"] = action_button_multihot_matrix(np, universal)
    backbone["action_hash"] = actions_hash(universal)
    backbone["action_signatures"] = [action_signature(action) for action in universal]
    apply_global_backbone(np, backbone, full_transfer=True)
    backbone["_semantic_cache_limit"] = training_vector_cache_limit(
        len(shared_records)
    )
    pretrain_visual_encoder(
        np,
        backbone,
        shared_records,
        float(config["learning_rate"]),
        stop_event,
        maximum_steps=min(384, max(64, len(shared_records) // 64)),
    )
    save_global_backbone(np, backbone)
    return True
...
def verify_model_archive(np, path: Path, model: dict, signatures: list[str]) -> None:
    ...
    with np.load(path, allow_pickle=False) as data:
        ...
        required_arrays = (
            ...
            "world_encoder_w", "world_encoder_b", "world_dynamics_w",
            "world_dynamics_action_w", "world_dynamics_duration_w", "world_stochastic_w",
            "world_prior_state_w", "world_prior_action_w", "world_prior_duration_w",
            "world_prior_b", "world_posterior_state_w",
            "world_posterior_observation_w", "world_posterior_b",
            "world_dynamics_b",
            "world_reward_w", "world_reward_action_w", "world_reward_duration_w",
            "world_reward_b", "world_done_w", "world_done_b",
            "world_continuation_w", "world_continuation_b",
        ...
        for key in (
            ...
            if values.shape != (1,) or not np.isfinite(values).all() or float(values[0]) < -1.0:
                raise RuntimeError(f"世界模型验证指标无效:{key}")
...
def model_from_global_prior(np, prior: dict | None, action_list: list[dict], hidden_size: int) -> dict:
    ...
    apply_global_backbone(np,model,full_transfer=False)
...
def refresh_global_prior(np, index: dict, config: dict, stop_event: threading.Event | None) -> bool:
    ...
    for profile_id in sorted(index.get("profiles",{})):
        ...
        try:
            ...
            model,_=load_model(np,paths["model"],MODEL_INPUT_DIM,configured_teacher_hidden_size(config),len(profile["actions"]),profile["actions"])
    ...
    seed=initialize_model(np,MODEL_INPUT_DIM,configured_teacher_hidden_size(config),1)
...
def distill_teacher_model(
    ...
    # Only tensors whose complete shapes agree are transferable.  Cropping a
    # hidden axis assumes neurons are ordered by importance, which they are not.
    for key, destination in list(student.items()):
        ...
        if (
            ...
            and source.shape == destination.shape
            and not key.startswith("optimizer_")
        ):
            student[key] = source.astype(destination.dtype, copy=True)
    ...
    for key in (
        ...
        "world_validation_reward_mae", "world_validation_done_brier",
        "world_validation_latent_error", "world_validation_multistep_error",
    ):
        student[key] = teacher.get(key, student.get(key, 0))
    student["runtime_tier"] = str(runtime_tier)

    calibration_records = list(records or [])[:DISTILLATION_RECORD_LIMIT]
    if calibration_records and int(hidden_size) != int(teacher["hidden_size"]):
        factor_matrix = student["action_factors"].astype(np.int64, copy=False)
        key_matrix = student["action_key_multihot"].astype(np.float32, copy=False)
        button_matrix = student["action_button_multihot"].astype(np.float32, copy=False)
        learning_rate = 0.0010
        batch_limit = 8 if str(runtime_tier) == "low_numpy" else 16

        def add_policy_gradient(hidden, action_gradient, gradients):
            action_gradient = np.asarray(action_gradient, dtype=np.float32)
            memorized_gradient = 0.30 * action_gradient
            factor_action_gradient = (0.70 / 4.0) * action_gradient
            hidden_gradient = student["policy_action_w"] @ memorized_gradient
            gradients["policy_action_w"] += np.outer(hidden, memorized_gradient)
            gradients["policy_action_b"] += memorized_gradient
            for weight_key, bias_key, column in (
                ("policy_control_w", "policy_control_b", 0),
                ("policy_mouse_w", "policy_mouse_b", 1),
            ):
                bucket = _factor_bucket_gradient(
                    np, factor_matrix, column, factor_action_gradient, student[bias_key].shape[0]
                )
                gradients[weight_key] += np.outer(hidden, bucket)
                gradients[bias_key] += bucket
                hidden_gradient += student[weight_key] @ bucket
            key_gradient = factor_action_gradient @ key_matrix
            button_gradient = factor_action_gradient @ button_matrix
            gradients["policy_key_w"] += np.outer(hidden, key_gradient)
            gradients["policy_key_b"] += key_gradient
            gradients["policy_button_w"] += np.outer(hidden, button_gradient)
            gradients["policy_button_b"] += button_gradient
            hidden_gradient += student["policy_key_w"] @ key_gradient
            hidden_gradient += student["policy_button_w"] @ button_gradient
            return hidden_gradient.astype(np.float32, copy=False)

        def add_critic_gradient(hidden, teacher_twins, student_twins, gradients):
            hidden_gradient = np.zeros_like(hidden, dtype=np.float32)
            normalization = 0.70 / max(1, teacher_twins.size)
            differences = np.clip(student_twins - teacher_twins, -4.0, 4.0)
            for twin in range(Q_TWIN_COUNT):
                for head in range(VALUE_HEAD_COUNT):
                    action_gradient = differences[twin, head].astype(np.float32) * normalization
                    memorized_gradient = 0.30 * action_gradient
                    factor_action_gradient = 0.70 * action_gradient
                    gradients["q_action_w"][twin, head] += np.outer(hidden, memorized_gradient)
                    gradients["q_action_b"][twin, head] += memorized_gradient
                    hidden_gradient += student["q_action_w"][twin, head] @ memorized_gradient
                    for weight_key, bias_key, column in (
                        ...
                        bucket = _factor_bucket_gradient(
                            ...
                            factor_matrix,
                            column,
                            factor_action_gradient / 4.0,
                            student[bias_key].shape[-1],
                        )
                        gradients[weight_key][twin, head] += np.outer(hidden, bucket)
                        gradients[bias_key][twin, head] += bucket
                        hidden_gradient += student[weight_key][twin, head] @ bucket
                    key_gradient = (factor_action_gradient / 4.0) @ key_matrix
                    button_gradient = (factor_action_gradient / 4.0) @ button_matrix
                    gradients["q_key_w"][twin, head] += np.outer(hidden, key_gradient)
                    gradients["q_key_b"][twin, head] += key_gradient
                    gradients["q_button_w"][twin, head] += np.outer(hidden, button_gradient)
                    gradients["q_button_b"][twin, head] += button_gradient
                    hidden_gradient += student["q_key_w"][twin, head] @ key_gradient
                    hidden_gradient += student["q_button_w"][twin, head] @ button_gradient
            loss = 0.70 * float(np.mean((student_twins - teacher_twins) ** 2))
            return loss, hidden_gradient.astype(np.float32, copy=False)

        def reward_member_outputs(model, hidden, next_hidden):
            following = hidden if next_hidden is None else next_hidden
            transition = np.tanh(following - 0.35 * hidden).astype(np.float32)
            logits = np.einsum(
                "h,mho->mo", transition, model["reward_model_w"], optimize=True
            ) + model["reward_model_b"]
            outputs = np.empty_like(logits, dtype=np.float32)
            outputs[:, 0] = np.tanh(logits[:, 0])
            outputs[:, 1:] = _sigmoid(np, logits[:, 1:])
            return transition, outputs

        def add_reward_model_gradient(
            hidden, next_hidden, teacher_outputs, gradients
        ):
            transition, student_outputs = reward_member_outputs(
                student, hidden, next_hidden
            )
            output_gradient = (student_outputs - teacher_outputs).astype(np.float32)
            output_gradient[:, 0] *= 1.0 - student_outputs[:, 0] ** 2
            output_gradient[:, 1:] *= student_outputs[:, 1:] * (
                1.0 - student_outputs[:, 1:]
            )
            output_gradient *= 0.20 / max(1, output_gradient.size)
            current_gradient = np.zeros_like(hidden, dtype=np.float32)
            next_gradient = np.zeros_like(hidden, dtype=np.float32)
            transition_gradient = np.zeros_like(hidden, dtype=np.float32)
            for member in range(REWARD_MODEL_MEMBERS):
                gradients["reward_model_w"][member] += np.outer(
                    transition, output_gradient[member]
                )
                gradients["reward_model_b"][member] += output_gradient[member]
                transition_gradient += student["reward_model_w"][member] @ output_gradient[member]
            transition_gradient *= 1.0 - transition * transition
            current_gradient -= 0.35 * transition_gradient
            if next_hidden is not None:
                next_gradient += transition_gradient
            else:
                current_gradient += transition_gradient
            loss = 0.20 * float(np.mean((student_outputs - teacher_outputs) ** 2))
            return loss, current_gradient, next_gradient

        def world_member_targets(model, hidden, action_id, duration_index):
            latent = world_model_latent(np, model, hidden)
            action_embedding = model["action_embedding"][action_id]
            duration_embedding = model["duration_embedding"][duration_index]
            deterministic = np.tanh(
                np.einsum("ml,mlk->mk", latent, model["world_dynamics_w"])
            ...
            prior_mean, _, _ = rssm_prior(
                np, model, deterministic, action_id, duration_index
            )
            next_latent = rssm_latent_from_state(
                np, model, deterministic, prior_mean
            )
            rewards = (
                np.einsum("ml,mlv->mv", next_latent, model["world_reward_w"])
            ...
            continuation = _sigmoid(
                np,
                np.einsum(
                    "ml,mlq->mq", next_latent,
                    model["world_continuation_w"],
                )
                + model["world_continuation_b"],
            )[:, 0].astype(np.float32)
            return latent, next_latent, rewards, 1.0 - continuation

        def add_world_gradient(
            hidden, action_id, duration_index, teacher_targets, gradients
        ):
            target_latent, target_next, target_rewards, target_done = teacher_targets
            student_latent, student_next, student_rewards, student_done = world_member_targets(
                student, hidden, action_id, duration_index
            )
            action_embedding = student["action_embedding"][action_id]
            duration_embedding = student["duration_embedding"][duration_index]
            hidden_gradient = np.zeros_like(hidden, dtype=np.float32)
            action_gradient_total = np.zeros_like(action_embedding)
            duration_gradient_total = np.zeros_like(duration_embedding)
            for member in range(WORLD_MODEL_MEMBERS):
                reward_gradient = np.clip(
                    student_rewards[member] - target_rewards[member], -4.0, 4.0
                ) * (0.20 / (WORLD_MODEL_MEMBERS * VALUE_HEAD_COUNT))
                gradients["world_reward_w"][member] += np.outer(
                    student_next[member], reward_gradient
                )
                gradients["world_reward_action_w"][member] += np.outer(
                    action_embedding, reward_gradient
                )
                gradients["world_reward_duration_w"][member] += np.outer(
                    duration_embedding, reward_gradient
                )
                gradients["world_reward_b"][member] += reward_gradient
                next_gradient = student["world_reward_w"][member] @ reward_gradient
                action_gradient = student["world_reward_action_w"][member] @ reward_gradient
                duration_gradient = student["world_reward_duration_w"][member] @ reward_gradient
                continuation_gradient = (
                    target_done[member] - student_done[member]
                ) * (0.10 / WORLD_MODEL_MEMBERS)
                gradients["world_continuation_w"][member, :, 0] += (
                    student_next[member] * continuation_gradient
                )
                gradients["world_continuation_b"][member, 0] += continuation_gradient
                next_gradient += (
                    student["world_continuation_w"][member, :, 0]
                    * continuation_gradient
                )
                next_gradient += (
                    student_next[member] - target_next[member]
                ) * (0.10 / (WORLD_MODEL_MEMBERS * WORLD_LATENT_SIZE))
                dynamics_gradient = next_gradient * (
                    1.0 - student_next[member] * student_next[member]
                )
                gradients["world_dynamics_w"][member] += np.outer(
                    student_latent[member], dynamics_gradient
                )
                gradients["world_dynamics_action_w"][member] += np.outer(
                    action_embedding, dynamics_gradient
                )
                gradients["world_dynamics_duration_w"][member] += np.outer(
                    duration_embedding, dynamics_gradient
                )
                gradients["world_dynamics_b"][member] += dynamics_gradient
                latent_gradient = student["world_dynamics_w"][member] @ dynamics_gradient
                latent_gradient += (
                    student_latent[member] - target_latent[member]
                ) * (0.05 / (WORLD_MODEL_MEMBERS * WORLD_LATENT_SIZE))
                encoder_gradient = latent_gradient * (
                    1.0 - student_latent[member] * student_latent[member]
                )
                gradients["world_encoder_w"][member] += np.outer(
                    hidden, encoder_gradient
                )
                gradients["world_encoder_b"][member] += encoder_gradient
                hidden_gradient += student["world_encoder_w"][member] @ encoder_gradient
                action_gradient += student["world_dynamics_action_w"][member] @ dynamics_gradient
                duration_gradient += student["world_dynamics_duration_w"][member] @ dynamics_gradient
                action_gradient_total += action_gradient
                duration_gradient_total += duration_gradient
            gradients["action_embedding"][action_id] += action_gradient_total
            gradients["duration_embedding"][duration_index] += duration_gradient_total
            loss = (
                0.20 * float(np.mean((student_rewards - target_rewards) ** 2))
                + 0.10 * float(np.mean((student_done - target_done) ** 2))
                + 0.10 * float(np.mean((student_next - target_next) ** 2))
            )
            return loss, hidden_gradient.astype(np.float32, copy=False)
        ...
        for record in calibration_records:
            ...
            try:
                ...
                teacher_next = (
                    _latest_temporal_hidden(np, teacher, record["next_state"])
                    if record.get("next_state") is not None and not bool(record.get("done"))
                    else None
                )
                teacher_cache.append((record, teacher_hidden, teacher_next))
            except Exception:
                continue

        gradients = _training_gradient_buffer(np, student)
        pending = 0
        total_schedule = max(64, len(teacher_cache) * DISTILLATION_EPOCHS // batch_limit)

        def apply_distillation_gradients():
            nonlocal gradients, pending
            if pending <= 0:
                return
            ...
            if active:
                ...
                _adam_apply(
                    np, student, gradients, learning_rate,
                    warmup_steps=64, cosine_steps=total_schedule, weight_decay=0.00005,
                )
                _synchronize_quantized_conv_weights(np, student)
            gradients = _training_gradient_buffer(np, student)
            pending = 0

        indices = list(range(len(teacher_cache)))
        for epoch in range(DISTILLATION_EPOCHS):
            random.Random(0xD157111 + epoch).shuffle(indices)
            for cache_index in indices:
                raise_if_cancelled(stop_event)
                record, teacher_hidden, teacher_next = teacher_cache[cache_index]
                try:
                    student_hidden, student_caches = _latest_temporal_hidden(
                        np, student, record["state"], return_cache=True
                    )
                    student_next = None
                    student_next_caches = None
                    if teacher_next is not None:
                        student_next, student_next_caches = _latest_temporal_hidden(
                            np, student, record["next_state"], return_cache=True
                        )
                    teacher_probability, _, teacher_head_hidden = factorized_action_outputs_from_hidden(
                        np, teacher, teacher_hidden
                    )
                    student_probability, _, student_head_hidden = factorized_action_outputs_from_hidden(
                        np, student, student_hidden
                    )
                    teacher_probability = np.clip(teacher_probability, 1e-7, 1.0)
                    student_probability = np.clip(student_probability, 1e-7, 1.0)
                    student_probability /= float(student_probability.sum())
                    teacher_probability /= float(teacher_probability.sum())
                    policy_kl_loss = float(
                        np.sum(
                            teacher_probability
                            * (
                                np.log(teacher_probability)
                                - np.log(student_probability)
                            )
                        )
                    )
                    if not math.isfinite(policy_kl_loss):
                        continue
                    hidden_gradient = add_policy_gradient(
                        student_head_hidden,
                        student_probability - teacher_probability,
                        gradients,
                    )
                    teacher_twins = factorized_twin_q_outputs_from_hidden(
                        np, teacher, teacher_head_hidden, hidden_is_adapted=True
                    )
                    student_twins = factorized_twin_q_outputs_from_hidden(
                        np, student, student_head_hidden, hidden_is_adapted=True
                    )
                    _, critic_hidden_gradient = add_critic_gradient(
                        student_head_hidden, teacher_twins, student_twins, gradients
                    )
                    hidden_gradient += critic_hidden_gradient

                    action_id = max(0, min(len(action_list) - 1, int(record.get("action", 0))))
                    kind = int(factor_matrix[action_id, 0])
                    teacher_duration, _ = conditional_duration_outputs(
                        np, teacher, teacher_head_hidden, kind
                    )
                    student_duration, _ = conditional_duration_outputs(
                        np, student, student_head_hidden, kind
                    )
                    duration_gradient = (
                        student_duration - teacher_duration
                    ).astype(np.float32) * 0.10
                    gradients["policy_duration_w"] += np.outer(
                        student_head_hidden, duration_gradient
                    )
                    gradients["policy_duration_b"] += duration_gradient
                    gradients["policy_duration_kind_b"][kind] += duration_gradient
                    hidden_gradient += student["policy_duration_w"] @ duration_gradient
                    teacher_duration_twins = (
                        np.einsum(
                            "h,tvhd->tvd", teacher_head_hidden,
                            teacher["q_duration_w"], optimize=True,
                        )
                        + teacher["q_duration_b"]
                        + teacher["q_duration_kind_b"][:, :, kind, :]
                    )
                    student_duration_twins = (
                        np.einsum(
                            "h,tvhd->tvd", student_head_hidden,
                            student["q_duration_w"], optimize=True,
                        )
                        + student["q_duration_b"]
                        + student["q_duration_kind_b"][:, :, kind, :]
                    )
                    q_duration_gradient = np.clip(
                        student_duration_twins - teacher_duration_twins,
                        -4.0,
                        4.0,
                    ).astype(np.float32) * (
                        0.10 / max(1, student_duration_twins.size)
                    )
                    gradients["q_duration_w"] += np.einsum(
                        "h,tvd->tvhd", student_head_hidden,
                        q_duration_gradient, optimize=True,
                    )
                    gradients["q_duration_b"] += q_duration_gradient
                    gradients["q_duration_kind_b"][:, :, kind, :] += q_duration_gradient
                    hidden_gradient += np.einsum(
                        "tvhd,tvd->h", student["q_duration_w"],
                        q_duration_gradient, optimize=True,
                    )

                    teacher_value = float(
                        teacher_head_hidden @ teacher["value_w"][:, 0] + teacher["value_b"][0]
                    )
                    student_value = float(
                        student_head_hidden @ student["value_w"][:, 0] + student["value_b"][0]
                    )
                    value_gradient = 0.30 * max(-4.0, min(4.0, student_value - teacher_value))
                    gradients["value_w"][:, 0] += student_head_hidden * value_gradient
                    gradients["value_b"][0] += value_gradient
                    hidden_gradient += student["value_w"][:, 0] * value_gradient

                    teacher_progress = predict_progress(np, teacher, teacher_hidden)
                    student_progress = predict_progress(np, student, student_hidden)
                    progress_gradient = 0.25 * max(
                        -4.0, min(4.0, student_progress - teacher_progress)
                    )
                    gradients["progress_w"][:, 0] += student_hidden * progress_gradient
                    gradients["progress_b"][0] += progress_gradient
                    hidden_gradient += student["progress_w"][:, 0] * progress_gradient

                    teacher_safety = predict_safety_probability(np, teacher, teacher_hidden)
                    student_safety = predict_safety_probability(np, student, student_hidden)
                    safety_gradient = (
                        0.25
                        * (student_safety - teacher_safety)
                        * student_safety
                        * (1.0 - student_safety)
                    )
                    gradients["safety_w"][:, 0] += student_hidden * safety_gradient
                    gradients["safety_b"][0] += safety_gradient
                    hidden_gradient += student["safety_w"][:, 0] * safety_gradient

                    _, teacher_reward_outputs = reward_member_outputs(
                        teacher, teacher_hidden, teacher_next
                    )
                    _, reward_current_gradient, reward_next_gradient = add_reward_model_gradient(
                        student_hidden, student_next, teacher_reward_outputs, gradients
                    )
                    hidden_gradient += reward_current_gradient

                    duration_index = _record_duration_index(record)
                    teacher_world_targets = world_member_targets(
                        teacher, teacher_hidden, action_id, duration_index
                    )
                    _, world_hidden_gradient = add_world_gradient(
                        student_hidden, action_id, duration_index,
                        teacher_world_targets, gradients,
                    )
                    hidden_gradient += world_hidden_gradient

                    teacher_skill_logits = (
                        teacher_head_hidden @ teacher["policy_skill_w"]
                        + teacher["policy_skill_b"]
                    )
                    student_skill_logits = (
                        student_head_hidden @ student["policy_skill_w"]
                        + student["policy_skill_b"]
                    )
                    teacher_skill_probability = _softmax_vector(np, teacher_skill_logits)
                    student_skill_probability = _softmax_vector(np, student_skill_logits)
                    skill_gradient = (
                        student_skill_probability - teacher_skill_probability
                    ).astype(np.float32) * 0.10
                    gradients["policy_skill_w"] += np.outer(
                        student_head_hidden, skill_gradient
                    )
                    gradients["policy_skill_b"] += skill_gradient
                    hidden_gradient += student["policy_skill_w"] @ skill_gradient
                    teacher_skill_values = (
                        teacher_head_hidden @ teacher["skill_value_w"]
                        + teacher["skill_value_b"]
                    )
                    student_skill_values = (
                        student_head_hidden @ student["skill_value_w"]
                        + student["skill_value_b"]
                    )
                    skill_value_gradient = np.clip(
                        student_skill_values - teacher_skill_values, -4.0, 4.0
                    ).astype(np.float32) * (0.10 / SKILL_HEAD_SIZE)
                    gradients["skill_value_w"] += np.outer(
                        student_head_hidden, skill_value_gradient
                    )
                    gradients["skill_value_b"] += skill_value_gradient
                    hidden_gradient += student["skill_value_w"] @ skill_value_gradient

                    _backprop_temporal_encoder(
                        np, student, student_caches, hidden_gradient,
                        learning_rate * 0.35, gradients,
                    )
                    if student_next_caches is not None:
                        _backprop_temporal_encoder(
                            np, student, student_next_caches, reward_next_gradient,
                            learning_rate * 0.25, gradients,
                        )
                    pending += 1
                    if pending >= batch_limit:
                        apply_distillation_gradients()
                except (ValueError, OverflowError, FloatingPointError):
                    continue
        apply_distillation_gradients()
        quality_records = calibration_records[-min(512, len(calibration_records)):]
        if quality_records:
            try:
                quality_metrics = evaluate_model_records(np, student, quality_records)
                student["validation_score"] = float(quality_metrics.get("score", -0.20))
                student["world_validation_reward_mae"] = float(
                    quality_metrics.get("world_reward_mae", -1.0)
                )
                student["world_validation_done_brier"] = float(
                    quality_metrics.get("world_done_brier", -1.0)
                )
                student["world_validation_latent_error"] = float(
                    quality_metrics.get("world_latent_error", -1.0)
                )
                student["world_validation_multistep_error"] = float(
                    quality_metrics.get("world_multistep_error", -1.0)
                )
            except (KeyError, TypeError, ValueError, OverflowError):
                student["world_validation_reward_mae"] = -1.0
                student["world_validation_done_brier"] = -1.0
                student["world_validation_latent_error"] = -1.0
                student["world_validation_multistep_error"] = -1.0
    _synchronize_quantized_conv_weights(np, student)
...
def save_runtime_students(
    ...
    for name, hidden_size, runtime_tier in specifications:
        ...
        candidate_path = paths[f"student_{name}_candidate"]
        candidate_target_path = paths[f"student_{name}_candidate_target"]
        active_path = paths[f"student_{name}"]
        active_target_path = paths[f"student_{name}_target"]
        # Offline metrics remain diagnostics and can never promote a model.
        # Existing champions are replaced only by the cross-session live gate
        # in promote_live_challenger().
        if active_path.is_file() and active_target_path.is_file():
            save_model(np, candidate_path, student)
            save_model(np, candidate_target_path, student_target)
        else:
            save_model(np, active_path, student)
            save_model(np, active_target_path, student_target)
...
def save_model(np, path: Path, model: dict) -> None:
    ...
    try:
        values = {
            ...
            "world_validation_reward_mae": np.asarray([model.get("world_validation_reward_mae", -1.0)], dtype=np.float32),
...
def _temporal_combined_feature(
    np,
    model: dict,
    frame,
    action_value: int,
    ...
    button_density = 0.0
    mouse_value = 0.0
    if 0 <= int(action_value) < len(model["action_factors"]):
        action_index = int(action_value)
        has_action = 1.0
        action_embedding = model["action_embedding"][action_index]
        factors = model["action_factors"][action_index]
        kind_index = int(factors[0])
        if 0 <= kind_index < len(kind_values):
            kind_values[kind_index] = 1.0
        mouse_value = int(factors[1]) / max(1, MOUSE_HEAD_SIZE - 1)
        key_density = min(
            1.0,
            float(np.count_nonzero(model["action_key_multihot"][action_index]))
            / 6.0,
        )
        button_density = min(
            1.0,
            float(np.count_nonzero(model["action_button_multihot"][action_index]))
            / 3.0,
        )
    duration_index = duration_bin(float(duration_value))
    context = np.concatenate(
        (
            action_embedding,
            model["duration_embedding"][duration_index],
            np.asarray(
                [
                    has_action,
                    *kind_values,
                    ...
                    button_density,
                    mouse_value,
                    max(0.0, min(1.0, float(duration_value) / 0.5)),
                ],
                dtype=np.float32,
            ),
        )
    ...
    combined = np.concatenate((visual, context)).astype(np.float32, copy=False)
    if combined.shape != (int(model["frame_proj"].shape[0]),):
        raise RuntimeError("时序模型输入尺寸无效")
    return combined
...
def _model_hidden_state_from_vector(
    ...
    if (
        not return_cache
        and str(model.get("runtime_tier", "low_numpy")) != "low_numpy"
        and 1 <= len(vector["frames"]) <= TEMPORAL_FRAMES
    ):
        try:
            combined_steps = [
                _temporal_combined_feature(
                    np,
                    model,
                    frame,
                    int(actions[index]),
            ...
            accelerated_hidden = accelerated_temporal_policy(
                np, model, combined_steps, hidden
            )
            if accelerated_hidden is not None:
                return accelerated_hidden
        except Exception:
            pass
    for index, frame in enumerate(vector["frames"]):
        if return_cache:
...
def _model_hidden_state(np, model: dict, feature: bytes):
    return _latest_temporal_hidden(np, model, feature)
...
def pretrain_visual_encoder(
    ...
    """Reliable visual contrastive and action-conditioned pretraining."""
    parts = [part for part in _continuous_trajectory_parts(records) if len(part) >= 3]
    if not parts:
        return {"steps": 0.0, "loss": 0.0, "action_prediction_steps": 0.0}

    def last_frame(record: dict) -> bytes:
        frames, _, _ = decode_temporal_state(record["state"])
        return frames[-1]

    def frame_luminance(record: dict):
        frame = normalize_feature_bytes(last_frame(record))
        pixels = FEATURE_WIDTH * FEATURE_HEIGHT
        return np.frombuffer(frame[:pixels], dtype=np.uint8).astype(np.float32)

    def reliable_adjacent(anchor: dict, positive: dict) -> bool:
        if bool(anchor.get("done")) or anchor.get("next_state") is None:
            return False
        if int(anchor.get("outcome_type", OUTCOME_NONE)) in (
            OUTCOME_DEATH,
            OUTCOME_VICTORY,
            OUTCOME_LEVEL,
        ):
            return False
        try:
            anchor_frame = last_frame(anchor)
            positive_frame = last_frame(positive)
            left = frame_luminance(anchor)
            right = frame_luminance(positive)
            left_mean = float(left.mean())
            right_mean = float(right.mean())
            if left_mean < 8.0 or right_mean < 8.0:
                return False
            pixels = FEATURE_WIDTH * FEATURE_HEIGHT
            motion, changed, flicker = visual_change_metrics(
                anchor_frame[:pixels],
                positive_frame[:pixels],
            )
            fade = max(0.0, min(1.0, (left_mean - right_mean) / 255.0))
            return (
                flicker <= 0.12
                and fade <= 0.18
                and changed <= 0.42
    ...
    def record_hash(record: dict) -> str:
        ...
        if value is None:
            value = _temporal_frame_hash(record["state"])
            hashes[key] = value
        return value

    examples: list[tuple[dict, dict | None, dict, int]] = []
    action_pairs: list[tuple[dict, dict]] = []
    for part_index, part in enumerate(parts):
        for position, anchor in enumerate(part):
            negative = None
            offsets = list(range(8, 33))
            for offset in offsets:
                candidate_position = position + offset
                if candidate_position >= len(part):
                    break
                candidate = part[candidate_position]
                if _perceptual_hash_similarity(record_hash(anchor), record_hash(candidate)) <= 0.70:
                    negative = candidate
                    break
            if negative is None:
                for other_index in range(1, len(parts)):
                    other = parts[(part_index + other_index) % len(parts)]
                    candidate = other[(position * 7 + other_index) % len(other)]
                    if _perceptual_hash_similarity(record_hash(anchor), record_hash(candidate)) <= 0.65:
                        negative = candidate
                        break
            if negative is None:
                continue
            # The strongest positive is a controlled augmentation of the same frame.
            examples.append((anchor, None, negative, position + part_index * 131))
            if position + 1 < len(part) and reliable_adjacent(anchor, part[position + 1]):
                examples.append((anchor, part[position + 1], negative, -1))
                action_pairs.append((anchor, part[position + 1]))

    if not examples:
        return {"steps": 0.0, "loss": 0.0, "action_prediction_steps": 0.0}
    rng = np.random.default_rng(
        VISUAL_INITIALIZATION_SEED + int(model.get("visual_pretraining_steps", 0))
    )
    ...
    def encoded_augmentation(record: dict, seed: int):
        ...
        shift_x = int(local.integers(-3, 4))
        base = np.roll(base, (shift_y, shift_x), axis=(1, 2))
        brightness = float(local.uniform(0.92, 1.08))
        base[:3] = np.clip(base[:3] * brightness, -1.0, 1.0)
        if float(local.random()) < 0.75:
            height = int(local.integers(3, max(4, FEATURE_HEIGHT // 6)))
            width = int(local.integers(4, max(5, FEATURE_WIDTH // 8)))
            top = int(local.integers(0, max(1, FEATURE_HEIGHT - height + 1)))
    ...
    def apply_contrastive_batch() -> None:
    ...
    for anchor_record, positive_record, negative_record, augmentation_seed in examples[:limit]:
        raise_if_cancelled(stop_event)
        anchor, anchor_cache = encoded_record(anchor_record)
        if positive_record is None:
            positive, positive_cache = encoded_augmentation(
                anchor_record, augmentation_seed
            )
        else:
            positive, positive_cache = encoded_record(positive_record)
        negative, negative_cache = encoded_record(negative_record)
    ...
    # Action-conditioned prediction: current hidden state and executed action
    # predict the next observed representation. Reward targets remain external.
    action_steps = 0
    action_loss = 0.0
    action_limit = min(max(1, limit // 4), len(action_pairs), 48)
    frozen_target = target_model if target_model is not None else clone_target_model(np, model)
    for anchor_record, positive_record in action_pairs[:action_limit]:
        raise_if_cancelled(stop_event)
        ...
        next_hidden = _latest_temporal_hidden(
            np, model, anchor_record["next_state"]
        )
        target_next_hidden = _latest_temporal_hidden(
            np, frozen_target, anchor_record["next_state"]
        )
        gradients = _training_gradient_buffer(np, model)
        transition_loss, current_hidden_gradient = train_world_model_transition(
            np,
            model,
            current_hidden,
            next_hidden,
            ...
            (
                smooth_reward(float(anchor_record.get("task_reward", 0.0))),
                max(
                    0.0,
                    min(1.0, float(anchor_record.get("exploration_reward", 0.0))),
                ),
                max(
                    0.0,
                    min(2.0, float(anchor_record.get("safety_penalty", 0.0))),
                ),
            ),
            bool(anchor_record.get("done", False)),
            0.25,
            gradients,
            target_model=frozen_target,
        ...
        _backprop_temporal_encoder(
            ...
            model,
            current_caches,
            current_hidden_gradient,
            step_size,
            gradients,
        )
        active = [gradient for gradient in gradients.values() if np.any(gradient)]
        if active:
            clip_gradients_by_global_norm(np, active, 4.0)
            for key, gradient in gradients.items():
                model[key] -= step_size * 0.50 * gradient
            _synchronize_quantized_conv_weights(np, model)
        action_loss += transition_loss
        action_steps += 1

    model["visual_pretraining_steps"] = int(
        model.get("visual_pretraining_steps", 0)
    ) + trained
    total_steps = trained + action_steps
    return {
        "steps": float(trained),
        "loss": float((loss_total + action_loss) / max(1, total_steps)),
...
def world_stochastic_sample(
    ...
    seed = (
        0x51A7C0DE
        ^ (max(0, int(action_id)) * 0x9E3779B1)
        ^ (max(0, int(duration_index)) * 0x85EBCA77)
        ^ (max(0, int(token)) * 0xC2B2AE3D)
    ) & 0xFFFFFFFFFFFFFFFF
    rng = np.random.default_rng(seed)
    return (
        rng.standard_normal((WORLD_MODEL_MEMBERS, WORLD_STOCHASTIC_SIZE))
        .astype(np.float32)
    )


def _rssm_distribution(np, parameters):
    values = np.asarray(parameters, dtype=np.float32).reshape(
        WORLD_MODEL_MEMBERS, 2 * WORLD_STOCHASTIC_SIZE
    )
    mean = np.clip(values[:, :WORLD_STOCHASTIC_SIZE], -6.0, 6.0)
    log_std = np.clip(
        values[:, WORLD_STOCHASTIC_SIZE:],
        RSSM_MIN_LOG_STD,
        RSSM_MAX_LOG_STD,
    )
    std = np.exp(log_std).astype(np.float32)
    return mean.astype(np.float32), std, log_std.astype(np.float32)
...
def rssm_prior(
    ...
    deterministic = np.asarray(deterministic_state, dtype=np.float32).reshape(
        WORLD_MODEL_MEMBERS, WORLD_LATENT_SIZE
    )
    parameters = (
        np.einsum(
            "ml,mlz->mz", deterministic, model["world_prior_state_w"], optimize=True
        )
        + np.einsum(
            "a,maz->mz", model["action_embedding"][action],
            model["world_prior_action_w"], optimize=True,
        )
        + np.einsum(
            "d,mdz->mz", model["duration_embedding"][duration],
            model["world_prior_duration_w"], optimize=True,
        )
        + model["world_prior_b"]
    )
    return _rssm_distribution(np, parameters)


def rssm_posterior(np, model: dict, deterministic_state, observation_latent):
    deterministic = np.asarray(deterministic_state, dtype=np.float32).reshape(
        WORLD_MODEL_MEMBERS, WORLD_LATENT_SIZE
    )
    observation = np.asarray(observation_latent, dtype=np.float32).reshape(
        WORLD_MODEL_MEMBERS, WORLD_LATENT_SIZE
    )
    parameters = (
        np.einsum(
            "ml,mlz->mz", deterministic,
            model["world_posterior_state_w"], optimize=True,
        )
        + np.einsum(
            "ml,mlz->mz", observation,
            model["world_posterior_observation_w"], optimize=True,
        )
        + model["world_posterior_b"]
    )
    return _rssm_distribution(np, parameters)


def rssm_latent_from_state(np, model: dict, deterministic_state, stochastic_state):
    deterministic = np.asarray(deterministic_state, dtype=np.float32).reshape(
        WORLD_MODEL_MEMBERS, WORLD_LATENT_SIZE
    )
    stochastic = np.asarray(stochastic_state, dtype=np.float32).reshape(
        WORLD_MODEL_MEMBERS, WORLD_STOCHASTIC_SIZE
    )
    return np.tanh(
        deterministic
        + np.einsum(
            "ms,msl->ml", stochastic, model["world_stochastic_w"], optimize=True
        )
    ).astype(np.float32)


def rssm_kl_divergence(np, posterior, prior):
    posterior_mean, posterior_std, posterior_log_std = posterior
    prior_mean, prior_std, prior_log_std = prior
    prior_variance = np.maximum(prior_std * prior_std, 1e-8)
    divergence = (
        prior_log_std
        - posterior_log_std
        + (
            posterior_std * posterior_std
            + (posterior_mean - prior_mean) ** 2
        ) / (2.0 * prior_variance)
        - 0.5
    )
    return np.maximum(0.0, divergence).astype(np.float32)
...
def latent_world_model_step(
    ...
    stochastic_token: int | None = None,
):
    action = max(0, min(len(model["action_embedding"]) - 1, int(action_id)))
    duration = max(0, min(DURATION_HEAD_SIZE - 1, int(duration_index)))
    ...
    deterministic_state = np.tanh(
        np.einsum("ml,mlk->mk", latent_value, model["world_dynamics_w"])
        + np.einsum("a,mak->mk", action_embedding, model["world_dynamics_action_w"])
        + np.einsum("d,mdk->mk", duration_embedding, model["world_dynamics_duration_w"])
        + model["world_dynamics_b"]
    ).astype(np.float32)
    prior_mean, prior_std, _ = rssm_prior(
        np, model, deterministic_state, action, duration
    )
    stochastic = prior_mean
    if stochastic_token is not None:
        stochastic = prior_mean + prior_std * world_stochastic_sample(
            np, action, duration, stochastic_token
        )
    next_latents = rssm_latent_from_state(
        np, model, deterministic_state, stochastic
    )
    member_rewards = (
    ...
    continuation_probabilities = _sigmoid(
        np,
        np.einsum(
            "ml,mlq->mq", next_latents, model["world_continuation_w"]
        )
        + model["world_continuation_b"],
    )[:, 0]
    return (
        ...
        float(1.0 - continuation_probabilities.mean()),
    )


def world_model_planning_confidence(model: dict, world_steps: int, minimum_steps: int) -> float:
    full_confidence_steps = max(
        minimum_steps + 1,
        WORLD_MODEL_FULL_CONFIDENCE_STEPS,
    )
    step_confidence = max(
        0.0,
        min(
            1.0,
            (int(world_steps) - int(minimum_steps))
            / max(1, full_confidence_steps - int(minimum_steps)),
        ),
    )
    try:
        reward_mae = float(model.get("world_validation_reward_mae", -1.0))
        done_brier = float(model.get("world_validation_done_brier", -1.0))
        latent_error = float(model.get("world_validation_latent_error", -1.0))
        multistep_error = float(model.get("world_validation_multistep_error", -1.0))
    except (TypeError, ValueError, OverflowError):
        return 0.0
    if (
        not all(
            math.isfinite(value)
            for value in (reward_mae, done_brier, latent_error, multistep_error)
        )
        or reward_mae < 0.0
        or done_brier < 0.0
        or latent_error < 0.0
        or multistep_error < 0.0
    ):
        return 0.0
    quality_confidence = math.exp(
        -2.0 * max(0.0, reward_mae)
        -2.0 * max(0.0, done_brier)
...
def latent_world_model_duration_values(
    ...
    next_latent_values = []
    for duration_index, duration_embedding in enumerate(duration_embeddings):
        deterministic = np.tanh(
            np.einsum("ml,mlk->mk", latent, model["world_dynamics_w"])
            + np.einsum(
                "a,mak->mk", action_embedding, model["world_dynamics_action_w"]
            )
            + np.einsum(
                "d,mdk->mk", duration_embedding,
                model["world_dynamics_duration_w"],
            )
            + model["world_dynamics_b"]
        ).astype(np.float32)
        prior_mean, _, _ = rssm_prior(
            np, model, deterministic, action, duration_index
        )
        next_latent_values.append(
            rssm_latent_from_state(np, model, deterministic, prior_mean)
        )
    next_latents = np.stack(next_latent_values, axis=0)
    member_rewards = (
    ...
    continuation_probabilities = _sigmoid(
        np,
        np.einsum(
            "dml,mlq->dmq", next_latents,
            model["world_continuation_w"], optimize=True,
        )
        + model["world_continuation_b"][None, :, :],
    )[:, :, 0].mean(axis=1)
    conservative = member_combined.mean(axis=1) - uncertainty_weight * member_combined.std(axis=1)
    confidence = world_model_planning_confidence(model, world_steps, minimum_steps)
    result = conservative * np.maximum(0.0, continuation_probabilities) * confidence
    return np.clip(result, -4.0, 4.0)


def adaptive_world_model_horizon(
    confidence: float,
    requested_horizon: int,
    hierarchical_low_level: bool = False,
) -> int:
    quality = max(0.0, min(1.0, float(confidence)))
    if hierarchical_low_level:
        minimum, maximum = 1, 6
    else:
        minimum, maximum = 4, WORLD_MODEL_MAX_PLANNING_HORIZON
    quality_target = minimum + quality * (maximum - minimum)
    requested = max(minimum, min(maximum, int(requested_horizon)))
    return max(
        minimum,
        min(maximum, int(round(0.75 * quality_target + 0.25 * requested))),
    )
...
def latent_world_model_plan_values(
    ...
    horizon_value = adaptive_world_model_horizon(
        planning_confidence,
        int(horizon),
        hierarchical_low_level,
    )
    weights = critic_weight_vector(np, critic_weights)
    uncertainty_weight = (
        max(0.0, min(2.0, float(critic_weights.get("world_uncertainty_penalty", 0.18))))
    ...
    transition_cache: dict[tuple[bytes, int, int], tuple] = {}

    def predict_transition(latents, action: int, duration: int):
        quantized_latent = np.clip(
            np.rint(np.asarray(latents, dtype=np.float32) * 64.0),
            -127,
            127,
        ).astype(np.int8)
        digest = hashlib.blake2s(
            quantized_latent.tobytes(), digest_size=8
        ).digest()
        cache_key = (digest, int(action), int(duration))
        ...
        duration_embedding = model["duration_embedding"][duration]
        deterministic = np.tanh(
            np.einsum("ml,mlk->mk", latents, model["world_dynamics_w"], optimize=True)
        ...
        prior_mean, prior_std, _ = rssm_prior(
            np, model, deterministic, action, duration
        )
        # Beam members sample from the learned prior.  The deterministic token
        # is only an epsilon source; it no longer defines the latent dynamics.
        epsilon = world_stochastic_sample(
            np, action, duration, int.from_bytes(digest[:4], "little")
        )
        next_latents = rssm_latent_from_state(
            np, model, deterministic, prior_mean + prior_std * epsilon
        )
        member_rewards = (
        ...
        continuation_probability = float(
            _sigmoid(
                np,
                np.einsum(
                    "ml,mlq->mq", next_latents,
                    model["world_continuation_w"], optimize=True,
                )
                + model["world_continuation_b"],
            )[:, 0].mean()
        )
        done_probability = 1.0 - continuation_probability
        member_combined = member_rewards @ weights
        latent_disagreement = float(np.mean(np.std(next_latents, axis=0)))
        branch_uncertainty = float(member_combined.std()) + latent_disagreement
        answer = (next_latents, member_combined, done_probability, branch_uncertainty)
        ...
        for beam_score, beam_survival, first_action, beam_latents in beams:
            ...
            for action in actions:
                ...
                for duration in durations:
                    next_latents, member_combined, done_probability, branch_uncertainty = predict_transition(
                        beam_latents, action, duration
                    )
                    ...
                    uncertainty_limit = 0.55 + 0.65 * planning_confidence
                    if branch_uncertainty <= uncertainty_limit and new_survival >= 0.02:
...
def soft_update_target_model(np, target: dict, source: dict, rate: float) -> None:
    ...
    for cache_key in (
        "_vision_onnx_export",
        "_temporal_onnx_export",
        "_accelerated_policy_cache",
    ):
        target.pop(cache_key, None)
...
def temporal_critic_targets(
    ...
    evaluator = target_model if target_model is not None else model
    online_hidden = (
        _latest_temporal_hidden(np, model, next_feature)
        if online_next_hidden is None
        else np.asarray(online_next_hidden, dtype=np.float32)
    )
    evaluator_hidden = (
        _latest_temporal_hidden(np, evaluator, next_feature)
        if target_next_hidden is None
        else np.asarray(target_next_hidden, dtype=np.float32)
    )
    if online_hidden.shape != (int(model["hidden_size"]),):
        raise ValueError("在线回放隐藏状态尺寸无效")
    if evaluator_hidden.shape != (int(evaluator["hidden_size"]),):
        raise ValueError("目标回放隐藏状态尺寸无效")
...
def online_model_update(
    ...
    if lr <= 0.0:
        return False
    # Online play never changes the visual encoder, GRU, reward model or world
    # model.  It learns a small residual adapter plus exact policy/Q/value heads;
    # the complete offline representation remains a stable rollback anchor.
    hidden = (
        _latest_temporal_hidden(np, model, feature)
        if hidden_state is None
        else np.asarray(hidden_state, dtype=np.float32)
    )
    if hidden.shape != (int(model["hidden_size"]),) or not np.isfinite(hidden).all():
        return False
    adapted_hidden, adapter_cache = online_adapter_hidden(
        np, model, hidden, return_cache=True
    )
    probabilities, q_values, _ = factorized_action_outputs_from_hidden(np, model, hidden)
    value = float(adapted_hidden @ model["value_w"][:, 0] + model["value_b"][0])
    ...
    targets = temporal_critic_targets(
        ...
        critic_weights,
        next_hidden_state,
        target_next_hidden_state,
    )
    del target_hidden_state
    ...
    _, value_hidden_gradient = _train_value_head(
        np, model, adapted_hidden, q_data, value, 0.70, 1.0, gradients
    )
    _, q_hidden_gradient = _train_factor_q(
        np, model, adapted_hidden, int(action_index), targets, lr, 1.0,
        gradients=gradients, q_values=q_values, cql_weight=0.005,
        duration_index=executed_duration,
        hidden_is_adapted=True,
    )
    del world_rewards, world_next_feature, world_terminal
    ...
    if advantage > -0.25:
        ...
        _, policy_hidden_gradient = _train_factor_policy(
            np, model, adapted_hidden, model["action_factors"][int(action_index)], lr,
    ...
    adapter_input, adapter_residual = adapter_cache
    adapter_gradient = (
        np.asarray(hidden_gradient, dtype=np.float32)
        * ONLINE_ADAPTER_SCALE
        * (1.0 - adapter_residual * adapter_residual)
    )
    gradients["online_adapter_w"] += adapter_input * adapter_gradient
    gradients["online_adapter_b"] += adapter_gradient
    for key, gradient in gradients.items():
        if key not in ONLINE_TRAINABLE_KEYS:
            gradient.fill(0.0)
    active_gradients = [
        gradients[key] for key in ONLINE_TRAINABLE_KEYS if np.any(gradients[key])
    ]
    if not active_gradients:
        return False
    clip_gradients_by_global_norm(np, active_gradients, 3.5)
...
def human_training_signal(
    ...
    # Human controls are expert labels by default. A missing/heuristic score is
    # not evidence that the demonstrated action was wrong.
    return 1.0, value, 1.0


def human_record_has_explicit_failure(record: dict) -> bool:
    if bool(record.get("explicit_human_failure", False)):
        return True
    outcome_type = int(record.get("outcome_type", OUTCOME_NONE))
    confidence = max(
        0.0,
        min(1.0, float(record.get("outcome_confidence", 0.0))),
    )
    if outcome_type == OUTCOME_DEATH and confidence >= 0.50:
        return True
    return (
        outcome_type == OUTCOME_SCORE
        and confidence >= 0.60
        and float(record.get("score_delta", 0.0)) < 0.0
    )


def explicit_outcome_event(record: dict) -> float:
    outcome_type = int(record.get("outcome_type", OUTCOME_NONE))
    confidence = max(
        0.0,
        min(1.0, float(record.get("outcome_confidence", 0.0))),
    )
    if outcome_type == OUTCOME_VICTORY:
        return 1.0 * confidence
    if outcome_type == OUTCOME_LEVEL:
        return 0.70 * confidence
    if outcome_type == OUTCOME_DEATH:
        return -1.0 * confidence
    if outcome_type == OUTCOME_SCORE:
        delta = float(record.get("score_delta", 0.0))
        return math.tanh(delta / 3.0) * confidence
    return 0.0
...
def apply_outcome_credit(
    ...
    future_outcome = 0.0
    discount = max(0.0, min(1.0, float(gamma)))
    for record in reversed(records):
        event = explicit_outcome_event(record)
        future_outcome = event + discount * future_outcome
        future_outcome = max(-1.0, min(1.0, future_outcome))
        old_task = float(record.get("task_reward", 0.0))
        if abs(future_outcome) > 1e-6:
            record["task_reward"] = max(
                -4.0,
                min(
                    4.0,
                    0.30 * old_task
...
def _continuous_trajectory_parts(records: list[dict]) -> list[list[dict]]:
    episodes: dict[tuple[str, str, str], list[dict]] = {}
    for record in records:
        run_id = str(record.get("run_id", record["episode_id"]))
        episode_id = str(record["episode_id"])
        sequence_id = str(record.get("_sequence_id", episode_id))
        episodes.setdefault((run_id, episode_id, sequence_id), []).append(record)
...
def _select_episode_ids_stratified(
    ...
    while strata:
        ...
        for key in strata:
            ...
            selected.append(
                (str(candidate["run_id"]), str(candidate["episode_id"]))
            )
...
def contiguous_trajectory_records(
    ...
    strata: dict[str, dict[tuple[str, str], deque]] = {}
    for chunk in chunks:
        stratum = _sampling_stratum(chunk[0])
        episode_key = (
            str(chunk[0].get("run_id", chunk[0]["episode_id"])),
            str(chunk[0]["episode_id"]),
        )
        strata.setdefault(stratum, {}).setdefault(episode_key, deque()).append(chunk)
    for episode_map in strata.values():
        for episode_key, values in list(episode_map.items()):
            episode_map[episode_key] = deque(sorted(values, key=chunk_priority, reverse=True))

    stratum_order = list(strata)
    rng.shuffle(stratum_order)
    episode_orders: dict[str, list[tuple[str, str]]] = {}
    episode_offsets: dict[str, int] = {}
    for stratum in stratum_order:
        order = list(strata[stratum])
        rng.shuffle(order)
        order.sort(
            key=lambda episode_key: chunk_priority(strata[stratum][episode_key][0]),
            reverse=True,
        )
    ...
    while len(selected) < maximum and stratum_order:
        ...
        for stratum in stratum_order:
            ...
            if chosen:
                sequence_id = (
                    f"{chosen[0].get('run_id', chosen[0]['episode_id'])}:"
...
def load_training_data(
    ...
    try:
        ...
        for (
            ...
            victory_value = max(0.0, min(1.0, float(victory_confidence or 0.0)))
            death_value = max(0.0, min(1.0, float(death_confidence or 0.0)))
            level_value = max(0.0, min(1.0, float(level_confidence or 0.0)))
            explicit_score_value = float(explicit_score_delta or 0.0)
            if victory_value > death_value and victory_value > 0.0:
                failure_type = "none"
                success_rate = max(0.85, victory_value)
            elif death_value >= victory_value and death_value > 0.0:
                failure_type = "explicit_death"
                success_rate = max(0.0, 0.15 * (1.0 - death_value))
            elif level_value > 0.0:
                failure_type = "none"
                success_rate = max(0.70, 0.70 + 0.25 * level_value)
            elif explicit_score_value > 0.0:
                failure_type = "none"
                success_rate = max(
                    0.60,
                    min(0.90, 0.65 + 0.20 * math.tanh(explicit_score_value / 3.0)),
                )
            elif explicit_score_value < 0.0:
                failure_type = "explicit_score_loss"
                success_rate = max(
                    0.05,
                    0.30 - 0.20 * math.tanh(abs(explicit_score_value) / 3.0),
                )
            else:
        ...
        if not selected:
            return None
        connection.execute(
            "CREATE TEMP TABLE selected_episodes("
            "run_id TEXT NOT NULL,episode_id TEXT NOT NULL,"
            "PRIMARY KEY(run_id,episode_id)) WITHOUT ROWID"
        )
        connection.executemany(
            "INSERT OR IGNORE INTO selected_episodes(run_id,episode_id) VALUES(?,?)",
            selected,
        )
        rows = connection.execute(
            "SELECT t.run_id,t.episode_id,t.step,t.source,t.state,t.action,t.reward,"
            ...
            "t.terminated,t.truncated,t.capture_failed "
    ...
    for (
        ...
        try:
            ...
            terminated_value = bool(int(terminated))
            truncated_value = bool(int(truncated))
            boundary_value = terminated_value or truncated_value
            done_value = terminated_value
            ...
            if outcome_value not in OUTCOME_TYPES or not 0.0 <= outcome_confidence_value <= 1.0:
                raise ValueError
            task_value = max(-4.0, min(4.0, raw_task))
            ...
            if not boundary_value and next_blob is None:
                raise ValueError
            episodes.setdefault((str(run_id), str(episode_id)), []).append(
                {
                    "run_id": str(run_id),
                    "episode_id": str(episode_id),
                    "step": int(step),
    ...
    for records in episodes.values():
        ...
        victory_confidence = max(
            (
                float(record.get("outcome_confidence", 0.0))
                for record in records
                if int(record.get("outcome_type", OUTCOME_NONE)) == OUTCOME_VICTORY
            ),
            default=0.0,
        )
        death_confidence = max(
            (
                float(record.get("outcome_confidence", 0.0))
                for record in records
                if int(record.get("outcome_type", OUTCOME_NONE)) == OUTCOME_DEATH
            ),
            default=0.0,
        )
        level_confidence = max(
            (
                float(record.get("outcome_confidence", 0.0))
                for record in records
                if int(record.get("outcome_type", OUTCOME_NONE)) == OUTCOME_LEVEL
            ),
            default=0.0,
        )
        explicit_score_delta = sum(
            float(record.get("score_delta", 0.0))
            * float(record.get("outcome_confidence", 0.0))
            for record in records
            if int(record.get("outcome_type", OUTCOME_NONE)) == OUTCOME_SCORE
        )
        has_explicit_outcome = any(
            int(record.get("outcome_type", OUTCOME_NONE)) != OUTCOME_NONE
            and float(record.get("outcome_confidence", 0.0)) > 0.0
            for record in records
        )
        if victory_confidence > death_confidence and victory_confidence > 0.0:
            failure_type = "none"
            success_rate = max(0.85, victory_confidence)
        elif death_confidence >= victory_confidence and death_confidence > 0.0:
            failure_type = "explicit_death"
            success_rate = max(0.0, 0.15 * (1.0 - death_confidence))
        elif level_confidence > 0.0:
            failure_type = "none"
            success_rate = max(0.70, 0.70 + 0.25 * level_confidence)
        elif explicit_score_delta > 0.0:
            failure_type = "none"
            success_rate = max(
                0.60,
                min(0.90, 0.65 + 0.20 * math.tanh(explicit_score_delta / 3.0)),
            )
        elif explicit_score_delta < 0.0:
            failure_type = "explicit_score_loss"
            success_rate = max(
                0.05,
                0.30 - 0.20 * math.tanh(abs(explicit_score_delta) / 3.0),
            )
        elif human_episode:
            # No score is the normal case for demonstrations. Do not infer
            # failure from weak visual heuristics or a user-ended trajectory.
            failure_type = "none"
            success_rate = 0.85
        else:
            failure_type = _classify_failure_type(
        ...
        trajectory_class = (
            ...
            if (
                victory_confidence > death_confidence
                or level_confidence > 0.0
                or explicit_score_delta > 0.0
                or (not has_explicit_outcome and success_rate >= 0.65 and failure_type == "none")
            )
            else "failed_ai"
            if (
                death_confidence >= max(victory_confidence, 1e-9)
                or explicit_score_delta < 0.0
                or (not has_explicit_outcome and failure_type != "none" and success_rate < 0.45)
            )
            else "ordinary_ai"
        )
        explicit_human_failure = bool(
            human_episode
            and (
                death_confidence >= 0.50
                or explicit_score_delta < 0.0
            )
        )
        if human_episode:
            episode_quality = (
                max(0.15, min(0.40, success_rate))
                if explicit_human_failure
                else 1.0
            )
        ...
        for record in records:
            ...
            record["explicit_human_failure"] = explicit_human_failure
    ...
    # Explicit victory, level, death, and score events supervise every decision
    # that led to them. Recompose the scalar reward after task credit changes so
    # policy/value targets and the task critic observe the same signal.
    for records in episodes.values():
        apply_outcome_credit(records)
        for record in records:
            record["reward"] = max(
                -1.0,
                min(
                    ...
                    compose_reward(
                        {
                            ...
                            "exploration_reward": float(record["exploration_reward"]),
                            "safety_penalty": float(record["safety_penalty"]),
                        },
                        reward_config,
                    ),
                ),
    ...
    discount = 0.96
    run_ids = {
        str(record.get("run_id", record["episode_id"]))
        for records in episodes.values()
        for record in records[:1]
    }
    for (run_id, episode_id), records in episodes.items():
        run_id = str(run_id)
        validation_bucket = int.from_bytes(
            hashlib.sha256(run_id.encode("utf-8")).digest()[:4], "little"
        ) / 2**32
        # A single observed run cannot provide a leakage-free validation set.
        # Keep it for training and wait for a later independent run to validate.
        target = (
            validation_records
            if len(run_ids) > 1 and validation_bucket < validation_fraction
            else train_records
        )
        for index, record in enumerate(records):
            ...
            for offset in range(horizon):
                ...
                if bool(item.get("episode_end", n_done)):
                    break
                multiplier *= discount
            ...
            enriched.update(
                {
                    ...
                    "n_done": n_done,
                    "discount_power": (
                        0.0 if n_done or n_next is None else discount**steps_used
                    ),
                    "n_steps": steps_used,
                }
            )
            target.append(enriched)
    if not validation_records and len(run_ids) > 1:
        one_run = sorted(run_ids)[0]
        moved = [record for record in train_records if record["run_id"] == one_run]
        train_records = [record for record in train_records if record["run_id"] != one_run]
        validation_records.extend(moved)
    if not train_records and len(run_ids) > 1 and validation_records:
        one_run = str(validation_records[0]["run_id"])
        moved = [record for record in validation_records if record["run_id"] == one_run]
        validation_records = [record for record in validation_records if record["run_id"] != one_run]
...
def _convert_legacy_temporal_action_schema(state: bytes) -> bytes:
    frames, actions, durations, valid_length = decode_temporal_state_with_length(state)
    start = len(frames) - valid_length
    frames = frames[start:]
...
def hierarchical_skill_plan_values(
    np,
    model: dict,
    hidden,
    skills: list[dict],
    ...
    minimum_steps = max(WORLD_MODEL_MIN_TRAINING_STEPS, len(model["action_factors"]) * 8)
    confidence = world_model_planning_confidence(model, world_steps, minimum_steps)
    if count <= 0 or confidence <= 0.0:
        return result
    horizon = max(
        WORLD_MODEL_SKILL_MIN_HORIZON,
        min(
            WORLD_MODEL_SKILL_MAX_HORIZON,
            int(round(
                WORLD_MODEL_SKILL_MIN_HORIZON
                + confidence
                * (WORLD_MODEL_SKILL_MAX_HORIZON - WORLD_MODEL_SKILL_MIN_HORIZON)
            )),
        ),
    )
    gamma = (
        max(0.1, min(0.98, float(critic_weights.get("planning_discount", 0.85))))
        if isinstance(critic_weights, dict)
        else 0.85
    )
    uncertainty_weight = (
        max(0.0, min(2.0, float(critic_weights.get("world_uncertainty_penalty", 0.25))))
        if isinstance(critic_weights, dict)
        else 0.25
    )
    weights = critic_weight_vector(np, critic_weights)
    for skill_index, skill in enumerate(skills[:count]):
        actions = [
            max(0, min(len(model["action_factors"]) - 1, int(value)))
            for value in skill.get("actions", [])
        ]
        durations = [
            max(0, min(DURATION_HEAD_SIZE - 1, int(value)))
            for value in skill.get("durations", [])
        ]
        if not actions:
            continue
        if len(durations) < len(actions):
            durations.extend([DURATION_HEAD_SIZE // 2] * (len(actions) - len(durations)))
        latent = world_model_latent(np, model, hidden)
        survival = 1.0
        total = 0.0
        for depth in range(horizon):
            position = depth % len(actions)
            latent, reward_vector, done_probability = latent_world_model_step(
                np,
                model,
                latent,
                actions[position],
                durations[position],
                stochastic_token=skill_index * 257 + depth,
            )
            uncertainty = float(np.mean(np.std(latent, axis=0)))
            total += (
                gamma**depth
                * survival
                * (float(reward_vector @ weights) - uncertainty_weight * uncertainty)
            )
            survival *= max(0.0, 1.0 - done_probability)
            if survival < 0.02 or uncertainty > 0.55 + 0.65 * confidence:
                break
        result[skill_index] = float(total * confidence)
    return np.clip(result, -4.0, 4.0)
...
def choose_skill(
    ...
    world_steps = max(0, int(model.get("world_training_steps", 0)))
    minimum_steps = max(WORLD_MODEL_MIN_TRAINING_STEPS, len(model["action_factors"]) * 8)
    world_confidence = world_model_planning_confidence(model, world_steps, minimum_steps)
    effective_start = max(
    ...
    planned = hierarchical_skill_plan_values(
        np, model, hidden, skills[:count], critic_weights
    )
    planning_weight = (
        max(0.0, min(1.0, float(critic_weights.get("planning_weight", 0.38))))
        if isinstance(critic_weights, dict)
...
ONLINE_TRAINABLE_KEYS = frozenset({
    "online_adapter_w", "online_adapter_b",
    "policy_action_w", "policy_action_b",
    "q_action_w", "q_action_b",
    "value_w", "value_b",
})
...
def _train_factor_policy(
    ...
    signed_weight = max(-100.0, min(100.0, float(weight)))
    magnitude = abs(signed_weight)
    local = gradients is None
    if local:
        gradients = _training_gradient_buffer(np, model)
    if magnitude <= 1e-12:
        return 0.0, np.zeros_like(np.asarray(hidden, dtype=np.float32))
    direction = 1.0 if signed_weight > 0.0 else -1.0
    objective_loss, objective_gradient = signed_policy_objective(
        np,
        np.asarray(probabilities, dtype=np.float32)[None, :],
        np.asarray([int(action_id)], dtype=np.int64),
        np.asarray([direction], dtype=np.float32),
        with_gradient=True,
    )
    action_gradient = np.asarray(objective_gradient[0], dtype=np.float32) * magnitude
    loss = float(objective_loss) * magnitude
    factor_matrix = model["action_factors"].astype(np.int64, copy=False)
    memorized_gradient = 0.30 * action_gradient
    factor_action_gradient = (0.70 / 4.0) * action_gradient
    hidden_gradient = model["policy_action_w"] @ memorized_gradient
    gradients["policy_action_w"] += np.outer(hidden, memorized_gradient).astype(np.float32)
    gradients["policy_action_b"] += memorized_gradient
    for w_key, b_key, column in (
        ...
        bucket_gradient = _factor_bucket_gradient(
            np, factor_matrix, column, factor_action_gradient, model[b_key].shape[0]
        )
    ...
    key_gradient = factor_action_gradient @ model["action_key_multihot"]
    button_gradient = factor_action_gradient @ model["action_button_multihot"]
    ...
    # Avoidance examples only suppress the chosen action. They must not teach
    # its duration or pointer offset as a desirable target.
    if signed_weight > 0.0 and duration_index is not None:
        ...
        loss += -math.log(max(1e-9, float(duration_probabilities[duration_target]))) * magnitude

    mouse_bucket = int(factor_matrix[int(action_id), 1])
    mouse_target = None
    if signed_weight > 0.0 and 1 <= mouse_bucket <= MOUSE_GRID_WIDTH * MOUSE_GRID_HEIGHT:
    ...
    elif signed_weight > 0.0 and 577 <= mouse_bucket < 602:
    ...
    if mouse_target is not None:
        ...
        loss += 0.10 * float(np.mean((mouse_prediction - mouse_target) ** 2)) * magnitude
    if local:
        active = [gradient for gradient in gradients.values() if np.any(gradient)]
        if active:
            clip_gradients_by_global_norm(np, active, 6.0)
...
def _train_factor_q(
    ...
    duration_index: int | None = None,
    hidden_is_adapted: bool = False,
) -> tuple[float, object]:
    step = max(0.0, min(0.02, float(learning_rate)))
    twin_q_values = factorized_twin_q_outputs_from_hidden(
        np, model, hidden, hidden_is_adapted=hidden_is_adapted
    )
    local = gradients is None
    if local:
        gradients = _training_gradient_buffer(np, model)
    ...
    for twin in range(Q_TWIN_COUNT):
        for head in range(VALUE_HEAD_COUNT):
            ...
            memorized_gradient = 0.30 * action_gradient
            factor_action_gradient = 0.70 * action_gradient
            gradients["q_action_w"][twin, head] += np.outer(hidden, memorized_gradient).astype(np.float32)
            gradients["q_action_b"][twin, head] += memorized_gradient
            hidden_gradient += model["q_action_w"][twin, head] @ memorized_gradient
            for w_key, b_key, column in (
                ("q_control_w", "q_control_b", 0),
                ("q_mouse_w", "q_mouse_b", 1),
            ):
            ...
            key_gradient = (factor_action_gradient / 4.0) @ model["action_key_multihot"]
            button_gradient = (factor_action_gradient / 4.0) @ model["action_button_multihot"]
...
def train_world_model_transition(
    ...
    total_loss = 0.0
    continuation_target = 0.0 if done else 1.0
    member_scale = 1.0 / WORLD_MODEL_MEMBERS
    epsilon_values = world_stochastic_sample(
        np,
        action,
        duration,
    ...
    for member in range(WORLD_MODEL_MEMBERS):
        ...
        deterministic = np.tanh(dynamics_pre)
        prior_parameters = (
            deterministic @ model["world_prior_state_w"][member]
            + action_embedding @ model["world_prior_action_w"][member]
            + duration_embedding @ model["world_prior_duration_w"][member]
            + model["world_prior_b"][member]
        )
        prior_mean = np.clip(
            prior_parameters[:WORLD_STOCHASTIC_SIZE], -6.0, 6.0
        )
        prior_log_std = np.clip(
            prior_parameters[WORLD_STOCHASTIC_SIZE:],
            RSSM_MIN_LOG_STD,
            RSSM_MAX_LOG_STD,
        )
        prior_std = np.exp(prior_log_std).astype(np.float32)
        posterior_parameters = None
        if target_latents is not None:
            posterior_parameters = (
                deterministic @ model["world_posterior_state_w"][member]
                + target_latents[member]
                @ model["world_posterior_observation_w"][member]
                + model["world_posterior_b"][member]
            )
            posterior_mean = np.clip(
                posterior_parameters[:WORLD_STOCHASTIC_SIZE], -6.0, 6.0
            )
            posterior_log_std = np.clip(
                posterior_parameters[WORLD_STOCHASTIC_SIZE:],
                RSSM_MIN_LOG_STD,
                RSSM_MAX_LOG_STD,
            )
            posterior_std = np.exp(posterior_log_std).astype(np.float32)
            stochastic_state = (
                posterior_mean + posterior_std * epsilon_values[member]
            )
        else:
            posterior_mean = prior_mean
            posterior_log_std = prior_log_std
            posterior_std = prior_std
            stochastic_state = prior_mean
        stochastic_pre = (
            deterministic
            + stochastic_state @ model["world_stochastic_w"][member]
        )
        predicted_latent = np.tanh(stochastic_pre)
        predicted_rewards = (
        ...
        continuation_probability = float(
            _sigmoid(
                np,
                predicted_latent @ model["world_continuation_w"][member]
                + model["world_continuation_b"][member],
            )[0]
        )
        continuation_gradient = (
            continuation_probability - continuation_target
        ) * bootstrap_weight * 0.5 * member_scale
        gradients["world_continuation_w"][member, :, 0] += (
            predicted_latent * continuation_gradient
        )
        gradients["world_continuation_b"][member, 0] += continuation_gradient
        predicted_gradient += (
            model["world_continuation_w"][member, :, 0]
            * continuation_gradient
        )
        ...
        stochastic_pre_gradient = predicted_gradient * (
            1.0 - predicted_latent * predicted_latent
        )
        gradients["world_stochastic_w"][member] += np.outer(
            stochastic_state, stochastic_pre_gradient
        ).astype(np.float32)
        stochastic_gradient = (
            model["world_stochastic_w"][member] @ stochastic_pre_gradient
        )
        deterministic_gradient = stochastic_pre_gradient.copy()

        prior_gradient = np.zeros(2 * WORLD_STOCHASTIC_SIZE, dtype=np.float32)
        posterior_gradient = np.zeros_like(prior_gradient)
        kl_loss = 0.0
        if posterior_parameters is not None:
            posterior_gradient[:WORLD_STOCHASTIC_SIZE] += stochastic_gradient
            posterior_gradient[WORLD_STOCHASTIC_SIZE:] += (
                stochastic_gradient * posterior_std * epsilon_values[member]
            )
            ...
            divergence = (
                prior_log_std
                - posterior_log_std
                + (posterior_variance + difference * difference)
                / (2.0 * prior_variance)
                - 0.5
            )
            mean_kl = float(np.mean(np.maximum(0.0, divergence)))
            kl_loss = RSSM_KL_WEIGHT * bootstrap_weight * max(
                0.0, mean_kl - RSSM_KL_FREE_NATS
            )
            # Free nats clamp the reported loss, while a balanced KL gradient
            # still teaches the prior to track every observed posterior.
            if math.isfinite(mean_kl):
                kl_scale = (
                    RSSM_KL_WEIGHT
                    * bootstrap_weight
                    * member_scale
                    / WORLD_STOCHASTIC_SIZE
                )
                posterior_gradient[:WORLD_STOCHASTIC_SIZE] += (
                    difference / prior_variance
                ) * kl_scale
                posterior_gradient[WORLD_STOCHASTIC_SIZE:] += (
                    -1.0 + posterior_variance / prior_variance
                ) * kl_scale
                prior_gradient[:WORLD_STOCHASTIC_SIZE] -= (
                    difference / prior_variance
                ) * kl_scale
                prior_gradient[WORLD_STOCHASTIC_SIZE:] += (
                    1.0
            ...
            gradients["world_posterior_state_w"][member] += np.outer(
                deterministic, posterior_gradient
            )
            gradients["world_posterior_observation_w"][member] += np.outer(
                target_latents[member], posterior_gradient
            )
            gradients["world_posterior_b"][member] += posterior_gradient
            deterministic_gradient += (
                model["world_posterior_state_w"][member] @ posterior_gradient
            )
        else:
            prior_gradient[:WORLD_STOCHASTIC_SIZE] += stochastic_gradient

        gradients["world_prior_state_w"][member] += np.outer(
            deterministic, prior_gradient
        )
        gradients["world_prior_action_w"][member] += np.outer(
            action_embedding, prior_gradient
        )
        gradients["world_prior_duration_w"][member] += np.outer(
            duration_embedding, prior_gradient
        )
        gradients["world_prior_b"][member] += prior_gradient
        deterministic_gradient += (
            model["world_prior_state_w"][member] @ prior_gradient
        )
        action_gradient += (
            model["world_prior_action_w"][member] @ prior_gradient
        )
        duration_gradient += (
            model["world_prior_duration_w"][member] @ prior_gradient
        )

        dynamics_gradient = deterministic_gradient * (
            1.0 - deterministic * deterministic
        )
        ...
        continuation_loss = -0.5 * bootstrap_weight * (
            continuation_target
            * math.log(max(1e-9, continuation_probability))
            + (1.0 - continuation_target)
            * math.log(max(1e-9, 1.0 - continuation_probability))
        )
        total_loss += (
            dynamics_loss
            + projection_loss
...
def _legacy_train_world_model_overshooting(
    np,
    model: dict,
    start_hidden,
    rollout: list[tuple[dict, object | None]],
    sample_weight: float,
    gradients: dict[str, object],
    target_model: dict,
) -> tuple[float, object]:
    """Train recursive 5-12 step imagination against stop-gradient targets."""
    items = list(rollout)[:WORLD_MODEL_OVERSHOOT_MAX_STEPS]
    if len(items) < WORLD_MODEL_OVERSHOOT_MIN_STEPS:
        return 0.0, np.zeros_like(np.asarray(start_hidden, dtype=np.float32))
    weight = max(0.0, min(2.0, float(sample_weight)))
    latent = world_model_latent(np, model, start_hidden)
    caches = []
    total_loss = 0.0
    for offset, (record, target_next_hidden) in enumerate(items):
        action = max(0, min(len(model["action_embedding"]) - 1, int(record["action"])))
        duration = _record_duration_index(record)
        action_embedding = model["action_embedding"][action]
        duration_embedding = model["duration_embedding"][duration]
        stochastic = world_stochastic_sample(
            np,
            action,
            duration,
            int(model.get("world_training_steps", 0)) + offset,
        )
        next_latent = np.tanh(
            np.einsum("ml,mlk->mk", latent, model["world_dynamics_w"], optimize=True)
            + np.einsum("a,mak->mk", action_embedding, model["world_dynamics_action_w"], optimize=True)
            + np.einsum("d,mdk->mk", duration_embedding, model["world_dynamics_duration_w"], optimize=True)
            + np.einsum("mz,mzk->mk", stochastic, model["world_stochastic_w"], optimize=True)
            + model["world_dynamics_b"]
        ).astype(np.float32)
        predicted_rewards = (
            np.einsum("ml,mlv->mv", next_latent, model["world_reward_w"], optimize=True)
            + np.einsum("a,mav->mv", action_embedding, model["world_reward_action_w"], optimize=True)
            + np.einsum("d,mdv->mv", duration_embedding, model["world_reward_duration_w"], optimize=True)
            + model["world_reward_b"]
        ).astype(np.float32)
        done_probability = _sigmoid(
            np,
            np.einsum("ml,mlq->mq", next_latent, model["world_done_w"], optimize=True)
            + model["world_done_b"],
        )[:, 0].astype(np.float32)
        done_target = 1.0 if bool(record.get("done")) else 0.0
        reward_targets = np.asarray(
            (
                smooth_reward(float(record.get("task_reward", 0.0))),
                max(0.0, min(1.0, float(record.get("exploration_reward", 0.0)))),
                max(0.0, min(2.0, float(record.get("safety_penalty", 0.0)))),
            ),
            dtype=np.float32,
        )
        target_latent = (
            world_model_latent(np, target_model, target_next_hidden)
            if target_next_hidden is not None and not done_target
            else None
        )
        step_weight = weight * (0.92 ** offset)
        reward_error = predicted_rewards - reward_targets[None, :]
        total_loss += 0.35 * step_weight * float(np.mean(reward_error * reward_error))
        total_loss += -0.15 * step_weight * float(np.mean(
            done_target * np.log(np.maximum(done_probability, 1e-9))
            + (1.0 - done_target) * np.log(np.maximum(1.0 - done_probability, 1e-9))
        ))
        if target_latent is not None:
            latent_error = next_latent - target_latent
            total_loss += 0.50 * step_weight * float(np.mean(latent_error * latent_error))
        caches.append(
            {
                "latent": latent,
        ...
        if done_target:
            break
    if len(caches) < WORLD_MODEL_OVERSHOOT_MIN_STEPS:
        return 0.0, np.zeros_like(np.asarray(start_hidden, dtype=np.float32))

    carry = np.zeros((WORLD_MODEL_MEMBERS, WORLD_LATENT_SIZE), dtype=np.float32)
    for cache in reversed(caches):
        latent = cache["latent"]
        next_latent = cache["next_latent"]
        action_embedding = cache["action_embedding"]
        duration_embedding = cache["duration_embedding"]
        step_weight = float(cache["weight"])
        action_gradient_total = np.zeros_like(action_embedding)
        duration_gradient_total = np.zeros_like(duration_embedding)
        previous_carry = np.zeros_like(carry)
        for member in range(WORLD_MODEL_MEMBERS):
            reward_error = np.clip(
                cache["predicted_rewards"][member] - cache["reward_targets"],
                -3.0,
                3.0,
            )
            reward_gradient = reward_error * (
                0.35 * step_weight / (WORLD_MODEL_MEMBERS * VALUE_HEAD_COUNT)
            )
            gradients["world_reward_w"][member] += np.outer(
                next_latent[member], reward_gradient
            )
            gradients["world_reward_action_w"][member] += np.outer(
                action_embedding, reward_gradient
            )
            ...
            gradients["world_reward_b"][member] += reward_gradient
            next_gradient = carry[member] + model["world_reward_w"][member] @ reward_gradient
            action_gradient = model["world_reward_action_w"][member] @ reward_gradient
            duration_gradient = model["world_reward_duration_w"][member] @ reward_gradient
            done_gradient = (
                cache["done_probability"][member] - cache["done_target"]
            ) * (0.15 * step_weight / WORLD_MODEL_MEMBERS)
            gradients["world_done_w"][member, :, 0] += next_latent[member] * done_gradient
            gradients["world_done_b"][member, 0] += done_gradient
            next_gradient += model["world_done_w"][member, :, 0] * done_gradient
            if cache["target_latent"] is not None:
                next_gradient += (
                    next_latent[member] - cache["target_latent"][member]
                ) * (0.50 * step_weight / (WORLD_MODEL_MEMBERS * WORLD_LATENT_SIZE))
            dynamics_gradient = next_gradient * (1.0 - next_latent[member] ** 2)
            gradients["world_dynamics_w"][member] += np.outer(
                latent[member], dynamics_gradient
            )
            gradients["world_dynamics_action_w"][member] += np.outer(
                action_embedding, dynamics_gradient
            )
            ...
            gradients["world_stochastic_w"][member] += np.outer(
                cache["stochastic"][member], dynamics_gradient
            )
            gradients["world_dynamics_b"][member] += dynamics_gradient
            previous_carry[member] = model["world_dynamics_w"][member] @ dynamics_gradient
            action_gradient += model["world_dynamics_action_w"][member] @ dynamics_gradient
            duration_gradient += model["world_dynamics_duration_w"][member] @ dynamics_gradient
            action_gradient_total += action_gradient
            duration_gradient_total += duration_gradient
        gradients["action_embedding"][cache["action"]] += action_gradient_total
        gradients["duration_embedding"][cache["duration"]] += duration_gradient_total
        carry = previous_carry

    start_hidden_value = np.asarray(start_hidden, dtype=np.float32)
    initial_latent = world_model_latent(np, model, start_hidden_value)
    hidden_gradient = np.zeros_like(start_hidden_value)
    for member in range(WORLD_MODEL_MEMBERS):
        encoder_gradient = carry[member] * (1.0 - initial_latent[member] ** 2)
        gradients["world_encoder_w"][member] += np.outer(
            start_hidden_value, encoder_gradient
        )
        gradients["world_encoder_b"][member] += encoder_gradient
        hidden_gradient += model["world_encoder_w"][member] @ encoder_gradient
    model["world_training_steps"] = int(model.get("world_training_steps", 0)) + len(caches)
    return float(total_loss), hidden_gradient.astype(np.float32, copy=False)



def train_world_model_overshooting(
    np,
    model: dict,
    start_hidden,
    rollout: list[tuple[dict, object | None]],
    ...
    """Sequence burn-in for the RSSM posterior/prior and prediction heads.

    Every step is teacher-forced by the next observation belief.  Planning is
    still open-loop through the learned prior, while training never substitutes
    a fixed pseudo-random latent for an observed posterior.
    """
    items = list(rollout)[:WORLD_MODEL_OVERSHOOT_MAX_STEPS]
    if len(items) < WORLD_MODEL_OVERSHOOT_MIN_STEPS:
        return 0.0, np.zeros_like(np.asarray(start_hidden, dtype=np.float32))
    current_hidden = np.asarray(start_hidden, dtype=np.float32)
    initial_gradient = np.zeros_like(current_hidden)
    total_loss = 0.0
    trained = 0
    base_weight = max(0.0, min(2.0, float(sample_weight)))
    for offset, (record, target_next_hidden) in enumerate(items):
        if target_next_hidden is None and not bool(record.get("done", False)):
            break
        reward_targets = (
            smooth_reward(float(record.get("task_reward", 0.0))),
            max(0.0, min(1.0, float(record.get("exploration_reward", 0.0)))),
        ...
        step_loss, step_gradient = train_world_model_transition(
            np,
            model,
            current_hidden,
            target_next_hidden,
        ...
        if trained == 0:
            initial_gradient += step_gradient
        total_loss += step_loss
        trained += 1
        if bool(record.get("done", False)) or target_next_hidden is None:
            break
        current_hidden = np.asarray(target_next_hidden, dtype=np.float32)
    if trained < WORLD_MODEL_OVERSHOOT_MIN_STEPS:
        return 0.0, np.zeros_like(np.asarray(start_hidden, dtype=np.float32))
    return float(total_loss / trained), initial_gradient.astype(np.float32, copy=False)
...
def online_adapter_hidden(np, model: dict, hidden, return_cache: bool = False):
    base = np.asarray(hidden, dtype=np.float32)
    weights = np.asarray(model.get("online_adapter_w", np.zeros(base.shape[-1])), dtype=np.float32)
    bias = np.asarray(model.get("online_adapter_b", np.zeros(base.shape[-1])), dtype=np.float32)
    if weights.shape != (base.shape[-1],) or bias.shape != (base.shape[-1],):
        raise ValueError("在线 adapter 尺寸无效")
    preactivation = base * weights + bias
    residual = np.tanh(preactivation).astype(np.float32, copy=False)
    adapted = (base + ONLINE_ADAPTER_SCALE * residual).astype(np.float32, copy=False)
    if return_cache:
        return adapted, (base, residual)
    return adapted


def _batch_twin_q_outputs_from_hidden(
    np,
    model: dict,
    hidden_matrix,
    hidden_is_adapted: bool = False,
    ...
    if not hidden_is_adapted:
        hidden_matrix = online_adapter_hidden(np, model, hidden_matrix)
    factors = model["action_factors"].astype(np.int64, copy=False)
    ...
    return (0.70 * factor_q + 0.30 * exact_q).astype(np.float32)


def _batch_model_outputs_from_hidden(np, model: dict, hidden_matrix):
    hidden_matrix = online_adapter_hidden(
        np, model, np.asarray(hidden_matrix, dtype=np.float32)
    )
    factors = model["action_factors"].astype(np.int64, copy=False)
    ...
    logits = (0.70 * factor_logits + 0.30 * memorized_logits).astype(np.float64)
    ...
    twin_q_values = _batch_twin_q_outputs_from_hidden(
        np, model, hidden_matrix, hidden_is_adapted=True
    )
...
def factorized_twin_q_outputs_from_hidden(
    np, model: dict, hidden, hidden_is_adapted: bool = False
):
    hidden_value = np.asarray(hidden, dtype=np.float32)[None, :]
    return _batch_twin_q_outputs_from_hidden(
        np, model, hidden_value, hidden_is_adapted=hidden_is_adapted
    )[0]


def factorized_action_outputs_from_hidden(np, model: dict, hidden):
    hidden_value = np.asarray(hidden, dtype=np.float32).reshape(-1)
    cached = model.get("_accelerated_policy_cache")
    if isinstance(cached, tuple) and len(cached) == 3:
        cache_key = hashlib.blake2s(
            hidden_value.tobytes(), digest_size=8
        ).digest()
        if hmac.compare_digest(cache_key, cached[0]):
            probabilities = np.asarray(cached[1], dtype=np.float32)
            adapted_hidden = np.asarray(cached[2], dtype=np.float32)
            twin_values = factorized_twin_q_outputs_from_hidden(
                np, model, adapted_hidden, hidden_is_adapted=True
            )
            return (
                probabilities.copy(),
                np.minimum(twin_values[0], twin_values[1]).astype(np.float32),
                adapted_hidden.copy(),
            )
    adapted_hidden = online_adapter_hidden(np, model, hidden)
    probabilities, q_values, _ = _batch_model_outputs_from_hidden(
        np,
        model,
        np.asarray(hidden, dtype=np.float32)[None, :],
    )
    return probabilities[0], q_values[0], adapted_hidden
...
def evaluate_model_records(np, model: dict, items: list[dict]) -> dict[str, float]:
    empty = {
        "accuracy": 0.0,
        "human_log_likelihood": 0.0,
        "progress_accuracy": 0.0,
        "return_ranking_accuracy": 0.0,
        "q_mae": 0.0,
        "q_calibration_error": 1.0,
        "value_mae": 0.0,
        "safety_error": 1.0,
        "score": -0.20,
        "episode_count": 0.0,
        "success_episode_rate": 0.0,
        "average_survival_steps": 0.0,
        "actual_cumulative_score": 0.0,
        "episode_test_score": 0.0,
        "outcome_prediction_score": 0.0,
        "outcome_accuracy": 0.0,
        "explicit_outcome_count": 0.0,
        "victory_episode_rate": 0.0,
        "death_episode_rate": 0.0,
        "level_episode_rate": 0.0,
        "explicit_score_delta": 0.0,
        "world_reward_mae": -1.0,
    ...
    def outcome_target(record: dict) -> tuple[float | None, float]:
        outcome_type = int(record.get("outcome_type", OUTCOME_NONE))
        confidence = max(0.0, min(1.0, float(record.get("outcome_confidence", 0.0))))
        if outcome_type == OUTCOME_DEATH:
            return -1.0, confidence
        if outcome_type == OUTCOME_VICTORY:
            return 1.0, confidence
        if outcome_type == OUTCOME_LEVEL:
            return 0.80, confidence
        if outcome_type == OUTCOME_SCORE:
            delta = float(record.get("score_delta", 0.0))
            if abs(delta) <= 1e-12:
                return None, 0.0
            return math.tanh(delta / max(1.0, abs(delta) * 0.25)), confidence
        return None, 0.0
        ...
        if len(episode_outputs) >= 2:
            for left in range(0, len(episode_outputs) - 4, max(1, len(episode_outputs) // 12)):
                ...
                if abs(b["return"] - a["return"]) > 0.05:
                    return_pairs += 1
                    return_correct += int(
                        (b["value"] > a["value"]) == (b["return"] > a["return"])
                    )
        if len(episode_outputs) >= 5:
            stride = max(1, len(episode_outputs) // 8)
            for start in range(0, len(episode_outputs) - 4, stride):
                rollout_steps = min(6, len(episode_outputs) - start - 1)
                latent = world_model_latent(
                    np, model, episode_outputs[start]["hidden"]
                )
                for offset in range(rollout_steps):
                    item = episode_outputs[start + offset]
                    latent, _, _ = latent_world_model_step(
                        np,
                        model,
                        latent,
                        int(item["action"]),
                        int(item["duration_index"]),
                    )
                target_latent = world_model_latent(
                    np,
                    model,
                    episode_outputs[start + rollout_steps]["hidden"],
                )
                world_multistep_errors.append(
                    float(np.sqrt(np.mean((latent - target_latent) ** 2)))
                )
        if episode_outputs:
            ...
            explicit = [
                item for item in episode_outputs
                if int(item["outcome_type"]) != OUTCOME_NONE
                and float(item["outcome_confidence"]) > 0.0
            ]
            victory = max(
                (float(item["outcome_confidence"]) for item in explicit if int(item["outcome_type"]) == OUTCOME_VICTORY),
                default=0.0,
            )
            death = max(
                (float(item["outcome_confidence"]) for item in explicit if int(item["outcome_type"]) == OUTCOME_DEATH),
                default=0.0,
            )
            level = max(
                (float(item["outcome_confidence"]) for item in explicit if int(item["outcome_type"]) == OUTCOME_LEVEL),
                default=0.0,
            )
            score_delta = sum(
                float(item["score_delta"]) * float(item["outcome_confidence"])
                for item in explicit
                if int(item["outcome_type"]) == OUTCOME_SCORE
            )
            trajectory_class = str(episode_outputs[-1].get("trajectory_class", ""))
            source_name = str(episode_outputs[-1].get("source", ""))
            if victory > 0.0 or death > 0.0:
                actual_success = victory > death
            elif level > 0.0 or score_delta > 0.0:
                actual_success = True
            else:
                actual_success = trajectory_class == "successful_ai" or (
                    source_name == "human" and task_total > max(0.05, safety_total * 0.50)
                )
            episode_tests.append(
                {
                    "success": float(actual_success),
                    "survival": float(len(episode_outputs)),
                    "score": float(score_delta if explicit else task_total),
                    "victory": float(victory > 0.0),
                    "death": float(death > 0.0),
                    "level": float(level > 0.0),
                    "explicit": float(bool(explicit)),
                }
            )
        episode_outputs = []
    ...
    for record in sorted(
        items,
        key=lambda item: (
            str(item.get("run_id", item["episode_id"])),
            str(item["episode_id"]),
        ...
        sequence_key = (
            run_id,
            episode_id,
            str(record.get("_sequence_id", episode_id)),
        )
        ...
        hidden = recurrent_model_step(
            np, model, frames[-1], hidden, actions[-1], durations[-1]
        )
        probabilities, q_values, _ = factorized_action_outputs_from_hidden(np, model, hidden)
        ...
        latent = world_model_latent(np, model, hidden)
        predicted_latents, predicted_rewards, predicted_done = latent_world_model_step(
            np,
            model,
            latent,
            action_id,
            _record_duration_index(record),
        )
        external_rewards = np.asarray(
            [
                smooth_reward(float(record.get("task_reward", 0.0))),
                max(
                    0.0,
                    min(1.0, float(record.get("exploration_reward", 0.0))),
                ),
                max(
                    0.0,
                    min(2.0, float(record.get("safety_penalty", 0.0))),
                ),
            ],
            dtype=np.float32,
        )
        world_reward_errors.append(float(np.mean(np.abs(predicted_rewards - external_rewards))))
        world_done_errors.append((float(predicted_done) - float(bool(record.get("done")))) ** 2)
        if record.get("next_state") is not None and not bool(record.get("done")):
            try:
                actual_next_hidden = _latest_temporal_hidden(np, model, record["next_state"])
                actual_next_latents = world_model_latent(np, model, actual_next_hidden)
        ...
        explicit_target, explicit_weight = outcome_target(record)
        if explicit_target is not None and explicit_weight > 0.0:
            outcome_prediction = math.tanh(float(critic_prediction[TASK_CRITIC]))
            outcome_error_total += explicit_weight * abs(outcome_prediction - explicit_target)
            outcome_weight_total += explicit_weight
            outcome_correct_total += explicit_weight * float(
                (outcome_prediction >= 0.0) == (explicit_target >= 0.0)
            )
        ...
        episode_outputs.append(
            {
                ...
                "outcome_type": int(record.get("outcome_type", OUTCOME_NONE)),
                "outcome_confidence": float(record.get("outcome_confidence", 0.0)),
                "score_delta": float(record.get("score_delta", 0.0)),
    ...
    explicit_episodes = [item for item in episode_tests if item["explicit"] > 0.0]
    explicit_episode_count = len(explicit_episodes)
    victory_episode_rate = (
        sum(item["victory"] for item in explicit_episodes) / explicit_episode_count
        if explicit_episode_count else 0.0
    )
    death_episode_rate = (
        sum(item["death"] for item in explicit_episodes) / explicit_episode_count
        if explicit_episode_count else 0.0
    )
    level_episode_rate = (
        sum(item["level"] for item in explicit_episodes) / explicit_episode_count
        if explicit_episode_count else 0.0
    )
    explicit_score_delta = (
        sum(item["score"] for item in explicit_episodes) / explicit_episode_count
        if explicit_episode_count else 0.0
    )
    outcome_prediction_score = (
        max(0.0, min(1.0, 1.0 - outcome_error_total / (2.0 * outcome_weight_total)))
        if outcome_weight_total > 0.0 else 0.0
    )
    outcome_accuracy = (
        outcome_correct_total / outcome_weight_total
        if outcome_weight_total > 0.0 else 0.0
    )
    # Kept as a compatibility/logging alias. It is deliberately model-only;
    # actual win/death/score/survival statistics are dataset properties and do
    # not contribute to candidate ranking.
    episode_test_score = outcome_prediction_score
    world_reward_mae = (
        sum(world_reward_errors) / len(world_reward_errors)
        if world_reward_errors else -1.0
    )
    world_done_brier = (
        sum(world_done_errors) / len(world_done_errors)
        if world_done_errors else -1.0
    )
    world_latent_error = (
        sum(world_latent_errors) / len(world_latent_errors)
        if world_latent_errors else -1.0
    )
    world_multistep_error = (
    ...
    score = (
        ...
        + 0.14 * progress_accuracy
        + 0.14 * normalized_log_likelihood
        + 0.06 * accuracy
        + 0.05 * max(0.0, 1.0 - q_calibration_error)
        + 0.03 * max(0.0, 1.0 - safety_error)
    )
    return {
        ...
        "outcome_prediction_score": float(outcome_prediction_score),
        "outcome_accuracy": float(outcome_accuracy),
        "explicit_outcome_count": float(outcome_weight_total),
        "victory_episode_rate": float(victory_episode_rate),
        "death_episode_rate": float(death_episode_rate),
        "level_episode_rate": float(level_episode_rate),
        "explicit_score_delta": float(explicit_score_delta),
        "world_reward_mae": float(world_reward_mae),
...
def train_model(
    ...
    semantic_cache_limit = training_vector_cache_limit(len(records))
    model["_semantic_cache_limit"] = semantic_cache_limit
    target_model["_semantic_cache_limit"] = semantic_cache_limit
    _ensure_optimizer_state(np, model)
        ...
        for group_start in range(0, len(all_windows), windows_per_batch):
            ...
            for window, first_train in all_windows[group_start:group_start + windows_per_batch]:
                ...
                for local_index, record in enumerate(train_items):
                    ...
                    human_policy_multiplier, _, value_weight = (
                    ...
                    stored_task_target = float(
                        record.get("n_task_return", record.get("task_reward", 0.0))
                    )
                    task_return_anchor = stored_task_target
                    if (
                        has_future
                        and abs(stored_task_target) < 0.05
                        and int(record.get("outcome_type", OUTCOME_NONE)) == OUTCOME_NONE
                        and future_position is not None
                    ):
                        target_progress_now = predict_progress(
                            np,
                            target_model,
                            target_encoded[position][0],
                        )
                        target_progress_future = predict_progress(
                            np,
                            target_model,
                            target_encoded[future_position][0],
                        )
                        target_progress_delta = float(np.clip(
                            target_progress_future - target_progress_now,
                            -0.50,
                            0.50,
                        ))
                        task_return_anchor += 0.10 * target_progress_delta
                    ...
                    world_task_target = smooth_reward(external_task_target)
                    ...
                    if local_index % WORLD_MODEL_OVERSHOOT_MIN_STEPS == 0:
                        overshoot_rollout = []
                        start_step = int(record["step"])
                        for rollout_offset in range(WORLD_MODEL_OVERSHOOT_MAX_STEPS):
                            rollout_position = position + rollout_offset
                            if rollout_position >= len(window):
                                break
                            rollout_record = window[rollout_position]
                            if int(rollout_record["step"]) != start_step + rollout_offset:
                                break
                            rollout_next_position = step_to_position.get(
                                int(rollout_record["step"]) + 1
                            )
                            if bool(rollout_record.get("done")):
                                overshoot_rollout.append((rollout_record, None))
                                break
                            if rollout_next_position is None:
                                break
                            overshoot_rollout.append(
                                (
                                    rollout_record,
                                    target_encoded[rollout_next_position][0],
                                )
                            )
                        if len(overshoot_rollout) >= WORLD_MODEL_OVERSHOOT_MIN_STEPS:
                            ...
                            world_hidden_gradient += overshoot_hidden_gradient
                            world_loss += overshoot_loss
                    advantage = q_data - value
                    actor_weight = min(
                        20.0,
                    ...
                    if record["source"] == "human":
                        ...
                        failed_safety = float(
                            record.get("n_safety_return", record.get("safety_penalty", 0.0))
                        )
                        trajectory_scale = (
                            0.0
                            if failed_return <= 0.0 or failed_safety >= 0.25
                            else 0.02
                        )
                        if failed_return <= -0.20 or failed_safety >= 0.50:
                            policy_direction = -1.0
                            trajectory_scale = max(0.06, trajectory_scale)
                    else:
                        trajectory_scale = 0.08
                    safe_scale = max(0.05, 1.0 - 0.55 * float(record["safety_penalty"]))
                    policy_weight = policy_direction * bc_weight * actor_weight * trajectory_scale * safe_scale * min(2.0, max(0.5, float(record["priority"]) / 2.0))
    ...
    model["world_validation_reward_mae"] = float(validation_metrics["world_reward_mae"])
    ...
    return {
        ...
        "validation_outcome_prediction_score": validation_metrics["outcome_prediction_score"],
        "validation_outcome_accuracy": validation_metrics["outcome_accuracy"],
        "validation_explicit_outcome_count": validation_metrics["explicit_outcome_count"],
        "validation_victory_episode_rate": validation_metrics["victory_episode_rate"],
        "validation_death_episode_rate": validation_metrics["death_episode_rate"],
        "validation_level_episode_rate": validation_metrics["level_episode_rate"],
        "validation_explicit_score_delta": validation_metrics["explicit_score_delta"],
        "validation_world_reward_mae": validation_metrics["world_reward_mae"],
        "validation_world_done_brier": validation_metrics["world_done_brier"],
        "validation_world_latent_error": validation_metrics["world_latent_error"],
        "validation_world_multistep_error": validation_metrics["world_multistep_error"],
        ...
        "visual_action_prediction_steps": int(visual_pretraining.get("action_prediction_steps", 0)),
...
def runtime_self_check(np) -> None:
    ...
    legacy_payload = bytearray(LEGACY_TEMPORAL_STATE_MAGIC)
    ...
    second_feature = make_feature(previous, current, red, blue)
    sequence_state = build_temporal_state(
        [feature, second_feature], [0, 1], [0.07, 0.13]
    )
    _, _, _, valid_history_length = decode_temporal_state_with_length(
        sequence_state
    )
    full_history_hidden = _latest_temporal_hidden(np, model, sequence_state)
    last_only_hidden = _latest_temporal_hidden(
        np,
        model,
        build_temporal_state([second_feature], [1], [0.13]),
    )
    if (
        valid_history_length != 2
        or np.allclose(full_history_hidden, last_only_hidden, atol=1e-7)
        or not np.allclose(
            _model_hidden_state(np, model, sequence_state),
            full_history_hidden,
            atol=1e-7,
        )
    ):
        raise RuntimeError("AGT4 有效历史与按序 burn-in 自检失败")
    ...
    if (
        ...
        or model["online_adapter_w"].shape != (model["hidden_size"],)
        or model["online_adapter_b"].shape != (model["hidden_size"],)
        ...
        or model["world_prior_state_w"].shape != (
            WORLD_MODEL_MEMBERS, WORLD_LATENT_SIZE, 2 * WORLD_STOCHASTIC_SIZE
        )
        or model["world_posterior_observation_w"].shape != (
            WORLD_MODEL_MEMBERS, WORLD_LATENT_SIZE, 2 * WORLD_STOCHASTIC_SIZE
        )
        or model["world_continuation_w"].shape != (
            WORLD_MODEL_MEMBERS, WORLD_LATENT_SIZE, 1
        )
    ...
    overshoot_loss, overshoot_hidden_gradient = train_world_model_overshooting(
        ...
        [
            ...
            for offset in range(WORLD_MODEL_OVERSHOOT_MIN_STEPS)
        ],
        0.5,
        overshoot_gradients,
        clone_target_model(np, model),
    )
    if (
        ...
        or not np.any(overshoot_gradients["world_dynamics_w"])
    ):
        raise RuntimeError("独立安全头与多步动作时长潜在世界模型训练自检失败")
    ...
    frozen_online_parameters = {
        key: model[key].copy()
        for key in (
            *_CONV_PARAMETER_KEYS,
            *_TEMPORAL_PARAMETER_KEYS,
            "progress_w", "progress_b", "safety_w", "safety_b",
            "reward_model_w", "reward_model_b", "world_dynamics_w",
            "world_reward_w", "world_done_w", "world_prior_state_w",
            "world_prior_action_w", "world_prior_duration_w", "world_prior_b",
            "world_posterior_state_w", "world_posterior_observation_w",
            "world_posterior_b", "world_stochastic_w",
            "world_continuation_w", "world_continuation_b",
        )
    }
    online_model_update(
    ...
    if any(
        not np.array_equal(model[key], value)
        for key, value in frozen_online_parameters.items()
    ):
        raise RuntimeError("在线 adapter 冻结离线骨干自检失败")
    ...
    confidence_probe = dict(model)
    confidence_probe["world_validation_reward_mae"] = 0.10
    confidence_probe["world_validation_done_brier"] = 0.10
    confidence_probe["world_validation_latent_error"] = 0.0
    confidence_probe["world_validation_multistep_error"] = 0.05
    accurate_world_confidence = world_model_planning_confidence(
        confidence_probe, WORLD_MODEL_FULL_CONFIDENCE_STEPS, 16
    )
    confidence_probe["world_validation_latent_error"] = 1.0
    confidence_probe["world_validation_multistep_error"] = 1.0
    inaccurate_world_confidence = world_model_planning_confidence(
        confidence_probe, WORLD_MODEL_FULL_CONFIDENCE_STEPS, 16
    )
    if not 0.0 <= inaccurate_world_confidence < accurate_world_confidence <= 1.0:
        raise RuntimeError("世界模型潜状态置信度自检失败")
...
def validate_model_file(path: Path, profile: dict, config: dict) -> bool:
    np=import_numpy(); model,changed=load_model(np,path,MODEL_INPUT_DIM,configured_teacher_hidden_size(config),len(profile["actions"]),profile["actions"])
...
def repair_profile(profile_id: str, config: dict, stop_event: threading.Event | None) -> dict:
    ...
    np=import_numpy(); model,changed=load_model(np,paths["model"],MODEL_INPUT_DIM,configured_teacher_hidden_size(config),len(profile["actions"]),profile["actions"])
    if changed or not paths["model"].is_file(): save_model(np,paths["model"],model);repaired+=1
    best,best_changed=load_model(np,paths["best_model"],MODEL_INPUT_DIM,configured_teacher_hidden_size(config),len(profile["actions"]),profile["actions"])
    ...
    target,changed_target=load_model(np,paths["target_model"],MODEL_INPUT_DIM,configured_teacher_hidden_size(config),len(profile["actions"]),profile["actions"])
    ...
    best_target,best_target_changed=load_model(np,paths["best_target_model"],MODEL_INPUT_DIM,configured_teacher_hidden_size(config),len(profile["actions"]),profile["actions"])
    ...
    if paths["candidate_model"].exists():
        candidate,candidate_changed=load_model(np,paths["candidate_model"],MODEL_INPUT_DIM,configured_teacher_hidden_size(config),len(profile["actions"]),profile["actions"])
        if candidate_changed:
            save_model(np,paths["candidate_model"],best);repaired+=1
    if paths["candidate_target_model"].exists():
        candidate_target,candidate_target_changed=load_model(np,paths["candidate_target_model"],MODEL_INPUT_DIM,configured_teacher_hidden_size(config),len(profile["actions"]),profile["actions"])
        if candidate_target_changed:
            save_model(np,paths["candidate_target_model"],best_target);repaired+=1
    calibration_dataset=load_training_data(
        np,paths["db"],len(profile["actions"]),
        min(4096,int(config["train_sample_limit_per_game"])),stop_event,
        int(config["n_step_horizon"]),float(config["validation_episode_fraction"]),config,
    )
    calibration_records=list((calibration_dataset or {}).get("train",[]))
    for tier_name, hidden_size, runtime_tier in (("low",256,"low_numpy"),("mid",512,"mid_onnx"),("high",768,"high_directml")):
        ...
        if student_changed or not student_path.is_file():
            student=distill_teacher_model(np,best,hidden_size,profile["actions"],calibration_records,runtime_tier,stop_event)
            save_model(np,student_path,student);repaired+=1
        if student_target_changed or not student_target_path.is_file():
            student_target=distill_teacher_model(np,best_target,hidden_size,profile["actions"],calibration_records,runtime_tier,stop_event)
...
def ensure_files(stop_event: threading.Event | None) -> dict:
    ...
    if prior_existed and load_global_prior(np) is None:
        repaired += 1
    if GLOBAL_BACKBONE_PATH.exists() and load_global_backbone(np) is None:
        backup_corrupt(GLOBAL_BACKBONE_PATH)
        repaired += 1
...
def record_human_session(target: int, stop_event: threading.Event) -> str:
    ...
    try:
        while not stop_event.is_set():
            ...
            transition=(episode_id,step,"human",state,composite_action,reward,next_state,done,priority,float(metrics.get("task_reward",reward)),float(metrics.get("exploration_reward",0.0)),float(metrics.get("safety_penalty",max(0.0,-reward))),outcome_type,outcome_confidence,score_delta,run_id,terminated,truncated,capture_failed)
...
def train_all_profiles(stop_event: threading.Event) -> str:
    ...
    training_hidden_size=configured_teacher_hidden_size(config)
    backbone_updated=refresh_global_backbone(np,index,config,stop_event)
    for profile_id in sorted(index.get("profiles",{})):
        ...
        try:
            ...
            apply_global_backbone(np,candidate,full_transfer=first_model)
            apply_global_backbone(np,candidate_target,full_transfer=first_model)
            ...
            baseline_outcome_score=float(baseline.get("outcome_prediction_score",0.0))
            candidate_outcome_score=float(metrics.get("validation_outcome_prediction_score",0.0))
            explicit_outcome_count=float(metrics.get("validation_explicit_outcome_count",0.0))
            baseline_human_ll=float(baseline.get("human_log_likelihood",0.0))
            candidate_human_ll=float(metrics.get("validation_human_log_likelihood",0.0))
            baseline_return_ranking=float(baseline.get("return_ranking_accuracy",0.0))
            candidate_return_ranking=float(metrics.get("validation_return_ranking_accuracy",0.0))
            episode_count=int(metrics.get("validation_episode_count",0))
            minimum_validation_episodes=max(1,int(config.get("minimum_validation_episodes",20)))
            enough_validation=episode_count>=minimum_validation_episodes
            minimum_improvement=max(
                0.0,
                min(
                    0.25,
                    float(config.get("candidate_min_improvement",0.005)),
                ),
            )
            minimum_explicit_outcomes=max(
                3.0,
                minimum_validation_episodes*0.25,
            )
            has_reliable_outcomes=(
                explicit_outcome_count>=minimum_explicit_outcomes
            )
            if has_reliable_outcomes:
                primary_improved=(
                    candidate_outcome_score
                    >=baseline_outcome_score+minimum_improvement
                )
            else:
                primary_improved=(
                    candidate_human_ll>=baseline_human_ll+0.005
                    and candidate_return_ranking>=baseline_return_ranking
                )
            ...
            if accepted:
                ...
                decision=(
                    f"已建立首个 champion（{candidate_score:.3f}）"
                    if first_model
            ...
            planning_confidence=world_model_planning_confidence(
                candidate,
                int(candidate.get("world_training_steps",0)),
                max(WORLD_MODEL_MIN_TRAINING_STEPS,len(profile["actions"])*8),
            )
            summaries.append(f"{profile['name']}：{metrics['samples']}条/{metrics['episodes']}回合，技能{metrics['skills']}个，成功回合{metrics['validation_success_episode_rate']:.0%}，明确结果预测{metrics['validation_outcome_prediction_score']:.1%}，世界奖励MAE{metrics['validation_world_reward_mae']:.3f}/终止Brier{metrics['validation_world_done_brier']:.3f}/潜态{metrics['validation_world_latent_error']:.3f}/多步{metrics['validation_world_multistep_error']:.3f}/规划置信度{planning_confidence:.1%}，生存{metrics['validation_average_survival_steps']:.1f}步，实际累计分{metrics['validation_actual_cumulative_score']:.2f}，{decision}")
    ...
    return f"升级完成：{total_profiles} 个候选模型通过验证，训练 {total_samples} 条完整转移；清理 {total_removed} 条；best_teacher_model 可自动回滚"+("；已更新跨游戏共享骨干" if backbone_updated else "")+("；已更新跨游戏因子先验" if prior_updated else "")+"。"+detail
...
def _training_record_replay_sample(record: dict) -> dict | None:
    try:
        state = record["state"]
        action = int(record["action"])
        next_state = record.get("n_next_state", record.get("next_state"))
        terminal = bool(record.get("n_done", record.get("done", False)))
        source = str(record.get("source", "ai"))
        trajectory = str(record.get("trajectory_class", "ordinary_ai"))
        return {
            "episode_id": str(record.get("episode_id", "offline")),
            "state": state,
            "action": action,
            "duration_index": _record_duration_index(record),
            "reward": max(-3.0, min(3.0, float(record.get("n_return", record.get("reward", 0.0))))),
            "critic_rewards": (
                max(-3.0, min(3.0, float(record.get("n_task_return", record.get("task_reward", 0.0))))),
                max(0.0, min(3.0, float(record.get("n_exploration_return", record.get("exploration_reward", 0.0))))),
                max(0.0, min(4.0, float(record.get("n_safety_return", record.get("safety_penalty", 0.0))))),
            ),
            "world_rewards": (
                smooth_reward(float(record.get("task_reward", 0.0))),
                max(0.0, min(1.0, float(record.get("exploration_reward", 0.0)))),
            ...
            "world_next_state": record.get("next_state"),
            "world_terminal": bool(record.get("done", False)),
            "next_state": next_state,
            "discount": 0.0 if terminal else max(0.0, min(0.99, float(record.get("discount_power", 0.0)))),
            "terminal": terminal,
            "reliability": 1.0 if source == "human" else 0.90 if trajectory == "successful_ai" else 0.65,
            "success_rate": max(0.0, min(1.0, float(record.get("episode_success_rate", 0.50)))),
            "failure_type": str(record.get("failure_type", "none")),
            "novelty_score": max(0.0, min(1.0, float(record.get("novelty_score", 0.0)))),
            "priority": max(0.05, float(record.get("priority", 1.0))),
            "source": source,
            "trajectory_class": trajectory,
            "replay_group": "human" if source == "human" else "successful_ai" if trajectory == "successful_ai" else "offline",
        }
    except (KeyError, TypeError, ValueError, OverflowError):
        return None


def _mixed_online_replay_samples(population: list[dict], count: int) -> list[dict]:
    """40% human, 30% successful AI, 20% recent, 10% hard/rare."""
    maximum = max(0, min(len(population), int(count)))
    if maximum <= 0:
        return []
    quotas = (
    ...
    def take(pool: list[dict], requested: int) -> None:
        available = [item for item in pool if id(item) not in selected_ids]
        for item in _stratified_replay_samples(available, min(requested, len(available))):
            selected.append(item)
            selected_ids.add(id(item))

    for group, quota in quotas:
        take(
            [item for item in population if str(item.get("replay_group", "")) == group],
            quota,
        )
    hard_quota = max(0, maximum - sum(quota for _, quota in quotas))
    hard_pool = sorted(
        population,
        key=lambda item: (
            float(item.get("priority", 1.0))
            + 1.5 * float(item.get("novelty_score", 0.0))
            + (0.75 if str(item.get("failure_type", "none")) != "none" else 0.0)
        ),
        reverse=True,
    )
    take(hard_pool, hard_quota)
    if len(selected) < maximum:
        take(population, maximum - len(selected))
...
def _flush_online_n_step(
    ...
    while queue_items and (force or len(queue_items) >= horizon or queue_items[0]["done"]):
        ...
        next_hidden_state = first.get("next_hidden_state")
        target_next_hidden_state = first.get("target_next_hidden_state")
        terminal = first["done"]
        used = 0
        ...
        for item in list(queue_items)[:horizon]:
            ...
            next_hidden_state = item.get("next_hidden_state")
            target_next_hidden_state = item.get("target_next_hidden_state")
        ...
        replay_buffer.append(
            {
                ...
                "world_rewards": (
                    smooth_reward(float(first.get("task_reward", first["reward"]))),
                ...
                "next_state": next_state,
                "hidden_state": first.get("hidden_state"),
                "target_hidden_state": first.get("target_hidden_state"),
                "next_hidden_state": next_hidden_state,
                "target_next_hidden_state": target_next_hidden_state,
                ...
                "priority": max(0.05, 1.0 + novelty_total / max(1, used)),
                "source": "ai",
                "trajectory_class": "online",
                "replay_group": "recent_online",
            }
        )
        prepared += 1
        queue_items.popleft()
    ...
    samples = _mixed_online_replay_samples(population, count)
    updated = 0
    for sample in samples:
        ...
        if online_model_update(
            ...
            target_hidden_state=sample.get("target_hidden_state"),
            next_hidden_state=sample.get("next_hidden_state"),
            target_next_hidden_state=sample.get("target_next_hidden_state"),
        ):
            updated += 1
    return updated
...
def run_ai_session(target: int, stop_event: threading.Event) -> str:
    ...
    candidate_path, candidate_target_path = runtime_student_candidate_paths(
        paths, runtime_tier
    )
    session_variant = live_session_variant(
        profile,
        runtime_tier,
        candidate_path.is_file() and candidate_target_path.is_file(),
    )
    model_load_path = candidate_path if session_variant == "challenger" else active_path
    target_load_path = (
        candidate_target_path
        if session_variant == "challenger"
        else active_target_path
    )
    if model_load_path.is_file():
        model, changed = load_model(
            np, model_load_path, MODEL_INPUT_DIM, model_hidden_size,
        ...
        if teacher_path.is_file():
            teacher, _ = load_model(
                np, teacher_path, MODEL_INPUT_DIM, configured_teacher_hidden_size(config),
                len(profile["actions"]), profile["actions"],
            )
            calibration_dataset = load_training_data(
                np,
                paths["db"],
                len(profile["actions"]),
                min(4096, int(config["train_sample_limit_per_game"])),
                stop_event,
                int(config["n_step_horizon"]),
                float(config["validation_episode_fraction"]),
                config,
            ...
            calibration_records = list(
                (calibration_dataset or {}).get("train", [])
            )
            model = distill_teacher_model(
                np, teacher, model_hidden_size, profile["actions"],
                calibration_records, runtime_tier, stop_event,
            )
        else:
            model = model_from_global_prior(np, prior, profile["actions"], model_hidden_size)
        changed = True
    if target_load_path.is_file():
        target_model, target_changed = load_model(
            np, target_load_path, MODEL_INPUT_DIM, model_hidden_size,
            len(profile["actions"]), profile["actions"],
        )
    else:
    ...
    if session_variant == "champion" and (changed or not active_path.is_file()):
        save_model(np, active_path, model)
    if session_variant == "champion" and (
        target_changed or not active_target_path.is_file()
    ):
        save_model(np, active_target_path, target_model)
    # Work on a disposable online candidate.  The validated runtime model is
    ...
    online_lr = (
        ...
        if online_ready and session_variant == "champion"
        else 0.0
    )
    online_td_discount = max(0.0, min(0.95, float(config["online_td_discount"])))
    potential_discount = max(0.1, min(0.99, float(config["planning_discount"])))
    ...
    run_id = f"ai-run-{time.time_ns():x}-{os.getpid():x}"
    episode_id = f"ai-{time.time_ns():x}-{os.getpid():x}"
    step = 0
    rows: list[tuple] = []
    online_queue = deque()
    replay_capacity = max(128, min(16384, int(runtime["online_replay_capacity"])))
    online_replay = deque(maxlen=replay_capacity)
    replay_dataset = load_training_data(
        np,
        paths["db"],
        action_count,
        min(replay_capacity, 4096, int(config["train_sample_limit_per_game"])),
        stop_event,
        int(config["n_step_horizon"]),
        float(config["validation_episode_fraction"]),
        config,
    )
    for replay_record in list((replay_dataset or {}).get("train", [])):
        replay_sample = _training_record_replay_sample(replay_record)
        if replay_sample is not None:
            online_replay.append(replay_sample)
    active_skill = deque()
    ...
    model_changed = bool(changed or target_changed)
    ...
    try:
        while not stop_event.is_set():
            ...
            if replacement:
                ...
                if rows:
                    ...
                    mark_episode_boundary(
                        paths["db"], episode_id, truncated=True
                    )
                sampler.close()
                target = replacement
                sampler = ScreenSampler(target, runtime_tier)
            ...
            if foreground_window() != target:
                release_all_inputs()
                _flush_online_n_step(
                    np, online_queue, model, target_model, online_lr, horizon,
                    online_td_discount, online_replay,
                    int(runtime["online_replay_batch_size"]),
                    int(config["online_replay_interval"]), True, config,
                )
                if rows:
                    last = list(rows[-1])
                    last[7] = 1
            ...
            if selected_base is not None and 0 <= selected_base < action_count and selected_base not in blocked_action_ids:
                ...
                generated_candidates = autoregressive_action_beam(
                    np,
                    model,
                    model_hidden,
                    profile["actions"],
                    ...
                    critic_weights=config,
                )
                generated_candidates = [
                    candidate
                    for candidate in generated_candidates
                    if not target_action_blocked(
                        candidate["action"], "generated", executable_name
                    )
                ]
                if generated_candidates:
                    if random.random() < min(0.35, exploration):
                        candidate_scores = np.asarray(
                            [float(item["final_action_score"]) for item in generated_candidates],
                            dtype=np.float64,
                        )
                        candidate_scores -= float(candidate_scores.max(initial=0.0))
                        candidate_weights = np.exp(np.clip(candidate_scores, -30.0, 0.0))
                        candidate_weights /= max(1e-12, float(candidate_weights.sum()))
                        selected_generated = generated_candidates[
                            int(np.random.choice(len(generated_candidates), p=candidate_weights))
                        ]
                    else:
                        selected_generated = generated_candidates[0]
                    action_id = int(selected_generated["memorized_action_id"])
                    generated_duration = int(selected_generated["duration_index"])
                else:
            ...
            elif selected_skill_duration is not None:
            ...
            # The database stores a catalog id, so execution must use that same
            # catalog control until structured actions are persisted end-to-end.
            execution_action = normalized_action(profile["actions"][base_action])
            ...
            if black:
                ...
                online_progress_delta = (
                    potential_discount * predict_progress(np, model, next_hidden_preview)
                ...
                potential_confidence = progress_confidence * progress_agreement
                ...
                learned_progress_delta = progress_head_delta
                learned_reward_signals = {
                    ...
                    "confidence": float(potential_confidence),
                    "potential_confidence": float(potential_confidence),
                    "outcome_confidence": float(learned_reward_confidence),
                }
                ...
                reward = compose_reward(metrics, config)
                terminated = 1 if float(metrics.get("confirmed_death", 0.0)) > 0.5 or float(metrics.get("confirmed_victory", 0.0)) > 0.5 else 0
                truncated = 0
                capture_failed = 0
                done = terminated
            ...
            if not capture_failed:
                ...
                score_confidence = max(
                    0.0,
                    min(1.0, float(metrics.get("score_delta_confidence", 0.0))),
                )
                live_score_total += (
                    float(metrics.get("score_delta", 0.0)) * score_confidence
                )
                if float(metrics.get("level_transition", 0.0)) > 0.5:
                    live_levels += 1
                if terminated:
                    live_completed_episodes += 1
            ...
            rows.append(
                (
                    ...
                    *transition_outcome_labels(metrics),
                    run_id,
                    terminated,
                    truncated,
                    capture_failed,
            ...
            online_queue.append(
                {
                    ...
                    "next_state": next_state,
                    "done": bool(done),
                    "hidden_state": model_hidden.copy(),
                    "target_hidden_state": target_hidden.copy(),
                    "next_hidden_state": next_hidden_preview.copy(),
                    "target_next_hidden_state": target_next_hidden_preview.copy(),
            ...
            if model_changed and steps % checkpoint == 0:
                ...
                save_model(np, candidate_path, model)
                save_model(np, candidate_target_path, target_model)
                model_changed = False
        ...
        if rows:
            last = list(rows[-1])
            if not any(int(value) for value in last[16:19]):
                last[7] = 1
                last[16:19] = [0, 1, 0]
            rows[-1] = tuple(last)
            insert_transitions(paths["db"], rows)
        else:
            mark_episode_boundary(paths["db"], episode_id, truncated=True)
        save_state_value_memory(
            paths["db"],
            state_memory,
            dirty_state_values,
        ...
        if steps:
            ...
            record_live_model_session(
                profile,
                runtime_tier,
                session_variant,
                {
                    "episodes": max(1, live_completed_episodes),
                    "victories": live_victories,
                    "deaths": live_deaths,
                    "levels": live_levels,
            ...
            if (
                session_variant == "challenger"
                and live_challenger_passes(
                    profile,
                    runtime_tier,
    ...
    stage = (
        (
            "真实会话 challenger 已晋升为 champion"
            if promoted
            else f"真实会话 {session_variant} 评测已记录，等待跨会话门控"
        )
        if online_ready
