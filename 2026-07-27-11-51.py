...
UNIVERSAL_ACTION_SCHEMA = 3
UNIVERSAL_ACTION_LIMIT = 112
...
def load_config() -> dict:
    try:
        ...
        if isinstance(raw, dict) and raw.get("schema") in (3, 4, 5, CONFIG_SCHEMA):
...
def load_index() -> dict:
    try:
        ...
        if isinstance(raw, dict) and raw.get("schema") in (3, 4, 5, APP_SCHEMA) and isinstance(raw.get("profiles"), dict):
            profiles = {
                profile_id: metadata
...
def universal_actions() -> list[dict]:
    ...
    for key in (0x20, 0x0D, 0x10, 0x11, 0x09, 0x45, 0x46, 0x51, 0x52, 0x54, 0x47, 0x5A, 0x58, 0x43, 0x56, 0x42, 0x4E, 0x4D):
        raw_actions.append({"keys": [key]})
    for key in range(0x31, 0x37):
        raw_actions.append({"keys": [key]})
    ...
    for movement in ([0x57], [0x41], [0x53], [0x44]):
        raw_actions.append({"keys": movement + [0x45]})
    for key in range(0x70, 0x76):
        raw_actions.append({"keys": [key]})
    ...
    raw_actions.extend(({"mouse_wheel": 1}, {"mouse_wheel": -1}))
    actions: list[dict] = []
...
def action_policy_bias(
    ...
    if cold_start and step < 36:
        if kind == "keyboard" and keys & movement_keys:
            bias += 0.12
...
def migrate_profile(data: object, profile_id: str) -> dict | None:
    if not isinstance(data, dict) or data.get("id") != profile_id:
        return None
    if data.get("schema") not in (2, 3, 4, 5, 6, PROFILE_SCHEMA):
        return None
    ...
    try:
        while not stop_event.is_set():
            ...
            if idle and feature_motion(feature) < 0.004:
                ...
                idle_streak = 0
            keep_sample = (idle_streak <= 2 or idle_streak % 6 == 0) and not black
            if keep_sample:
        ...
        while not stop_event.is_set():
            ...
            if len(reward_prior) == len(policy_values):
                ...
                for action_position, action in enumerate(profile["actions"]):
                    ...
                    state_action_key = (current_digest, action_position)
            ...
            repeated_state_action = state_action_visits.get((current_digest, action_index), 0)
            ...
            reward_sum += reward
            state_action_key = (current_digest, action_index)
