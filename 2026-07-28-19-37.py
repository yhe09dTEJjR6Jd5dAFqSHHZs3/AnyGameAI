...
def ensure_numpy(download: bool, stop_event: threading.Event | None = None) -> bool:
    ...
    try:
        ...
        if code != 0:
            log_text("运行组件安装失败:\n" + output[-12000:])
            raise RuntimeError("运行组件下载或安装失败；系统 Python 不会被修改，详情已写入日志。")
        probe_code, probe_output = run_process_cancelable(
...
class AnyGameAIApp:
    ...
    def upgrade_mode(self) -> None:
        self.run_worker("正在升级策略、动作价值、动作序列、场景动作记忆和跨游戏通用先验，并整理经验池；ESC 可取消", lambda: train_all_profiles(self.stop_event))
    ...
    def ai_mode(self) -> None:
        ...
        self.run_worker("请切换到游戏窗口；AI 将自动使用游戏模型、动作序列、场景动作记忆、在线适应、跨游戏迁移或通用探索；ESC 结束", work, hide=True)
...
def main() -> None:
    ...
    if os.name != "nt":
        ...
        return
    if sys.maxsize <= 2**32 or platform.machine().strip().lower() not in {"amd64", "x86_64"}:
        show_startup_error("需要 x64 版 Python。")
        return
    if sys.version_info[:2] != (3, 12) or platform.python_implementation() != "CPython":
        show_startup_error("需要 CPython 3.12 x64。")
        return
