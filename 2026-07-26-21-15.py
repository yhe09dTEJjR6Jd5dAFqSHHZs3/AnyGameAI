...
def load_index() -> dict:
    try:
        ...
        if isinstance(raw, dict) and raw.get("schema") in (3, APP_SCHEMA) and isinstance(raw.get("profiles"), dict):
            profiles = {
                profile_id: metadata
...
def default_profile(identity: dict) -> dict:
    return {
        ...
        "action_hold_seconds": [float(DEFAULT_CONFIG["action_hold_seconds"])],
        "action_duration_counts": [0],
        "transitions": {},
        "trained_samples": 0,
        "training_rounds": 0,
        "human_sessions": 0,
...
def migrate_profile(data: object, profile_id: str) -> dict | None:
    if not isinstance(data, dict) or data.get("id") != profile_id:
        return None
    if data.get("schema") not in (2, 3, PROFILE_SCHEMA):
        return None
...
def feature_vector(np, feature: bytes):
    ...
    centered = image - 0.5
    local = image - float(image.mean())
    appearance = centered * 0.7 + local * 0.3
    ...
    edges = np.minimum(1.0, (horizontal + vertical) * 1.5)
    return np.concatenate((appearance.reshape(-1), difference.reshape(-1), edges.reshape(-1))).astype(np.float32, copy=False)
...
def load_training_data(np, db_path: Path, action_count: int, sample_limit: int, stop_event: threading.Event | None):
    ...
    human_capacity = max(180, int(per_action * 0.80))
    ai_capacity = max(48, per_action - human_capacity)
    buckets: dict[tuple[str, int], list] = {}
    seen: dict[tuple[str, int], int] = {}
    invalid = 0
    connection = sqlite3.connect(db_path, timeout=30)
    try:
        cursor = connection.execute("SELECT source, action, reward, feature_dim, feature FROM samples ORDER BY id")
    ...
    for index, (feature, action, reward, source) in enumerate(samples):
        ...
        if source == "human":
            ...
            value_targets[index] = 0.75
            value_weights[index] = 0.18
        else:
            bounded_reward = max(-1.0, min(1.0, reward))
            policy_weights[index] = class_weight * min(0.38, max(0.01, 0.02 + max(0.0, bounded_reward) * 0.30))
...
def train_model(
    ...
    for _ in range(max(1, epochs)):
        ...
        for start in range(0, len(train_indices), max(8, batch_size)):
            ...
            last_loss = float((-np.log(chosen + 1e-8) * batch_policy_weights).sum() / policy_total)
            ...
            last_value_loss = float(((value_errors * value_errors) * batch_value_weights).sum() / value_total)
    ...
    hidden2 = np.maximum(0.0, hidden1 @ model["W2"] + model["b2"])
    predictions = np.argmax(hidden2 @ model["Wp"] + model["bp"], axis=1)
    accuracy = float((predictions == eval_y).mean())
    eval_values = np.tanh(hidden2 @ model["Wv"] + model["bv"])
    selected_values = eval_values[np.arange(len(evaluation_indices)), eval_y]
    value_mae = float(np.abs(selected_values - value_targets[evaluation_indices]).mean())
...
def run_ai_session(target: int, stop_event: threading.Event) -> str:
    ...
    try:
        while not stop_event.is_set():
            ...
            novel = digest not in seen_frames
            looped = digest in recent_frame_hashes
            if motion < 0.006:
                static_streak += 1
            else:
                static_streak = 0
            motion_reward = min(0.85, motion * 7.0)
            novelty_reward = 0.16 if novel else 0.0
            loop_penalty = 0.22 if looped else 0.0
            static_penalty = min(0.62, static_streak * 0.04) if action_index != 0 else min(0.28, static_streak * 0.02)
            black_penalty = 0.70 if black else 0.0
            reward = max(-1.0, min(1.0, motion_reward + novelty_reward - loop_penalty - static_penalty - black_penalty))
        ...
        sampler.close()
        if steps:
            profile["ai_sessions"] = int(profile.get("ai_sessions", 0)) + 1
            save_profile(profile, paths)
        compact_experience(paths["db"], int(config["experience_limit_per_game"]), len(profile["actions"]))
        wait_esc_release()
...
class AnyGameAIApp:
    ...
    def upgrade_mode(self) -> None:
        self.run_worker("正在训练策略、动作价值并整理所有游戏经验池；ESC 可取消", lambda: train_all_profiles(self.stop_event))
