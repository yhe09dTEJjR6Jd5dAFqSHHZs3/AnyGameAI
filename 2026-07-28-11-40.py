...
def local_numpy_probe_command(site_packages: Path = SITE_PACKAGES) -> list[str]:
    code = (
        ...
        "parts=tuple(int(x) for x in __import__('re').match(r'\\s*(\\d+)(?:\\.(\\d+))?(?:\\.(\\d+))?',n.__version__).groups(default='0'));"
        "assert (1,26,0)<=parts<(3,0,0);"
...
def target_action_blocked(action: dict, origin: str, executable_name: str) -> bool:
    ...
    if origin != "human" and keys and keys.issubset(modifier_keys):
        return True
    if {0x12, 0x73}.issubset(keys) or {0x11, 0x12, 0x2E}.issubset(keys):
        return True
    ...
    try:
        ...
        for sample_id, source, action, reward, feature_dim, blob in cursor:
            ...
            reservoir_add(
                ...
                (feature, action_index, reward_value, source),
                seen[key],
                capacity,
            )
    finally:
...
def runtime_self_check(np) -> None:
    ...
    if not target_action_blocked({"keys": [0x10]}, "generic", "game"):
        raise RuntimeError("独立修饰键隔离自检失败")
...
def run_ai_session(target: int, stop_event: threading.Event) -> str:
    ...
    try:
        while not stop_event.is_set():
            ...
            persistent_state_visits = sum(
                min(256, int(persistent_state_values.get((current_memory_state, action_position), (0.0, 0))[1]))
                for action_position in range(len(profile["actions"]))
            )
