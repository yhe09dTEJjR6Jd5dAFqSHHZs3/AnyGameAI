...
REQUIRED_SCRIPT_FUNCTIONS = frozenset({
    ...
    "run_ai_session", "runtime_self_check", "repair_main_script",
})
...
def adaptive_training_settings(
    ...
    gib = 1024 ** 3
    if memory and memory < gib:
        sample_limit = min(sample_limit, 6000)
        epochs = min(epochs, 6)
        batch_size = min(batch_size, 64)
    elif memory and memory < 2 * gib:
...
DEFAULT_CONFIG = {
    ...
    "planning_weight": 0.24,
    "planning_refresh_steps": 64,
...
def script_metadata(path: Path) -> dict | None:
    try:
        ...
        app_methods: set[str] = set()
        has_main_entry = False
        for node in tree.body:
            if isinstance(node, ast.Assign) and isinstance(node.value, ast.Constant) and isinstance(node.value.value, str):
                for target in node.targets:
                    if isinstance(target, ast.Name) and target.id in {"APP_NAME", "APP_VERSION", "SCRIPT_NAME"}:
        ...
        if (
            ...
            or not REQUIRED_SCRIPT_FUNCTIONS.issubset(top_level_functions)
            or not REQUIRED_APP_METHODS.issubset(app_methods)
    ...
    except (OSError, UnicodeError, SyntaxError, ValueError, OverflowError):
        return None
...
def load_integrity_state() -> dict:
    try:
        ...
        if (
            ...
            and schema in (1, 2, INTEGRITY_SCHEMA)
    ...
    try:
        with LOG_LOCK:
            ...
            if LOG_PATH.is_file() and LOG_PATH.stat().st_size > MAX_LOG_BYTES:
                ...
                temp = LOG_PATH.with_name(LOG_PATH.name + ".tmp")
...
def configure_runtime_environment() -> None:
    ...
    numeric_threads = max(1, min(4, cpu_count // 2 if cpu_count > 2 else 1))
    for variable in ("OMP_NUM_THREADS", "OPENBLAS_NUM_THREADS", "MKL_NUM_THREADS", "NUMEXPR_NUM_THREADS"):
        os.environ.setdefault(variable, str(numeric_threads))
...
def verify_installed_distribution(
    ...
    with record_path.open("r", encoding="utf-8", newline="") as file:
        for row_number, row in enumerate(csv.reader(file), 1):
            ...
            relative_text = PurePosixPath(row[0]).as_posix()
            aggregate.update(relative_text.encode("utf-8", errors="strict"))
...
def load_runtime_integrity_state() -> dict:
    try:
        data = json.loads(RUNTIME_INTEGRITY_PATH.read_text(encoding="utf-8"))
        if (
            isinstance(data, dict)
...
def runtime_integrity_matches(snapshot: dict, state: dict) -> bool:
    return all(
        ...
        for key in ("distribution", "version", "file_count", "record_sha256", "content_sha256")
    )
...
def select_numpy_wheel(index: dict) -> dict:
    ...
    for version_text, files in releases.items():
        ...
        for item in files:
            ...
            if not (
                lower.endswith("-cp312-cp312-win_amd64.whl")
                or lower.endswith("-cp312-abi3-win_amd64.whl")
            ):
                continue
    ...
    try:
        with zipfile.ZipFile(wheel_path, "r") as archive:
            ...
            for member in members:
                ...
                if mode == 0o120000:
                    raise RuntimeError("运行组件压缩包包含符号链接")
    ...
    if download:
        ...
        if probe_code == 0:
            try:
                ...
                if state and not runtime_integrity_matches(snapshot, state):
...
def ensure_runtime_ready(stop_event: threading.Event | None = None):
    ...
    try:
        ...
        if str(error) == "操作已取消":
            raise
        raise RuntimeError("本地运行组件缺失或损坏，请先重新运行程序并点击“文件”。") from error
    np = import_numpy()
...
def refresh_global_prior(np, index: dict, config: dict, stop_event: threading.Event | None) -> bool:
    ...
    base_sample_limit = int(training_settings["sample_limit"])
    global_sample_limit = min(60_000, max(4_000, base_sample_limit * 2))
    per_profile_limit = max(256, min(base_sample_limit, math.ceil(global_sample_limit / len(candidates))))
    x_parts = []
    y_parts = []
    policy_parts = []
    value_target_parts = []
    ...
    metrics = train_model(
        ...
        value_targets,
        value_weights,
        max(2, min(int(training_settings["epochs"]), max(3, int(training_settings["epochs"]) // 2))),
...
def runtime_self_check(np) -> None:
    ...
    high_training = adaptive_training_settings(DEFAULT_CONFIG, 16 * 1024 ** 3, 16)
    if (
    ...
    idle_signal = human_training_signal(0.08)
    active_signal = human_training_signal(0.90)
    if (
    ...
    try:
        wheel_member_destination(APP_DIR, "../escape")
    except RuntimeError:
        pass
    else:
    ...
    try:
        while not stop_event.is_set():
            ...
            if idle and feature_motion(feature) < 0.004:
                ...
                idle_streak = 0
            demonstration_reward = 0.08 if idle else min(1.0, 0.72 + human_effect * 0.28)
            if not black:
                update_action_reward(profile, action_index, demonstration_reward)
                update_action_effect(profile, action_index, human_effect)
                update_control_response(profile, action_index, demonstration_reward)
                update_scene_control_response(profile, scene_context_name, action_index, demonstration_reward)
                update_scene_action_response(profile, scene_context_name, action_index, demonstration_reward)
...
def run_ai_session(target: int, stop_event: threading.Event) -> str:
    ...
    planning_horizon = max(1, min(6, int(config["planning_horizon"])))
    ...
    planning_refresh_steps = max(8, min(512, int(config["planning_refresh_steps"])))
    ...
    translation_search_radius = max(1, min(4, int(config["translation_search_radius"])))
...
class AnyGameAIApp:
    ...
    def run_worker(self, status: str, worker, hide: bool = False) -> None:
        ...
        threading.Thread(target=task, daemon=True, name="AnyGameAI-Worker").start()
    ...
    def ai_mode(self) -> None:
        ...
        self.run_worker(
            "请切换到游戏窗口；AI 将自动使用游戏模型、多步动作规划、场景动作记忆、在线适应、跨游戏迁移或通用探索；ESC 结束",
            work,
            hide=True,
        )
...
def supported_host_error() -> str | None:
    ...
    if sys.maxsize <= 2**32 or machine not in SUPPORTED_X64_MACHINES:
        return "需要 x64 版 Python。"
    if (
