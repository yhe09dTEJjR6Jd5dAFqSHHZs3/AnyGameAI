...
def load_config():
    ...
    data["最近文件"] = str(data.get("最近文件", "")).strip()
    data["升级清单地址"] = str(data.get("升级清单地址", "")).strip()
    data["升级超时秒数"] = clamp_float(data.get("升级超时秒数"), 3.0, 120.0, 20.0)
    data["保留备份数量"] = clamp_int(data.get("保留备份数量"), 1, 100, 10)
...
class LearningAgent:
    ...
    def upgrade_storage(self, stop_event=None):
        ensure_directories()
        if stop_event is not None and stop_event.is_set():
            raise InterruptedError("升级已取消")
        self.flush()
        stamp = time.strftime("%Y%m%d-%H%M%S")
        backup_path = BACKUP_DIR / f"agent-{stamp}.sqlite3"
        ...
        maintenance = sqlite3.connect(DATABASE_PATH, timeout=30.0)
        try:
            maintenance.execute("PRAGMA wal_checkpoint(TRUNCATE)")
            maintenance.execute("ANALYZE")
            maintenance.execute("PRAGMA optimize")
            maintenance.commit()
        finally:
            maintenance.close()
        keep = self.config["保留备份数量"]
        backups = sorted(BACKUP_DIR.glob("agent-*.sqlite3"), key=lambda item: item.stat().st_mtime, reverse=True)
...
class AnyGameAI:
    def __init__(self, root, config):
        ...
        self.detail_var = tk.StringVar(
            value=f"文件：启动；人：示范；升级：维护与更新；AI：学习运行；{config['正反馈键']}/{config['负反馈键']}：反馈；ESC：结束")
        self.build_ui()
    ...
    def build_ui(self):
        ...
        ttk.Label(frame, textvariable=self.detail_var, anchor="center", justify="center", wraplength=500).pack(pady=(0, 18))
        buttons = ttk.Frame(frame)
        buttons.pack()
        self.file_button = ttk.Button(buttons, text="文件", command=self.begin_file, width=15)
        self.file_button.grid(row=0, column=0, padx=10, pady=8, ipadx=4, ipady=14)
        self.human_button = ttk.Button(buttons, text="人", command=lambda: self.begin("人"), width=15)
        self.human_button.grid(row=0, column=1, padx=10, pady=8, ipadx=4, ipady=14)
        self.upgrade_button = ttk.Button(buttons, text="升级", command=self.begin_upgrade, width=15)
        self.upgrade_button.grid(row=1, column=0, padx=10, pady=8, ipadx=4, ipady=14)
        self.ai_button = ttk.Button(buttons, text="AI", command=lambda: self.begin("AI"), width=15)
        self.ai_button.grid(row=1, column=1, padx=10, pady=8, ipadx=4, ipady=14)
        ttk.Label(frame, textvariable=self.status_var, font=("Microsoft YaHei UI", 10), wraplength=510).pack(pady=(18, 0))
    ...
    def set_buttons(self, enabled):
        ...
        self.upgrade_button.configure(state=state)
        self.ai_button.configure(state=state)

    def new_session(self, mode):
    ...
    def begin_file(self):
        if self.active_session is not None:
            return
        recent_value = str(self.config.get("最近文件", "")).strip()
        recent = Path(recent_value) if recent_value else None
        initial_dir = str(recent.parent) if recent is not None and recent.exists() else str(desktop_path())
        selected = filedialog.askopenfilename(
            title="选择要启动的游戏或文件",
            initialdir=initial_dir,
            filetypes=(("可启动文件", "*.exe *.lnk *.bat *.cmd *.com *.url *.py"), ("所有文件", "*.*")))
        if not selected:
            self.status_var.set("已取消文件选择")
            return
        session = self.new_session("文件")
        if session is None:
            return
        session.selected_file = selected
        self.config["最近文件"] = selected
        try:
            self.save_config()
        except OSError:
            write_log(traceback.format_exc())
        self.status_var.set(f"正在启动：{Path(selected).name}")
        self.root.withdraw()
        session.thread = threading.Thread(target=self.file_loop, args=(session,), daemon=True)
        session.thread.start()

    def file_loop(self, session):
        try:
            os.startfile(session.selected_file)
            session.end_reason = "文件模式已结束"
            while self.is_current(session) and not session.stop_event.wait(0.1):
                pass
        except Exception:
            write_log(traceback.format_exc())
            session.end_reason = "无法启动所选文件，详情见日志"
            session.end_error = True
        finally:
            self.events.put(("worker_finished", session.session_id))

    def begin_upgrade(self):
        session = self.new_session("升级")
        if session is None:
            return
        self.status_var.set("正在检查并升级…按 ESC 可结束")
        session.thread = threading.Thread(target=self.upgrade_loop, args=(session,), daemon=True)
        session.thread.start()
    ...
    def download_bytes(self, url, maximum):
        request = urllib.request.Request(url, headers={"User-Agent": f"{APP_NAME}/{APP_VERSION}"})
        with urllib.request.urlopen(request, timeout=self.config["升级超时秒数"]) as response:
            data = response.read(maximum + 1)
        if len(data) > maximum:
            raise ValueError("升级文件过大")
        return data

    def online_upgrade(self, session):
        manifest_url = self.config.get("升级清单地址", "").strip()
        if not manifest_url:
            return "未配置在线升级地址", False
        raw = self.download_bytes(manifest_url, 2 * 1024 * 1024)
        manifest = json.loads(raw.decode("utf-8-sig"))
        if not isinstance(manifest, dict):
            raise ValueError("升级清单格式无效")
        version = str(manifest.get("version", manifest.get("版本", ""))).strip()
        source_url = str(manifest.get("url", manifest.get("地址", ""))).strip()
        expected_hash = str(manifest.get("sha256", manifest.get("SHA256", ""))).strip().lower()
        if not version or not source_url or not re.fullmatch(r"[0-9a-f]{64}", expected_hash):
            raise ValueError("升级清单缺少版本、地址或 SHA256")
        if self.version_key(version) <= self.version_key(APP_VERSION):
            return f"当前已是最新版 {APP_VERSION}", False
        if session.stop_event.is_set():
            raise InterruptedError("升级已取消")
        source_url = urllib.parse.urljoin(manifest_url, source_url)
        data = self.download_bytes(source_url, 16 * 1024 * 1024)
        actual_hash = hashlib.sha256(data).hexdigest()
        if actual_hash != expected_hash:
            raise ValueError("升级文件 SHA256 校验失败")
        text = data.decode("utf-8-sig")
        compile(text, SCRIPT_NAME, "exec")
        target = WORK_DIR / SCRIPT_NAME
        temp_path = RUNTIME_DIR / f"{SCRIPT_NAME}.{uuid.uuid4().hex}.new"
        backup_path = BACKUP_DIR / f"{SCRIPT_NAME}.{APP_VERSION}.{time.strftime('%Y%m%d-%H%M%S')}.bak"
        temp_path.write_text(text, encoding="utf-8", newline="\n")
        shutil.copy2(target, backup_path)
        os.replace(temp_path, target)
        return f"程序已升级到 {version}", True

    def upgrade_loop(self, session):
        try:
            online_message, restart_required = self.online_upgrade(session)
            if session.stop_event.is_set():
                session.end_reason = "已按 ESC 结束"
                return
            backup_path, cancelled = self.agent.upgrade_storage(session.stop_event)
            if cancelled or session.stop_event.is_set():
                session.end_reason = "已按 ESC 结束；学习库备份已保存"
                return
            session.restart_required = restart_required
            session.end_reason = f"升级完成：{online_message}；学习库已备份并优化（{backup_path.name}）"
        except InterruptedError:
            session.end_reason = "已按 ESC 结束"
        except (OSError, ValueError, json.JSONDecodeError, urllib.error.URLError):
            write_log(traceback.format_exc())
            session.end_reason = "升级失败，详情见日志"
            session.end_error = True
        except Exception:
            write_log(traceback.format_exc())
            session.end_reason = "升级发生错误，详情见日志"
            session.end_error = True
        finally:
            self.events.put(("worker_finished", session.session_id))
    ...
    def finalize_session(self, session):
        ...
        self.detail_var.set(
            f"文件：启动；人：示范；升级：维护与更新；AI：学习运行；{self.config['正反馈键']}/{self.config['负反馈键']}：反馈；ESC：结束")
        ...
        elif session.restart_required:
            self.root.after(250, self.restart_after_upgrade)

    def restart_after_upgrade(self):
        target = WORK_DIR / SCRIPT_NAME
        try:
            subprocess.Popen([sys.executable, str(target), "--wait-pid", str(os.getpid())],
                             cwd=str(WORK_DIR), creationflags=getattr(subprocess, "CREATE_NEW_PROCESS_GROUP", 0))
        except OSError:
            write_log(traceback.format_exc())
            messagebox.showerror(APP_NAME, "升级已安装，但自动重启失败。请重新打开 AnyGameAI.py。")
            return
        self.close()
...
def wait_for_previous_instance():
    if len(sys.argv) < 3 or sys.argv[1] != "--wait-pid":
        return
    try:
