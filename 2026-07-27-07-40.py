...
MODEL_INPUT_DIM = FEATURE_WIDTH * FEATURE_HEIGHT * 3
...
UNIVERSAL_ACTION_SCHEMA = 1
UNIVERSAL_ACTION_LIMIT = 45
...
DEFAULT_CONFIG = {
    ...
    "zero_shot_exploration": 0.30,
    "transfer_action_limit": 24,
    "remote_manifest_url": "",
}
...
def load_config() -> dict:
    try:
        ...
        if isinstance(raw, dict) and raw.get("schema") in (3, CONFIG_SCHEMA):
...
def load_index() -> dict:
    try:
        ...
        if isinstance(raw, dict) and raw.get("schema") in (3, 4, APP_SCHEMA) and isinstance(raw.get("profiles"), dict):
            profiles = {
                profile_id: metadata
...
def distribution_record_path(distribution_name: str) -> Path:
...
def verify_installed_distribution(distribution_name: str, stop_event: threading.Event | None) -> int:
    root = SITE_PACKAGES.resolve()
    record_path = distribution_record_path(distribution_name)
    verified = 0
...
def ensure_numpy(download: bool, stop_event: threading.Event | None = None) -> bool:
    if download:
        ...
        if probe_code == 0:
            try:
                verify_installed_distribution("numpy", stop_event)
                return False
...
def universal_actions() -> list[dict]:
    raw_actions = [
        {},
        {"keys": [0x57]}, {"keys": [0x41]}, {"keys": [0x53]}, {"keys": [0x44]},
        {"keys": [0x26]}, {"keys": [0x25]}, {"keys": [0x28]}, {"keys": [0x27]},
        {"keys": [0x57, 0x41]}, {"keys": [0x57, 0x44]},
        {"keys": [0x53, 0x41]}, {"keys": [0x53, 0x44]},
        {"keys": [0x26, 0x25]}, {"keys": [0x26, 0x27]},
        {"keys": [0x28, 0x25]}, {"keys": [0x28, 0x27]},
        {"keys": [0x20]}, {"keys": [0x0D]}, {"keys": [0x10]}, {"keys": [0x11]},
        {"keys": [0x45]}, {"keys": [0x46]}, {"keys": [0x51]}, {"keys": [0x52]},
        {"keys": [0x5A]}, {"keys": [0x58]}, {"keys": [0x43]},
        {"keys": [0x57, 0x10]}, {"keys": [0x41, 0x10]},
        {"keys": [0x53, 0x10]}, {"keys": [0x44, 0x10]},
        {"mouse_dx": -1}, {"mouse_dx": 1}, {"mouse_dy": -1}, {"mouse_dy": 1},
        {"mouse_dx": -1, "mouse_dy": -1}, {"mouse_dx": 1, "mouse_dy": -1},
        {"mouse_dx": -1, "mouse_dy": 1}, {"mouse_dx": 1, "mouse_dy": 1},
        {"buttons": ["left"], "mouse_x": 16, "mouse_y": 9},
        {"buttons": ["left"], "mouse_x": 8, "mouse_y": 9},
        {"buttons": ["left"], "mouse_x": 24, "mouse_y": 9},
        {"buttons": ["left"], "mouse_x": 16, "mouse_y": 5},
        {"buttons": ["left"], "mouse_x": 16, "mouse_y": 13},
    ]
    actions: list[dict] = []
...
def migrate_profile(data: object, profile_id: str) -> dict | None:
    if not isinstance(data, dict) or data.get("id") != profile_id:
        return None
    if data.get("schema") not in (2, 3, 4, PROFILE_SCHEMA):
        return None
...
def feature_vector(np, feature: bytes):
    ...
    return np.concatenate((appearance.reshape(-1), temporal.reshape(-1), edges.reshape(-1))).astype(np.float32, copy=False)
...
def load_model(np, path: Path, input_dim: int, hidden_size: int, output_size: int, action_list: list[dict]) -> tuple[dict, bool]:
    ...
    try:
        with np.load(path, allow_pickle=False) as data:
            ...
            if loaded["input_dim"] != input_dim or loaded["hidden_size"] != hidden_size or loaded["hidden2_size"] != hidden2:
    ...
    if model["output_size"] != output_size:
        ...
        for weight_key, bias_key, scale in (("Wp", "bp", 1.0), ("Wv", "bv", 0.35)):
            expanded_weight = np.random.standard_normal((hidden2, output_size)).astype(np.float32) * (((2.0 / hidden2) ** 0.5) * scale)
            expanded_bias = np.zeros(output_size, dtype=np.float32)
            copy_size = min(old_size, output_size)
            expanded_weight[:, :copy_size] = model[weight_key][:, :copy_size]
            expanded_bias[:copy_size] = model[bias_key][:copy_size]
            model[weight_key] = expanded_weight
            model[bias_key] = expanded_bias
...
def load_global_prior(np) -> dict | None:
    try:
        ...
        if prior["schema"] != GLOBAL_PRIOR_SCHEMA:
            raise ValueError("通用先验版本不兼容")
        input_dim = prior["input_dim"]
        ...
        if input_dim != MODEL_INPUT_DIM or hidden2 != second_hidden_size(hidden_size):
...
def run_ai_session(target: int, stop_event: threading.Event) -> str:
    ...
    try:
        while not stop_event.is_set():
            ...
            policy_values = np.asarray(smoothed_values, dtype=np.float64)
            if len(reward_prior) == len(policy_values):
                uncertainty_bonus = 0.20 / np.sqrt(reward_counts + 1.0)
                if len(uncertainty_bonus):
                    uncertainty_bonus[0] = 0.0
                origin_bonus = np.zeros(len(policy_values), dtype=np.float64)
                for origin_index, origin in enumerate(profile.get("action_origins", [])):
                    if origin_index >= len(origin_bonus):
                        break
                    if origin == "human":
                        origin_bonus[origin_index] = 0.10
                    elif origin == "transfer":
                        origin_bonus[origin_index] = 0.04
                policy_values = np.clip(
                    policy_values + 0.26 * reward_prior + uncertainty_bonus + origin_bonus,
                    -1.0,
                    1.0,
                )
            ...
            visit_count = state_visits.get(digest, 0)
            looped = digest in recent_frame_hashes
            if useful_motion < 0.0045 and changed_ratio < 0.025:
                static_streak += 1
            else:
                static_streak = 0
            passive_motion = min(useful_motion, scene_motion * 0.60)
            causal_motion = max(0.0, useful_motion - passive_motion)
            motion_reward = min(0.58, causal_motion * 10.0)
            change_reward = min(0.16, changed_ratio * 0.32) if causal_motion >= 0.0015 else 0.0
            novel_state = visit_count == 0 and not looped
            novelty_reward = (0.22 / math.sqrt(visit_count + 1.0)) if useful_motion >= 0.0025 else 0.0
            progress_reward = 0.14 if novel_state and causal_motion >= 0.004 and changed_ratio >= 0.02 else 0.0
            loop_penalty = 0.26 if looped else 0.0
            ...
            reward_sum += reward
            reward_count = int(profile["action_reward_counts"][action_index])
            old_reward = float(profile["action_reward_ema"][action_index])
            reward_alpha = max(0.025, 1.0 / min(40, reward_count + 1))
            profile["action_reward_ema"][action_index] = old_reward + reward_alpha * (reward - old_reward)
            profile["action_reward_counts"][action_index] = min(1_000_000_000, reward_count + 1)
            rows.append((now_text(), "ai", action_index, float(reward), FEATURE_DIM, compress_feature(feature)))
            if len(rows) >= 100:
                insert_samples(paths["db"], rows)
                rows.clear()
            state_visits[digest] = visit_count + 1
            if len(state_visits) > 8192:
                retained = {key: state_visits.get(key, 1) for key in recent_frame_hashes[-128:]}
                state_visits.clear()
