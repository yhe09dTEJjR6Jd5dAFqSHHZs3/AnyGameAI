...
RELEASE_SOURCE_SHA256 = "ac10567836a9c3a892fbb41fa87539645035a290458095a1cbf68999a8f671fd"
...
FEATURE_DIM = FEATURE_WIDTH * FEATURE_HEIGHT * 3 + COLOR_FEATURE_DIM
MODEL_PIXEL_CHANNELS = 6
MODEL_GLOBAL_FEATURES = 16
...
ONLINE_REPLAY_CAPACITY_DEFAULT = 512
ONLINE_REPLAY_BATCH_DEFAULT = 16
...
REQUIRED_SCRIPT_FUNCTIONS = frozenset({
    ...
    "stable_visual_state_key", "balanced_trajectory_records",
...
REQUIRED_SCRIPT_CALLS = {
    ...
    "run_ai_session": frozenset({"ensure_runtime_ready", "profile_identity", "execute_action", "temporal_policy_blend", "load_human_action_memory", "human_action_memory_biases", "soft_update_target_model", "recurrent_ensemble_outputs", "adaptive_exploration_rate", "adaptive_runtime_settings", "load_transition_graph", "load_state_value_memory", "infer_scene_context", "action_safety_penalty", "target_action_blocked", "choose_recovery_action", "insert_transitions", "save_model", "compose_reward"}),
    "train_model": frozenset({"sequence_training_windows", "evaluate_model_records", "soft_update_target_model"}),
    "classify_transition_reward": frozenset({"compose_reward"}),
    "load_training_data": frozenset({"decode_temporal_state", "compose_reward"}),
...
def adaptive_runtime_settings(
    ...
    gib = 1024 ** 3
    tier = hardware_capability_tier(memory, processors)
    ...
    planning_horizon = max(2, min(8, int(config.get("planning_horizon", 4))))
    ...
    if tier == "low_numpy":
        ...
        planning_horizon = max(planning_horizon, 6)
    ...
    if tier == "low_numpy":
        model_hidden_size, sequence_length, burn_in_steps = 192, 48, 12
    elif tier == "high_directml":
        model_hidden_size, sequence_length, burn_in_steps = 768, 128, 32
    else:
        model_hidden_size, sequence_length, burn_in_steps = 384, 64, 16
    return {
        "hardware_tier": tier,
        ...
        "planning_horizon": max(2, min(8, planning_horizon)),
...
CONFIG_RANGES = {
    ...
    "online_replay_capacity": (32, 8192),
    "online_replay_batch_size": (4, 128),
...
def profile_paths(profile_id: str, repair_unsafe: bool = False) -> dict[str, Path]:
    ...
    paths = {
        ...
        "best_model": root / "best_model.npz",
        "candidate_model": root / "candidate_model.npz",
        "target_model": root / "target_model.npz",
        "best_target_model": root / "best_target_model.npz",
        "candidate_target_model": root / "candidate_target_model.npz",
    }
    managed_files = (
...
def conditional_duration_outputs(np, model: dict, hidden, control_kind: int):
    ...
    logits = (
        np.asarray(hidden, dtype=np.float32) @ model["policy_duration_w"]
        + model["policy_duration_b"]
        + model["policy_duration_kind_b"][kind]
    )
    probabilities = _softmax_vector(np, logits).astype(np.float32)
    q_values = (
        np.einsum("h,vhd->vd", np.asarray(hidden, dtype=np.float32), model["q_duration_w"])
        + model["q_duration_b"]
...
def select_conditional_duration(np, model: dict, hidden, base_action: int, exploration: float = 0.0) -> int:
    ...
    score = np.log(np.maximum(probabilities, 1e-9)) + 0.35 * q_values.mean(axis=0)
...
def decode_temporal_state(state: bytes) -> tuple[list[bytes], list[int], list[float]]:
    ...
    for _ in range(frame_count):
        ...
        offset += 4
        if length not in (LEGACY_FEATURE_DIM, V27_FEATURE_DIM, V69_FEATURE_DIM, FEATURE_DIM) or offset + length > len(candidate):
...
def classify_transition_reward(
    ...
    death_signal = max(darkness, fade_score * 0.9)
    if mean_before > 0.20 and mean_after < 0.08:
        death_signal = max(death_signal, 0.85)
    menu_signal = min(1.0, max(0.0, global_shift - world_change * 1.8) * max(0.0, 1.0 - changed_ratio * 0.5) * 3.0)
    ...
    score_history = _append_signal_history(signal_history, "score", score_signal)
    ...
    confirmed_death = (
        1.0
        if len(death_history) >= 2 and death_history[-1] > 0.75 and death_history[-2] > 0.65
        else 0.0
    )
    ...
    persistent_event_reward = max(-1.0, min(1.0, confirmed_score - confirmed_menu))
    ...
    explicit_score_signal = max(0.0, min(1.0, confirmed_score))
    death_penalty = max(0.0, min(1.0, max(confirmed_death, death_signal * 0.35)))

    task_reward = (
        0.45 * human_progress
        + 0.30 * persistent_event_reward
        + 0.20 * explicit_score_signal
        + 0.05 * learned_progress_value
    )
    exploration_reward = 0.10 * controllable_novelty_reward
    safety_penalty = (
        0.70 * death_penalty
        + 0.20 * cycle_penalty
        + 0.10 * jitter_signal
    ...
    metrics = {
        ...
        "explicit_score": float(explicit_score_signal),
        ...
        "persistent_event": float(persistent_event_reward),
...
def normalize_feature_bytes(feature: bytes) -> bytes:
    ...
    if len(feature) == V69_FEATURE_DIM:
        ...
        return gray + difference + signed + blue + red
    if len(feature) == V27_FEATURE_DIM:
        ...
        return gray + difference + signed + gray + gray
...
def _frame_channels(np, feature: bytes):
    feature = normalize_feature_bytes(feature)
    pixels = FEATURE_WIDTH * FEATURE_HEIGHT
    raw = np.frombuffer(feature, dtype=np.uint8).astype(np.float32)
...
def _quantized_conv_features(np, model: dict, frame, return_cache: bool = False, training: bool = False):
    ...
    quantized = np.clip(np.rint(frame / 0.01) + 128.0, 0, 255).astype(np.uint8)
    ...
    if return_cache:
        ...
        return pooled, {"stem": stem_cache, "pyramid": pyramid_cache}
    return _visual_pyramid_features(np, model, convolved)
...
def feature_vector(np, feature: bytes):
    ...
    return {
        "frames": np.stack([_frame_channels(np, frame) for frame in frames], axis=0),
...
def initialize_model(np, input_dim: int, hidden_size: int, output_size: int) -> dict:
    ...
    model = {
        ...
        "q_control_w": weight((VALUE_HEAD_COUNT, hidden, len(CONTROL_KINDS)), 0.025),
        "q_control_b": np.zeros((VALUE_HEAD_COUNT, len(CONTROL_KINDS)), dtype=np.float32),
        "q_key_w": weight((VALUE_HEAD_COUNT, hidden, KEY_HEAD_SIZE), 0.02),
        "q_key_b": np.zeros((VALUE_HEAD_COUNT, KEY_HEAD_SIZE), dtype=np.float32),
        "q_mouse_w": weight((VALUE_HEAD_COUNT, hidden, MOUSE_HEAD_SIZE), 0.02),
        "q_mouse_b": np.zeros((VALUE_HEAD_COUNT, MOUSE_HEAD_SIZE), dtype=np.float32),
        "q_button_w": weight((VALUE_HEAD_COUNT, hidden, BUTTON_HEAD_SIZE), 0.02),
        "q_button_b": np.zeros((VALUE_HEAD_COUNT, BUTTON_HEAD_SIZE), dtype=np.float32),
        "q_duration_w": weight((VALUE_HEAD_COUNT, hidden, DURATION_HEAD_SIZE), 0.025),
        "q_duration_b": np.zeros((VALUE_HEAD_COUNT, DURATION_HEAD_SIZE), dtype=np.float32),
        "q_duration_kind_b": np.zeros((VALUE_HEAD_COUNT, len(CONTROL_KINDS), DURATION_HEAD_SIZE), dtype=np.float32),
        "q_action_w": weight((VALUE_HEAD_COUNT, hidden, action_dim), 0.015),
        "q_action_b": np.zeros((VALUE_HEAD_COUNT, action_dim), dtype=np.float32),
...
def _resize_exact_action_heads(np, model: dict, output_size: int, signatures: list[str]) -> bool:
    ...
    expected = {
        ...
        "q_action_w": (VALUE_HEAD_COUNT, hidden, action_dim),
        "q_action_b": (VALUE_HEAD_COUNT, action_dim),
    }
...
def load_model(np, path: Path, input_dim: int, hidden_size: int, output_size: int, action_list: list[dict]) -> tuple[dict, bool]:
    ...
    try:
        ...
        with np.load(path, allow_pickle=False) as data:
            ...
            if archive_schema not in (15, MODEL_SCHEMA):
                raise ValueError("模型版本不兼容")
            legacy_model = archive_schema == 15
            model = {"schema": MODEL_SCHEMA}
            scalar_ints = ("input_dim", "hidden_size", "output_size", "temporal_frames", "trained_samples", "training_rounds", "online_updates")
            for key in scalar_ints:
                model[key] = int(data[key][0])
            arrays = (
            ...
            for key in arrays:
                dtype = np.int8 if key == "conv_w" else (np.int16 if key == "action_factors" else np.float32)
                model[key] = data[key].astype(dtype, copy=True)
            model["action_hash"] = str(data["action_hash"][0])
            model["action_signatures"] = [str(v) for v in data["action_signatures"].tolist()]
            ...
            if legacy_model:
                ...
                changed = True
            if int(model.get("temporal_frames", 0)) != TEMPORAL_FRAMES:
            ...
            for key, value in probe.items():
                if key in resizable:
                    continue
                if isinstance(value, np.ndarray) and (key not in model or model[key].shape != value.shape):
                    raise ValueError(f"模型数组尺寸无效:{key}")
    except Exception:
        if path.exists():
            backup_corrupt(path)
        model = initialize_model(np, input_dim, hidden_size, output_size)
        changed = True
    if _resize_exact_action_heads(np, model, output_size, signatures):
        changed = True
    expected_factors = action_factor_matrix(np, action_list)
    if model["action_factors"].shape != expected_factors.shape or not np.array_equal(model["action_factors"], expected_factors):
    ...
    _synchronize_quantized_conv_weights(np, model)
    if not np.array_equal(previous_w, model["conv_w"]) or abs(previous_scale - float(model["conv_scale"][0])) > 1e-8:
        changed = True
...
def verify_model_archive(np, path: Path, model: dict, signatures: list[str]) -> None:
    ...
    with np.load(path, allow_pickle=False) as data:
        ...
        required_arrays = (
            ...
            "value_b", "progress_w", "progress_b", "policy_skill_w", "policy_skill_b",
...
def refresh_global_prior(np, index: dict, config: dict, stop_event: threading.Event | None) -> bool:
    ...
    for profile_id in sorted(index.get("profiles",{})):
        ...
        try:
            ...
            model,_=load_model(np,paths["model"],MODEL_INPUT_DIM,int(adaptive_runtime_settings(config)["hidden_size"]),len(profile["actions"]),profile["actions"])
    ...
    seed=initialize_model(np,MODEL_INPUT_DIM,int(adaptive_runtime_settings(config)["hidden_size"]),1)
...
def _model_hidden_state_from_vector(
    ...
    for index, frame in enumerate(vector["frames"]):
        ...
        if 0 <= action_value < len(model["action_factors"]):
            ...
            duration_factor = int(factors[2]) / max(1, DURATION_HEAD_SIZE - 1)
            key_density = min(
                1.0,
        ...
        context = np.asarray(
            [
                ...
                *kind_values,
                ...
                duration_factor,
                math.sin(mouse_value * math.pi),
                math.cos(mouse_value * math.pi),
                index / max(1, len(vector["frames"]) - 1),
                1.0,
            ],
            dtype=np.float32,
        )
        ...
        hidden = (1.0 - update) * previous_hidden + update * candidate
        if return_cache:
            caches.append({
                "combined": combined,
                "projected": projected,
...
def _backprop_convolution(np, model: dict, conv_cache: dict | None, visual_gradient, gradients: dict[str, object]) -> None:
    ...
    gradient = np.asarray(visual_gradient, dtype=np.float32)
...
def _backprop_temporal_encoder(
    ...
    if local:
        gradients = {
            ...
            for key in (*_CONV_PARAMETER_KEYS, "frame_proj", "frame_bias", "Wz", "Uz", "bz", "Wr", "Ur", "br", "Wh", "Uh", "bh")
        }
...
def recurrent_model_step(
    ...
    vector = {
        "frames": np.stack([_frame_channels(np, frame)], axis=0),
...
def _model_head_outputs(np, model: dict, hidden):
    ...
    q = (
        np.einsum("h,vhc->vc", hidden, model["q_control_w"]) + model["q_control_b"],
        np.einsum("h,vhk->vk", hidden, model["q_key_w"]) + model["q_key_b"],
        np.einsum("h,vhm->vm", hidden, model["q_mouse_w"]) + model["q_mouse_b"],
        np.einsum("h,vhb->vb", hidden, model["q_button_w"]) + model["q_button_b"],
        np.einsum("h,vhd->vd", hidden, model["q_duration_w"]) + model["q_duration_b"],
    )
    return policy, q
...
def model_outputs(np, model: dict, feature: bytes):
    ...
    return probabilities, np.clip(q_values.mean(axis=0), -1.0, 1.0)



def model_ensemble_outputs(np, model: dict, target_model: dict, feature: bytes, target_weight: float):
    ...
    values = online_q.mean(axis=0) * (1.0 - weight) + target_q.mean(axis=0) * weight
    disagreement = np.abs(online_q.mean(axis=0) - target_q.mean(axis=0))
    uncertainty = np.std(np.concatenate((online_q, target_q), axis=0), axis=0)
    return probabilities.astype(np.float32), np.clip(values, -1.0, 1.0), np.clip(uncertainty, 0.0, 2.0), np.clip(disagreement, 0.0, 2.0)
...
def recurrent_ensemble_outputs(
    ...
    probabilities /= max(1e-12, float(probabilities.sum()))
    online_values = online_q.mean(axis=0)
    target_values = target_q.mean(axis=0)
    ...
    return (
        ...
        np.clip(uncertainty, 0.0, 2.0),
        np.clip(disagreement, 0.0, 2.0),
    )
...
def soft_update_target_model(np, target: dict, source: dict, rate: float) -> None:
    ...
    for key, source_value in source.items():
        ...
        elif key in ("trained_samples", "training_rounds", "online_updates", "action_hash", "action_signatures", "runtime_tier", "validation_score"):
...
def online_model_update(np, model: dict, feature: bytes, action_index: int, reward: float, learning_rate: float, next_feature: bytes | None = None, discount: float = 0.0, terminal: bool = False, target_model: dict | None = None) -> bool:
    ...
    hidden, caches = _model_hidden_state_from_vector(np, model, vector, return_cache=True)
    ...
    target = temporal_difference_target(np, model, reward, next_feature, discount, terminal, target_model)
    q_data = float(np.min(q_values[:, int(action_index)]))
    ...
    _, q_hidden_gradient = _train_factor_q(
        np, model, hidden, int(action_index), target, lr, 1.0,
    ...
    hidden_gradient = value_hidden_gradient + q_hidden_gradient + policy_hidden_gradient
    ...
    if active:
        ...
        for key, gradient in gradients.items():
            if key in _TEMPORAL_PARAMETER_KEYS:
                scale = 0.03
            elif key in _CONV_PARAMETER_KEYS:
                scale = 0.0
            else:
                scale = 1.0
            model[key] -= lr * scale * gradient
        _synchronize_quantized_conv_weights(np, model)
...
def choose_policy_action(
    ...
    if random.random() < max(0.0, min(0.8, float(exploration))):
        weights = probabilities[candidates] + 0.03
        weights /= weights.sum()
...
def balanced_trajectory_records(np, records: list[dict], limit: int, rng) -> list[dict]:
    maximum = max(1, int(limit))
    if len(records) <= maximum:
        return list(records)
    groups = {
        "human": [record for record in records if record.get("trajectory_class") == "human"],
        "successful_ai": [
            record
        ...
        "ordinary_ai": [
            record
            for record in records
            if record.get("trajectory_class") == "ordinary_ai"
        ],
    }
    quotas = {
        "human": int(round(maximum * 0.50)),
        "successful_ai": int(round(maximum * 0.30)),
    }
    quotas["ordinary_ai"] = maximum - quotas["human"] - quotas["successful_ai"]
    selected: list[dict] = []
    leftovers: list[dict] = []
    for name in ("human", "successful_ai", "ordinary_ai"):
        bucket = groups[name]
        quota = max(0, quotas[name])
        if len(bucket) <= quota:
            selected.extend(bucket)
            continue
        weights = np.asarray(
            [max(0.05, float(record.get("priority", 0.05))) for record in bucket],
            dtype=np.float64,
        )
        weights /= max(1e-12, float(weights.sum()))
        chosen = set(
            int(value)
            for value in rng.choice(len(bucket), size=quota, replace=False, p=weights)
        )
        selected.extend(record for index, record in enumerate(bucket) if index in chosen)
        leftovers.extend(record for index, record in enumerate(bucket) if index not in chosen)
    missing = maximum - len(selected)
    if missing > 0 and leftovers:
        weights = np.asarray(
            [max(0.05, float(record.get("priority", 0.05))) for record in leftovers],
            dtype=np.float64,
        )
        weights /= max(1e-12, float(weights.sum()))
        indices = rng.choice(
            len(leftovers),
            size=min(missing, len(leftovers)),
            replace=False,
            p=weights,
        )
        selected.extend(leftovers[int(index)] for index in indices)
...
def load_training_data(
    ...
    try:
        episode_rows = connection.execute(
            "SELECT episode_id,COUNT(*),SUM(priority),MAX(rowid) FROM transitions GROUP BY episode_id ORDER BY SUM(priority) DESC,MAX(rowid) DESC"
        ).fetchall()
        selected = []
        selected_count = 0
        limit = max(32, int(sample_limit))
        for episode_id, count, priority_sum, last_row in episode_rows:
            if stop_event is not None and stop_event.is_set(): raise RuntimeError("操作已取消")
            selected.append(str(episode_id)); selected_count += int(count)
            if selected_count >= limit * 2: break
        if not selected:
            return None
        ...
        raise_if_sqlite_cancelled(error, stop_event); raise
    finally:
        connection.close()
    ...
    for (
        ...
        try:
            action_id = int(action); stored_reward = float(reward); done_value = bool(int(done))
            ...
            if source not in ("human", "ai") or not 0 <= action_id < maximum_action: raise ValueError
            state_blob = _normalize_temporal_action_schema(bytes(state), action_count)
            decode_temporal_state(state_blob)
            next_blob = _normalize_temporal_action_schema(bytes(next_state), action_count) if next_state is not None else None
            if next_blob is not None:
                decode_temporal_state(next_blob)
            if not done_value and next_blob is None: raise ValueError
            episodes.setdefault(str(episode_id), []).append({
                "episode_id": str(episode_id), "step": int(step), "source": str(source), "state": state_blob,
                "action": action_id, "reward": max(-1.0, min(1.0, reward_value)), "next_state": next_blob,
                "done": done_value, "priority": max(0.05, float(priority)),
                "task_reward": task_value,
                "exploration_reward": exploration_value,
                "safety_penalty": safety_value,
            })
    ...
    for records in episodes.values():
        ...
        trajectory_class = (
            ...
            if task_total > safety_total * 0.75 and reward_mean > 0.01
            else "ordinary_ai"
        )
        for record in records:
    ...
    for episode_id, records in episodes.items():
        ...
        for index, record in enumerate(records):
            total=0.0; task_total=0.0; exploration_total=0.0; safety_total=0.0
            multiplier=1.0; n_next=record["next_state"]; n_done=record["done"]; steps_used=1
            for offset in range(horizon):
                position=index+offset
                if position>=len(records): break
                ...
                steps_used=offset+1; n_next=item["next_state"]; n_done=bool(item["done"])
                if n_done: break
                multiplier *= discount
    ...
    if not validation_records and len(episodes)>1:
        one_episode=next(iter(episodes)); moved=[r for r in train_records if r["episode_id"]==one_episode]
        train_records=[r for r in train_records if r["episode_id"]!=one_episode]; validation_records.extend(moved)
    if not train_records and validation_records:
        one_episode=validation_records[0]["episode_id"]; moved=[r for r in validation_records if r["episode_id"]==one_episode]
        validation_records=[r for r in validation_records if r["episode_id"]!=one_episode]; train_records.extend(moved)
    rng=np.random.default_rng(0xA61)
    train_records=balanced_trajectory_records(np,train_records,sample_limit,rng)
    validation_limit=max(64,min(4096,sample_limit//4))
    if len(validation_records)>validation_limit:
        indices=rng.choice(len(validation_records),size=validation_limit,replace=False)
        validation_records=[validation_records[int(i)] for i in indices]
    return {"train":train_records,"validation":validation_records,"invalid":invalid,"episodes":len(episodes),"horizon":horizon}
...
_TEMPORAL_PARAMETER_KEYS = frozenset({
    "frame_proj", "frame_bias", "Wz", "Uz", "bz", "Wr", "Ur", "br", "Wh", "Uh", "bh",
})
...
def _train_factor_q(
    ...
    step = max(0.0, min(0.02, float(learning_rate)))
    if q_values is None:
        _, q_values, _ = factorized_action_outputs_from_hidden(np, model, hidden)
    local = gradients is None
    if local:
        gradients = _training_gradient_buffer(np, model)
    factor_matrix = model["action_factors"].astype(np.int64, copy=False)
    ...
    td_losses = []
    cql_losses = []
    ensemble_scale = 1.0 / VALUE_HEAD_COUNT
    for head in range(VALUE_HEAD_COUNT):
        values = np.asarray(q_values[head], dtype=np.float32)
        error = float(values[int(action_id)] - float(target))
        clipped_error = max(-3.0, min(3.0, error))
        action_gradient = np.zeros(len(values), dtype=np.float32)
        action_gradient[int(action_id)] = clipped_error * max(0.0, float(sample_weight)) * ensemble_scale
        conservative_gradient = _softmax_vector(np, values).astype(np.float32)
        conservative_gradient[int(action_id)] -= 1.0
        conservative_gradient *= max(0.0, float(cql_weight)) * ensemble_scale
        ...
        gradients["q_action_w"][head] += np.outer(hidden, action_gradient).astype(np.float32)
        gradients["q_action_b"][head] += action_gradient
        hidden_gradient += model["q_action_w"][head] @ action_gradient
        for w_key, b_key, column in (
            ("q_control_w", "q_control_b", 0),
            ("q_mouse_w", "q_mouse_b", 1),
        ):
            bucket_gradient = _factor_bucket_gradient(np, factor_matrix, column, action_gradient / 4.0, model[b_key].shape[1])
            gradients[w_key][head] += np.outer(hidden, bucket_gradient).astype(np.float32)
            gradients[b_key][head] += bucket_gradient
            hidden_gradient += model[w_key][head] @ bucket_gradient
        ...
        gradients["q_key_w"][head] += np.outer(hidden, key_gradient).astype(np.float32)
        gradients["q_key_b"][head] += key_gradient
        gradients["q_button_w"][head] += np.outer(hidden, button_gradient).astype(np.float32)
        gradients["q_button_b"][head] += button_gradient
        hidden_gradient += model["q_key_w"][head] @ key_gradient
        hidden_gradient += model["q_button_w"][head] @ button_gradient

        if duration_index is not None:
            kind = int(factor_matrix[int(action_id), 0])
            duration_target = max(0, min(DURATION_HEAD_SIZE - 1, int(duration_index)))
            duration_values = (
                hidden @ model["q_duration_w"][head]
                + model["q_duration_b"][head]
                + model["q_duration_kind_b"][head, kind]
            )
            duration_error = max(-3.0, min(3.0, float(duration_values[duration_target]) - float(target)))
            duration_gradient = np.zeros(DURATION_HEAD_SIZE, dtype=np.float32)
            duration_gradient[duration_target] = duration_error * max(0.0, float(sample_weight)) * ensemble_scale
            gradients["q_duration_w"][head] += np.outer(hidden, duration_gradient).astype(np.float32)
            gradients["q_duration_b"][head] += duration_gradient
            gradients["q_duration_kind_b"][head, kind] += duration_gradient
            hidden_gradient += model["q_duration_w"][head] @ duration_gradient
            td_losses.append(0.5 * duration_error * duration_error)
    loss = float(sum(td_losses) / max(1, len(td_losses)) + max(0.0, float(cql_weight)) * sum(cql_losses) / max(1, len(cql_losses)))
    if local:
        active = [gradient for gradient in gradients.values() if np.any(gradient)]
        if active:
            clip_gradients_by_global_norm(np, active, 6.0)
...
def _adam_apply(np, model: dict, gradients: dict[str, object], state: dict, learning_rate: float) -> None:
    state["step"] = int(state.get("step", 0)) + 1
    step = state["step"]
    beta1 = 0.9
    beta2 = 0.999
    epsilon = 1e-8
    moments = state.setdefault("m", {})
    velocities = state.setdefault("v", {})
    for key, gradient in gradients.items():
        if not np.any(gradient):
            continue
        if key not in moments:
            moments[key] = np.zeros_like(gradient, dtype=np.float32)
            velocities[key] = np.zeros_like(gradient, dtype=np.float32)
        moments[key] *= beta1
        moments[key] += (1.0 - beta1) * gradient
        velocities[key] *= beta2
        velocities[key] += (1.0 - beta2) * gradient * gradient
        corrected_m = moments[key] / (1.0 - beta1 ** step)
        corrected_v = velocities[key] / (1.0 - beta2 ** step)
        scale = 0.35 if key in _TEMPORAL_PARAMETER_KEYS else (0.12 if key in _CONV_PARAMETER_KEYS else 1.0)
        model[key] -= float(learning_rate) * scale * corrected_m / (np.sqrt(corrected_v) + epsilon)
...
def _batch_model_outputs_from_hidden(np, model: dict, hidden_matrix):
    ...
    q_control = np.einsum("bh,vhc->bvc", hidden_matrix, model["q_control_w"]) + model["q_control_b"][None, :, :]
    q_key = np.einsum("bh,vhk->bvk", hidden_matrix, model["q_key_w"]) + model["q_key_b"][None, :, :]
    q_mouse = np.einsum("bh,vhm->bvm", hidden_matrix, model["q_mouse_w"]) + model["q_mouse_b"][None, :, :]
    q_button = np.einsum("bh,vhu->bvu", hidden_matrix, model["q_button_w"]) + model["q_button_b"][None, :, :]
    factor_q = (
        q_control[:, :, factors[:, 0]]
        + q_mouse[:, :, factors[:, 1]]
        + np.einsum("bvk,ak->bva", q_key, key_matrix, optimize=True)
        + np.einsum("bvu,au->bva", q_button, button_matrix, optimize=True)
    ) / 4.0
    exact_q = np.einsum("bh,vha->bva", hidden_matrix, model["q_action_w"]) + model["q_action_b"][None, :, :]
    q_values = factor_q + exact_q
...
def factorized_action_outputs_from_hidden(np, model: dict, hidden):
    probabilities, q_values, _ = _batch_model_outputs_from_hidden(np, model, np.asarray(hidden, dtype=np.float32)[None, :])
...
def sequence_training_windows(
    ...
    length = max(32, min(128, int(sequence_length)))
    burn = max(0, min(min(32, length - 1), int(burn_in_steps)))
    episodes: dict[str, list[dict]] = {}
    for record in records:
    ...
    for episode_records in episodes.values():
        ...
        for start in range(0, len(episode_records), length):
            context_start = max(0, start - burn)
            train_end = min(len(episode_records), start + length)
            lookahead = max((int(item.get("n_steps", 1)) for item in episode_records[start:train_end]), default=1)
            end = min(len(episode_records), train_end + lookahead)
            window = episode_records[context_start:end]
            if window:
...
def evaluate_model_records(np, model: dict, items: list[dict]) -> dict[str, float]:
    ...
    for record in sorted(items, key=lambda item: (str(item["episode_id"]), int(item["step"]))):
        ...
        if record["source"] == "human":
            human_total += 1
            human_correct += int(int(np.argmax(probabilities)) == action_id)
            human_log_probabilities.append(math.log(probability))
        q_data = float(np.min(q_values[:, action_id]))
        q_mean = float(q_values[:, action_id].mean())
        value = float(hidden @ model["value_w"][:, 0] + model["value_b"][0])
        target_return = float(record.get("n_return", record.get("reward", 0.0)))
        value_errors.append(abs(q_data - value))
        q_errors.append(abs(q_mean - target_return))
        safe_target = 1.0 - max(0.0, min(1.0, float(record.get("safety_penalty", 0.0)) / 1.0))
        safe_prediction = float(_sigmoid(np, np.asarray([q_mean], dtype=np.float32))[0])
        safety_errors.append(abs(safe_prediction - safe_target))
        episode_outputs.append({
            "source": str(record["source"]), "progress": predict_progress(np, model, hidden),
            "return": target_return, "value": value,
        })
    finish_episode()
    ...
    score = (
        0.35 * normalized_log_likelihood
        + 0.35 * return_ranking_accuracy
        - 0.15 * q_calibration_error
        - 0.15 * safety_error
    )
    return {
...
def train_model(
    ...
    sequence_length = max(32, min(128, int(config.get("sequence_length", TRAIN_SEQUENCE_LENGTH))))
    burn_in = max(4, min(min(32, sequence_length - 1), int(config.get("burn_in_steps", TRAIN_BURN_IN_STEPS))))
    cql_weight = 0.02
    rng = np.random.default_rng()
    optimizer = {"step": 0, "m": {}, "v": {}}
    ...
    def forward_window(window: list[dict], encoder_model: dict, with_cache: bool):
        ...
        for record in window:
            vector = feature_vector(np, record["state"])
            if with_cache:
                hidden, caches = _model_hidden_state_from_vector(
                    np, encoder_model, vector, return_cache=True, initial_hidden=hidden
                )
            else:
                hidden = _model_hidden_state_from_vector(
                    np, encoder_model, vector, return_cache=False, initial_hidden=hidden
                )
                caches = None
    ...
    def apply_gradients(gradients, divisor: int):
        ...
        if active:
            ...
            _adam_apply(np, model, gradients, optimizer, lr)
            _synchronize_quantized_conv_weights(np, model)
        ...
        for group_start in range(0, len(bc_windows), windows_per_batch):
            ...
            for window, first_train in bc_windows[group_start:group_start + windows_per_batch]:
                ...
                for local_index, record in enumerate(window[first_train:train_end]):
                    ...
                    action_id = int(record["action"])
                    quality, _, _ = human_training_signal(float(record["task_reward"]))
                ...
                if train_end - first_train >= 5:
                    earlier_position = first_train
                    later_position = train_end - 1
                    progress_loss, earlier_gradient, later_gradient = _train_progress_pair(
                        np, model, encoded[earlier_position][0], encoded[later_position][0],
                        progress_margin, progress_weight, gradients,
                    )
                    hidden_gradients[earlier_position] += earlier_gradient
                    hidden_gradients[later_position] += later_gradient
        ...
        for group_start in range(0, len(all_windows), windows_per_batch):
            ...
            for window, first_train in all_windows[group_start:group_start + windows_per_batch]:
                ...
                for local_index, record in enumerate(train_items):
                    ...
                    action_id = int(record["action"])
                    duration_index = _record_duration_index(record)
                    q_data = float(np.min(q_values[local_index, :, action_id]))
                    value = float(values[local_index])
                    _, _, value_weight = human_training_signal(float(record["task_reward"])) if record["source"] == "human" else (1.0, 0.0, 1.0)
                    value_loss, value_hidden_gradient = _train_value_head(
                        np, model, hidden, q_data, value, expectile, value_weight, gradients,
                    )
                    future_position = position + int(record.get("n_steps", 1))
                    has_future = not record["n_done"] and future_position < len(target_encoded)
                    next_value = 0.0
                    if has_future:
                        future_hidden = target_encoded[future_position][0]
                        next_value = float(future_hidden @ target_model["value_w"][:, 0] + target_model["value_b"][0])
                    # n_return already contains the executed trajectory's n-step reward.
                    target = float(record["n_return"])
                    if has_future:
                        target += float(record["discount_power"]) * next_value
                    target = max(-3.0, min(3.0, target))
                    priority_weight = min(3.0, max(0.2, float(record["priority"]) / 3.0))
                    q_loss, q_hidden_gradient = _train_factor_q(
                        np, model, hidden, action_id, target, lr, priority_weight,
                        gradients=gradients, q_values=q_values[local_index], cql_weight=cql_weight,
                        duration_index=duration_index,
                    )
                    advantage = q_data - value
                    actor_weight = min(100.0, math.exp(min(5.0, advantage / temperature)))
                    trajectory_scale = 1.0 if record["source"] == "human" else 0.45 if record.get("trajectory_class") == "successful_ai" else 0.08
                    ...
                    hidden_gradients[position] += policy_hidden_gradient + q_hidden_gradient + value_hidden_gradient
...
def runtime_self_check(np) -> None:
    ...
    if (
        ...
        or model["q_action_w"].shape != (VALUE_HEAD_COUNT, model["hidden_size"], expected)
    ...
    hidden_cached, caches = _model_hidden_state_from_vector(np, model, feature_vector(np, state), return_cache=True)
    gradients = _training_gradient_buffer(np, model)
    ...
    before = float(target["q_control_b"][0, 0])
    model["q_control_b"][0, 0] += 0.8
    soft_update_target_model(np, target, model, 0.25)
    if abs(float(target["q_control_b"][0, 0]) - (before + 0.2)) > 1e-5:
    ...
    online_model_update(np, model, state, 0, 0.4, 0.002, state, 0.9, False, target)
    ...
    progress_config["task_reward_weight"] = 1.0
...
def validate_model_file(path: Path, profile: dict, config: dict) -> bool:
    np=import_numpy(); model,changed=load_model(np,path,MODEL_INPUT_DIM,int(adaptive_runtime_settings(config)["hidden_size"]),len(profile["actions"]),profile["actions"])
...
def repair_profile(profile_id: str, config: dict, stop_event: threading.Event | None) -> dict:
    ...
    np=import_numpy(); model,changed=load_model(np,paths["model"],MODEL_INPUT_DIM,int(adaptive_runtime_settings(config)["hidden_size"]),len(profile["actions"]),profile["actions"])
    if changed or not paths["model"].is_file(): save_model(np,paths["model"],model);repaired+=1
    best,best_changed=load_model(np,paths["best_model"],MODEL_INPUT_DIM,int(adaptive_runtime_settings(config)["hidden_size"]),len(profile["actions"]),profile["actions"])
    ...
    target,changed_target=load_model(np,paths["target_model"],MODEL_INPUT_DIM,int(adaptive_runtime_settings(config)["hidden_size"]),len(profile["actions"]),profile["actions"])
    ...
    best_target,best_target_changed=load_model(np,paths["best_target_model"],MODEL_INPUT_DIM,int(adaptive_runtime_settings(config)["hidden_size"]),len(profile["actions"]),profile["actions"])
    ...
    if paths["candidate_model"].exists():
        candidate,candidate_changed=load_model(np,paths["candidate_model"],MODEL_INPUT_DIM,int(adaptive_runtime_settings(config)["hidden_size"]),len(profile["actions"]),profile["actions"])
        if candidate_changed:
            save_model(np,paths["candidate_model"],best);repaired+=1
    if paths["candidate_target_model"].exists():
        candidate_target,candidate_target_changed=load_model(np,paths["candidate_target_model"],MODEL_INPUT_DIM,int(adaptive_runtime_settings(config)["hidden_size"]),len(profile["actions"]),profile["actions"])
...
def record_human_session(target: int, stop_event: threading.Event) -> str:
    ...
    try:
        while not stop_event.is_set():
            ...
            pre_model=cursor_aware_frame(target,pre);pre_feature=make_feature(pre_model,previous_model_frame,pre_blue,pre_red)
            ...
            if black:
                ...
                post_model=cursor_aware_frame(target,post);post_feature=make_feature(post_model,pre_model,post_blue,post_red)
                ...
                done=1 if metrics["confirmed_death"]>0.5 else 0
...
def train_all_profiles(stop_event: threading.Event) -> str:
    ...
    training_hidden_size=int(runtime_training["hidden_size"])
    for profile_id in sorted(index.get("profiles",{})):
        ...
        try:
            ...
            skills=extract_human_skills(list(dataset.get("train",[]))+list(dataset.get("validation",[])),training_config)
            profile["skills"]=skills
            dataset["skills"]=skills
            validation_records=list(dataset.get("validation",[]))
            baseline=evaluate_model_records(np,incumbent,validation_records) if validation_records else {"score":float(incumbent.get("validation_score",-1e9))}
            ...
            minimum_gain=max(0.0,min(0.25,float(config["candidate_min_improvement"])))
            first_model=int(incumbent.get("training_rounds",0))<=0
            accepted=first_model or bool(validation_records) and candidate_score>=baseline_score+minimum_gain
            if accepted:
            ...
            summaries.append(f"{profile['name']}：{metrics['samples']}条/{metrics['episodes']}回合，技能{metrics['skills']}个，进度排序{metrics['validation_progress_accuracy']:.0%}，回报排序{metrics['validation_return_ranking_accuracy']:.0%}，{decision}")
    ...
    return f"升级完成：{total_profiles} 个候选模型通过验证，训练 {total_samples} 条完整转移；清理 {total_removed} 条；best_model 可自动回滚"+("；已更新跨游戏因子先验" if prior_updated else "")+"。"+detail
...
def _flush_online_n_step(
    ...
    requested_batch = max(4, min(128, int(batch_size)))
    ...
    samples = random.sample(population, count) if len(population) > count else population
    updated = 0
    for sample in samples:
        effective_lr = float(learning_rate) * float(sample["reliability"]) / math.sqrt(max(1, count))
        if online_model_update(
            np, model, sample["state"], sample["action"], sample["reward"],
            effective_lr, sample["next_state"], sample["discount"], sample["terminal"], target_model,
        ):
            updated += 1
    return updated
...
def run_ai_session(target: int, stop_event: threading.Event) -> str:
    ...
    prior = load_global_prior(np)
    active_path = paths["best_model"] if paths["best_model"].is_file() else paths["model"]
    model_exists = active_path.is_file()
    if model_exists:
        ...
        changed = True
    active_target_path = (
        paths["best_target_model"]
        if paths["best_target_model"].is_file()
        else paths["target_model"]
    )
    ...
    if target_changed and int(target_model.get("trained_samples", 0)) == 0:
    ...
    if changed or not paths["best_model"].is_file():
        save_model(np, paths["best_model"], model)
        save_model(np, paths["model"], model)
    if target_changed or not paths["best_target_model"].is_file():
        save_model(np, paths["best_target_model"], target_model)
        save_model(np, paths["target_model"], target_model)
    ...
    rows: list[tuple] = []
    online_queue = deque()
    online_replay = deque(maxlen=max(32, min(8192, int(config["online_replay_capacity"]))))
    active_skill = deque()
        ...
        while not stop_event.is_set():
            ...
            if replacement:
                _flush_online_n_step(
                    ...
                    online_td_discount,
                    online_replay,
                    int(config["online_replay_batch_size"]),
            ...
            if selected_base is not None and 0 <= selected_base < action_count and selected_base not in blocked_action_ids:
                ...
                action_id = choose_policy_action(
                    ...
                    uncertainty=uncertainty + disagreement,
            ...
            if not 0 <= base_action < action_count:
                ...
                base_action = 0
            duration_index = (
                max(0, min(DURATION_HEAD_SIZE - 1, int(selected_skill_duration)))
                if selected_skill_duration is not None
                else select_conditional_duration(np, model, model_hidden, base_action, exploration * 0.35)
            )
            ...
            if not execute_action(
                target,
                refine_pointer_action(profile["actions"][base_action], predict_mouse_offset(np, model, model_hidden)),
                hold,
                mouse_step,
                stop_event,
            ):
            ...
            if black:
                ...
                reward = compose_reward(metrics, config)
                done = 1 if float(metrics["confirmed_death"]) > 0.5 else 0
            ...
            if model_changed and steps % checkpoint == 0:
                ...
                save_model(np, paths["candidate_model"], model)
                save_model(np, paths["candidate_target_model"], target_model)
                model_changed = False
