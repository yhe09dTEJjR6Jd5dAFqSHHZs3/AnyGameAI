...
RELEASE_SOURCE_SHA256 = "372e50aaf99a9ce8a7d144d2322c92640f43c2a4eb9bce36057a8ee6043842ba"
...
def ensure_runtime_ready(stop_event: threading.Event | None = None):
    ...
    try:
        ...
        if str(error) == "操作已取消":
            raise
        raise RuntimeError("本地运行组件缺失、损坏或尚未校验；请先点击“文件”。") from error
    np = import_numpy()
...
def temporal_difference_target(
    ...
    expected_next_value = float(
        np.dot(
            ...
            next_values.astype(np.float64, copy=False),
        )
    )
...
def runtime_self_check(np) -> None:
    if (
        ...
        or release_version_tuple(APP_VERSION) < (57, 0, 0)
        or CONFIG_SCHEMA < 26
    ...
    if not (
        ...
        raise RuntimeError("不确定性感知时序差分自检失败")
